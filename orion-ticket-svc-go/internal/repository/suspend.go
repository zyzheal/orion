...
func (r *SuspendRepository) Create(s *models.SuspendRecord) error {
...
func (r *SuspendRepository) GetByID(id string) (*models.SuspendRecord, error) {
	var s models.SuspendRecord
	err := r.db.Get(&s, "SELECT * FROM suspend_records WHERE id = $1", id)
	if err != nil {
		return nil, err
	}
	return &s, nil
...
func (r *SuspendRepository) Update(s *models.SuspendRecord) error {
	query := `UPDATE suspend_records SET status=$1, activated_at=$2, ended_at=$3, cancelled_at=$4, updated_at=NOW()
		WHERE id=$5`
	_, err := r.db.Exec(query, s.Status, s.ActivatedAt, s.EndedAt, s.CancelledAt, s.ID)
	return err
}

func (r *SuspendRepository) ListByStatus(status string) ([]models.SuspendRecord, error) {
	var records []models.SuspendRecord
	if status == "" {
		err := r.db.Select(&records, "SELECT * FROM suspend_records ORDER BY created_at DESC")
		return records, err
	}
	err := r.db.Select(&records, "SELECT * FROM suspend_records WHERE status = $1 ORDER BY created_at DESC", status)
	return records, err
}

func (r *SuspendRepository) ListByEngineer(engineerID string) ([]models.SuspendRecord, error) {
	var records []models.SuspendRecord
...
func (r *SuspendRepository) FindActiveByEngineer(engineerID string) (*models.SuspendRecord, error) {
	var s models.SuspendRecord
	err := r.db.Get(&s,
...
func (r *SuspendRepository) CountPendingByEngineer(engineerID string) (int, error) {
	var count int
	err := r.db.Get(&count,
...
func (r *SuspendRepository) CountActiveByEngineer(engineerID string) (int, error) {
	var count int
	err := r.db.Get(&count,
