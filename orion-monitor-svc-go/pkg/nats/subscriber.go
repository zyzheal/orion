...
// NATSSubscriber subscribes to MetricEvent via NATS JetStream.
...
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "monitor-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received MetricEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleMetricEvent(ctx, &event); err != nil {
		s.log.Error("handle metric event", zap.Error(err))
...
func (s *NATSSubscriber) handleMetricEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的监控操作
	// 例如：MetricCollected → 写入时序数据库
	//       AlertTriggered → 生成告警记录
	return nil
}
