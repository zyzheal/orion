...
// NATSSubscriber subscribes to FinOpsEvent via NATS JetStream.
...
// Start subscribes to FinOpsEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "finops-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received finops event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleFinOpsEvent(ctx, &event); err != nil {
		s.log.Error("handle finops event", zap.Error(err))
...
func (s *NATSSubscriber) handleFinOpsEvent(ctx context.Context, event *EventBusEvent) error {
