...
### Added
- **`docs/architecture/plugin-framework-design.md`** — 插件框架设计 (微内核/生命周期/安全沙箱)
- **`docs/iac/iac-management-design.md`** — IaC 管理设计 (Terraform 集成/State 管理/AI 审查)
- **`docs/sre/oncall-scheduling-design.md`** — OnCall 排班系统设计 (排班规则/告警路由/升级策略)
- **`docs/architecture/tool-marketplace-design.md`** — 工具市场设计 (工具发现/安装/升级/依赖解析)
- **`docs/architecture/hotfix-channel-design.md`** — Hotfix 通道设计 (紧急发布/分支策略/快速验证)
- **`docs/ai/skill-marketplace-design.md`** — Skill 市场设计 (Skill 发现/安装/评分/审核)
- `docs/ai/测试推荐效果评估设计.md` — 测试推荐效果评估 (准确率/采纳率/A-B 测试/反馈收集)
...
### Added
- `docs/ai/代码规范规则引擎设计.md` — 代码规范规则引擎设计 (DSL/热更新/优先级/AI 集成)
- `docs/ai/测试用例生成设计.md` — 测试用例生成设计 (变更分析/覆盖率缺口/推荐算法)
- `docs/knowledge/Orion-Knowledge 集成方案.md` — Orion-Knowledge 集成方案 (RAG 对接/自动积累/SSO)
- `docs/efficiency/自动周报设计.md` — 自动周报设计 (数据采集/模板引擎/定时调度)
- `docs/sre/OnCall 排班系统设计.md` — OnCall 排班系统 (排班规则/告警升级/通知渠道)
- `docs/collaboration/新人 Onboarding 设计.md` — 新人 Onboarding 设计 (任务清单/导师配对/进度追踪)
...
### Added
- **`docs/ai/mlops-and-ml-frameworks-design.md`** — MLOps 与 ML 框架设计 (PyTorch/TensorFlow/MLflow/Kubeflow)
- **`docs/ai/gnn-and-rl-design.md`** — GNN 与强化学习设计 (根因分析/自愈决策/部署优化)
- **`docs/ai/code-representation-learning-design.md`** — 代码表示学习设计 (AST/Code2Vec/CodeBERT/LangChain)
- `docs/db/sql-audit-design.md` — SQL 审计详细设计 (Yearning + gemini-next 集成)
- `docs/knowledge/knowledge-base-design.md` — 知识库详细设计 (RAG/向量数据库/知识图谱)
...
- **`docs/db/database-migration-and-query-design.md`** — 数据库迁移与查询设计 (Flyway+CQRS 模式)
...
- `docs/db/sql-audit-design.md` — 更新服务名称为 orion-dba
...
- **`docs/ai/ai-cost-control-design.md`** — AI 成本控制设计 (Token 计费/预算管理/成本优化)
- **`docs/architecture/configuration-drift-detection-design.md`** — 配置漂移检测与回滚设计 (GitOps 漂移检测/自动回滚)
...
- **`docs/db/database-migration-strategy-design.md`** — 数据库迁移策略设计 (Flyway/零停机/一致性保障)
- **`docs/architecture/cache-layer-design.md`** — 缓存层设计 (Redis/失效策略/一致性/热点 Key)
- **`docs/knowledge/orion-knowledge-integration-design.md`** — Orion-Knowledge 集成设计 (Nginx/SSO/RAG API)
