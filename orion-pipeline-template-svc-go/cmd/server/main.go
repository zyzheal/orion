...
import (
	...
	"orion/pipeline-template-svc-go/internal/config"
	"orion/pipeline-template-svc-go/internal/handler"
	"orion/pipeline-template-svc-go/internal/repository"
	"orion/pipeline-template-svc-go/internal/service"
	"orion/go-common/pkg/auth"
	 nats_subscriber "orion/pipeline-template-svc-go/internal/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-pipeline-template-svc"))
	defer logger.Sync()
	...
	svc.InitializeBuiltinTemplates(ctx)
	...
	r.GET("/healthz", middleware.HealthCheck("orion-pipeline-template-svc"))

	addr := fmt.Sprintf(":%d", cfg.Port)
	logger.Info("pipeline-template-svc listening", zap.String("addr", addr))
