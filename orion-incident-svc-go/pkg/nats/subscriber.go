...
// Start subscribes to IncidentEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "incident-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received IncidentEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleIncidentEvent(ctx, &event); err != nil {
		s.log.Error("handle incident event", zap.Error(err))
...
func (s *NATSSubscriber) handleIncidentEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的 Incident 操作
	// 例如：IncidentCreated → 创建响应计划
	//       IncidentResolved → 通知相关方
	return nil
}
