...
import (
	...
	"orion/incident-svc-go/internal/selfhealing/service"
...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	r := rg.Group("/rules")
	r.POST("", auth.RequirePermission("selfhealing", "write"), h.Create); r.GET("", h.List); r.GET("/:id", h.Get)
	r.DELETE("/:id", auth.RequirePermission("selfhealing", "delete"), h.Delete)
	r.GET("/count", h.Count)
}

func (h *Handler) Create(c *gin.Context) {
	tenantID := c.GetString("tenant_id")
	var req models.CreateHealingRuleRequest
