...
// NATSSubscriber subscribes to DREvent via NATS JetStream.
...
// Start subscribes to DREvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "dr-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received dr event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleDREvent(ctx, &event); err != nil {
		s.log.Error("handle dr event", zap.Error(err))
...
func (s *NATSSubscriber) handleDREvent(ctx context.Context, event *EventBusEvent) error {
