...
import (
	...
	"orion/dr-svc-go/internal/config"
	"orion/dr-svc-go/internal/handler"
	"orion/dr-svc-go/internal/repository"
	"orion/dr-svc-go/internal/service"
	nats_subscriber "orion/dr-svc-go/pkg/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-dr-svc"))
	defer logger.Sync()
	...
	r.GET("/healthz", middleware.HealthCheck("orion-dr-svc"))
	...
	logger.Info("dr-svc listening", zap.String("addr", addr))
	...
	logger.Info("shutting down dr-svc...")
	if natsSub != nil {
