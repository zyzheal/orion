package config

import (
	"fmt"
	"os"

	"github.com/spf13/viper"
)

type Config struct {
	ServiceName   string
	Environment   string
	HTTPAddr      string
	DatabaseURL   string
	RedisAddr     string
	RedisDB       int
	OTelEndpoint  string
	JWTSecret     string
	BackupPath    string
	MaxStorageBytes int64
}

func Load() (*Config, error) {
	v := viper.New()
	v.SetConfigName("config")
	v.SetConfigType("yaml")
	v.AddConfigPath(".")
	v.AddConfigPath("./config")

	v.SetDefault("service_name", "orion-backup-svc")
	v.SetDefault("environment", "development")
	v.SetDefault("http_addr", ":8086")
	v.SetDefault("database_url", "postgres://orion:orion@localhost:5432/orion_backup?sslmode=disable")
	v.SetDefault("redis_addr", "localhost:6379")
	v.SetDefault("redis_db", 0)
	v.SetDefault("otel_endpoint", "")
	v.SetDefault("jwt_secret", "")
	v.SetDefault("backup_path", "/var/backups/orion")
	v.SetDefault("max_storage_bytes", int64(0))

	_ = v.ReadInConfig()
	v.AutomaticEnv()

	cfg := &Config{
		ServiceName:     getEnvOrConfig("SERVICE_NAME", v.GetString("service_name")),
		Environment:     getEnvOrConfig("ENVIRONMENT", v.GetString("environment")),
		HTTPAddr:        getEnvOrConfig("HTTP_ADDR", v.GetString("http_addr")),
		DatabaseURL:     getEnvOrConfig("DATABASE_URL", v.GetString("database_url")),
		RedisAddr:       getEnvOrConfig("REDIS_ADDR", v.GetString("redis_addr")),
		RedisDB:         v.GetInt("redis_db"),
		OTelEndpoint:    getEnvOrConfig("OTEL_ENDPOINT", v.GetString("otel_endpoint")),
		JWTSecret:       getEnvOrConfig("JWT_SECRET", v.GetString("jwt_secret")),
		BackupPath:      getEnvOrConfig("BACKUP_PATH", v.GetString("backup_path")),
		MaxStorageBytes: int64(v.GetInt64("max_storage_bytes")),
	}

	if cfg.JWTSecret == "" {
		return nil, fmt.Errorf("JWT_SECRET is required")
	}

	return cfg, nil
}

func getEnvOrConfig(envKey string, fallback string) string {
	if val := os.Getenv(envKey); val != "" {
		return val
	}
	return fallback
}
