...
import (
	...
	"orion/ci-cd-svc-go/internal/build/repository"
	"orion/ci-cd-svc-go/internal/build/service"
...
// RegisterRoutes registers all build-related routes on the given router group.
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	// Builds
	builds := rg.Group("/builds")
	builds.GET("", h.ListBuilds)
	builds.POST("", auth.RequirePermission("build", "write"), h.CreateBuild)
	...
	builds.PUT("/:id", auth.RequirePermission("build", "write"), h.UpdateBuild)
	builds.DELETE("/:id", auth.RequirePermission("build", "delete"), h.DeleteBuild)
	builds.POST("/:id/trigger", auth.RequirePermission("build", "execute"), h.TriggerBuild)
	builds.GET("/:id/status", h.GetBuildStatus)
	builds.POST("/:id/cancel", auth.RequirePermission("build", "execute"), h.CancelBuild)
	...
	// Builder Images
	images := rg.Group("/build-images")
	images.GET("", h.ListBuilderImages)
	images.POST("", auth.RequirePermission("build", "write"), h.RegisterBuilderImage)
	images.GET("/presets", h.GetBuilderImagePresets)
	images.GET("/available", h.GetAvailableBuilderImages)
	images.GET("/by-type/:type", h.GetBuilderImagesByType)
	images.GET("/:id", h.GetBuilderImage)
	images.PUT("/:id", auth.RequirePermission("build", "write"), h.UpdateBuilderImage)
	images.DELETE("/:id", auth.RequirePermission("build", "delete"), h.DeleteBuilderImage)
	images.GET("/:id/deprecate", auth.RequirePermission("build", "write"), h.DeprecateBuilderImage)
	images.GET("/:id/restore", auth.RequirePermission("build", "write"), h.RestoreBuilderImage)

	// Cache
	cache := rg.Group("/cache")
	cache.POST("/cleanup", auth.RequirePermission("build", "write"), h.CleanupCache)
	cache.POST("/cleanup-lru", auth.RequirePermission("build", "write"), h.CleanupCacheLRU)
	cache.GET("/configs", h.ListCacheConfigs)
	cache.POST("/configs", auth.RequirePermission("build", "write"), h.CreateCacheConfig)
	cache.PUT("/configs/:id", auth.RequirePermission("build", "write"), h.UpdateCacheConfig)
	cache.DELETE("/configs/:id", auth.RequirePermission("build", "delete"), h.DeleteCacheConfig)
	cache.GET("/entries", h.ListCacheEntries)
	cache.DELETE("/entries/:id", auth.RequirePermission("build", "delete"), h.DeleteCacheEntry)
}
