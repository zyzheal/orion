...
import (
	...
	"orion/federation-svc-go/internal/config"
	"orion/federation-svc-go/internal/handler"
	"orion/federation-svc-go/internal/repository"
	"orion/federation-svc-go/internal/service"
	nats_subscriber "orion/federation-svc-go/pkg/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-federation-svc"))
	defer logger.Sync()
	...
	r.GET("/healthz", middleware.HealthCheck("orion-federation-svc"))
	...
	logger.Info("federation-svc listening", zap.String("addr", addr))
	...
	logger.Info("shutting down federation-svc...")
	if natsSub != nil {
