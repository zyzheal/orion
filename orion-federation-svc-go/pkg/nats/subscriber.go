...
// NATSSubscriber subscribes to FederationEvent via NATS JetStream.
...
// Start subscribes to FederationEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "federation-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received federation event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleFederationEvent(ctx, &event); err != nil {
		s.log.Error("handle federation event", zap.Error(err))
...
func (s *NATSSubscriber) handleFederationEvent(ctx context.Context, event *EventBusEvent) error {
