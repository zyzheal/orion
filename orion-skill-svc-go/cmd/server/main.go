...
import (
	...
	"orion/skill-svc-go/internal/config"
	"orion/skill-svc-go/internal/handler"
	"orion/skill-svc-go/internal/repository"
	"orion/skill-svc-go/internal/service"
	"orion/skill-svc-go/pkg/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-skill-svc"))
	defer logger.Sync()
	...
	r.GET("/healthz", middleware.HealthCheck("orion-skill-svc"))

	addr := fmt.Sprintf(":%d", cfg.Port)
	logger.Info("skill-svc listening", zap.String("addr", addr))
	...
	logger.Info("shutting down skill-svc...")
	if natssub != nil {
		natssub.Close()
	}
