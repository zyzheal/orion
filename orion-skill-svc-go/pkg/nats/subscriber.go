...
// NATSSubscriber subscribes to SkillEvent via NATS JetStream.
...
// Start subscribes to SkillEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "skill-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received skill event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleSkillEvent(ctx, &event); err != nil {
		s.log.Error("handle skill event", zap.Error(err))
...
func (s *NATSSubscriber) handleSkillEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理技能相关事件
	return nil
}
