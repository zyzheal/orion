...
// NATSSubscriber subscribes to SchedulerEvent via NATS JetStream.
...
// Start subscribes to SchedulerEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "scheduler-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received scheduler event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleSchedulerEvent(ctx, &event); err != nil {
		s.log.Error("handle scheduler event", zap.Error(err))
...
func (s *NATSSubscriber) handleSchedulerEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理调度器相关事件
	return nil
}
