...
// NATSSubscriber subscribes to NotificationEvent via NATS JetStream.
...
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "notify-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received NotificationEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleNotificationEvent(ctx, &event); err != nil {
		s.log.Error("handle notification event", zap.Error(err))
...
func (s *NATSSubscriber) handleNotificationEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的通知操作
	// 例如：NotificationCreated → 创建通知任务
	//       NotificationRead → 标记通知已读
	return nil
}
