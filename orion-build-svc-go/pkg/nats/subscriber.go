...
// NATSSubscriber subscribes to BuildEvent via NATS JetStream.
...
// Start subscribes to BuildEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "build-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received build event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleBuildEvent(ctx, &event); err != nil {
		s.log.Error("handle build event", zap.Error(err))
...
func (s *NATSSubscriber) handleBuildEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理构建相关事件
	// 例如：PipelineCompleted -> 触发构建
	//       ArtifactPublished -> 记录制品信息
	return nil
}
