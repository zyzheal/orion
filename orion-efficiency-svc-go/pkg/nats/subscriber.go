...
// NATSSubscriber subscribes to EfficiencyEvent via NATS JetStream.
...
// Start subscribes to EfficiencyEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "efficiency-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received efficiency event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleEfficiencyEvent(ctx, &event); err != nil {
		s.log.Error("handle efficiency event", zap.Error(err))
...
func (s *NATSSubscriber) handleEfficiencyEvent(ctx context.Context, event *EventBusEvent) error {
