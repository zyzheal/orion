...
## §67 一条在册路由的 SLA 状态整条读零：`Repository.GetTicketSLAStatus` 是连通性检查，改成委托真计算并删掉占位实现（2026-08-26）

扫描起点 HEAD `a37a630f5`（§66 已提交）。目标：再扫一遍最新 HEAD 上是否还有挂在活路由上的占位实现。

### 67.1 唯一确认的活 stub：整条 SLA 状态读为 0，且返回 200

`GET /api/v1/ticketing/sla/tickets/:ticketId/status` 是活路由，注册在 `internal/ticketing/handler/handler.go:48`，挂在 `rg.Group("/ticketing")` 下，带 `auth.RequirePermission("ticketing", "read")`。调用链：

```
handler.go:48 → handler_sla_policy.go:88 Handler.GetTicketSLAStatus
  → ServiceInterface.GetTicketSLAStatus（service_interface.go:72）
  → Service.GetTicketSLAStatus（ticket_sla_policy.go）
  → RepositoryInterface.GetTicketSLAStatus
  → Repository.GetTicketSLAStatus（repository.go:512-521）   ← 占位
```
...
危害不是"错数据"，是**零数据被当成健康数据返回 200**：`ticket_id` 有值、其余全零——`target_response_time_ms` / `target_resolution_time_ms` 是 0，`response_due` / `resolution_due` 是空串，`response_ok` / `resolution_ok` 是 false。一个满是违约的队列和一个空队列在这条路由上长得完全一样，而且调用方无从分辨：数据库真挂掉时唯一能看到的失败信号是 500，而这个占位连库都通。

### 67.2 修法：委托 + 删除，不是实现

`Service.GetTicketSLAStatus` 改成一行委托 `s.GetTicketSLA(ctx, tenantID, ticketID)`，占位方法从仓库里整个删掉（5 处涟漪）。`GetTicketSLA` 是 §66 刚修完的真计算：走 `slaTargetsFor` 拿窗口、`ResponseOK` 是真比较、落库目标优先、落库违约置位不被覆盖。两条路由现在报同一个值。

**为什么是删除而不是在仓库里实现**——四条都指向同一结论：

1. `ticket_sla_tracking` 没有 `target_response_time_ms` 列（`245_ticketing_schema_fixes.sql` 建表时就没有）。要算响应窗口只能回到 `defaultSLATargets`，也就是把 §66 里 `slaTargetsFor` 专门去掉的那份硬编码再抄一份到仓库层——两个副本，创建时写一个、上报时算另一个，正是 `slaTargetsFor` 引入前那个漂移缺陷。
2. `models.TicketSLAStatus` 没有 `CreatedAt`，而 deadline 是从创建时间推的。仓库层能拿到窗口值却推不出截止时间，除非再改 DTO 和全部调用点。
3. `SLAPolicyRepository.GetTicketSLAStatus`（`repository/sla_policy.go:186`）看起来是个现成实现——`SELECT ... FROM sla_records r JOIN sla_targets t ... ORDER BY r.created_at DESC LIMIT 1`。它是真的，但读的是 686 迁移建的**裸名表族**，活链从不写。`ErrNoRows` 直接转成 `fmt.Errorf("sla record not found for ticket")`，接过去等于让平台创建的**每一个**工单在这条路由上 500。
4. 第 3 条同时解释了这条占位当初为什么是"连通性检查"形状：作者大概知道没法算，就留了个只证明库通了的壳，路由照常注册。

**删除后的 5 处涟漪**：`repository.go` 方法本体、`service.go` 的 `RepositoryInterface` 声明、`repository_interface.go` 的生成声明、`fake_repo_test.go` 的假实现、`ticket_sla_workflow_test.go` 的 `wfRepo` 假实现。保留的合法引用：`service_interface.go:72`（Handler 通过它调）、`service/sla_policy.go:100`（死链克隆服务）、`repository/sla_policy.go:186`（死链克隆仓库）、`handler/sla_policy_handler.go:132`（死链克隆 handler）、`repository/interfaces.go:74`、`testutil/mocks.go:183`。

**工具局限（如实记录）**：`repository_interface.go` 头是 `// Code generated by repository interface extractor. DO NOT EDIT.`，但 `tools/gen-interface/main.go:25` 在文件已存在时只打印 `SKIP_EXISTS` 直接跳过，无法原地重生成（且重生成会丢掉手写进去的 `TicketAssignment` 类型）。改一行，与其余方法保持一致。

**一个值得记的细节**：死链克隆的 `SLAPolicyHandler.RegisterRoutes` 注册的是**完全同一条路径** `GET /ticketing/sla/tickets/:ticketId/status`。`cmd/server/router.go:351` 的注释正是因此写的：`slaPolicyTicketH ... are NOT registered: ticketingH above already owns those paths`——Gin 对重复注册会 panic。这条克隆接的正是上面第 3 条那个会 500 的实现，所以"把注册打开"绝不是修复方向。克隆在两个模块各有一套：模块 A 的 `SLAPolicyHandler` 在 `wiring-core-domains.go:201` 被构造但从不注册，模块 B 的连 `RegisterRoutes` 都没有调用点。

### 67.3 两个被否掉的"第三个 stub"

顺手核了上轮留下的两条疑似项，都不是 stub，记录以免下轮重复排查：

- **`Repository.GetTransferStats`**：看着像空返回，实际是分层。`Service.GetTransferStats` 用 `CountTickets` 算出 `AvgTransfers` 再填进去。`ActiveTransfers` 结构上恒 0 是真，但那是 `ticket_transfer_history` 没有 status 列导致的——没有任何基础设施可以算，不是漏写。
- **`AnalyticsEnhanced.GetCategoryBreakdown`** 的 `categoryCounts["general"]++` 常量容器：整簇是死的。`NewAnalyticsEnhanced` 在 `cmd/` 里零调用（只有 handler 构造函数引用它，而 handler 本身也不注册）。要改成真实现需要 tenantID，签名里没有，改签名就得动整条链——本轮不动。

### 67.4 3 条新回归

**service 层 2 条**（`ticket_sla_workflow_test.go`，现 719 行）：

- `TestGetTicketSLAStatusReportsTheComputedWindow`——种一个 `high`、15 分钟前创建的工单，断言 `ticket_id`/`priority`/`status`、`target_response_time_ms == 3600*1000`、`target_resolution_time_ms == 8*3600*1000`、`response_due` 与 `resolution_due` 等于创建时间加 1h/8h 的 RFC3339，且两者仍为 true。**刻意选 high 而不是 critical**：critical 的响应窗口是 0 小时，`TargetResponseTimeMs` 是 0，和占位返回的零值无法区分。用 high 才能让"0"真的只能来自占位。
- `TestGetTicketSLAStatusPropagatesTheTrackingFailure`——断言 `errors.Is(err, want)` 且 `sla == nil`。修复前这条路由的错误形状是 200 + 空状态，和真数据库故障无法区分；现在必须把错误带上去。

**handler 层 1 条**（新文件 `handler_sla_policy_test.go`，58 行）：`TestHandler_GET_TicketingSLATicketStatusReturnsTheComputedWindow` 走 wire——构造 `fakeTicketingRepo`，调注册过的那个 handler 方法，解 `{"success":true,"data":{...}}` 断言 `data` 里的窗口与截止时间。service 测试验方法，这条验路由契约，两条独立。

**两处假实现补了缺口**（否则断言落不到值上）：

- `fakeTicketingRepo` 原本是空结构体，加 `tickets []models.Ticket` 和 `GetTicket` 查表（命中返回、未命中返回只带 ID 的工单）。现有 `&fakeTicketingRepo{}` 字面量全部仍能编译。
- `wfRepo` 加 `errGetSLATracking error`，并在 `GetSLATracking` 第一条语句注入——原来没有这个注入点，第二条测试写不出来。

### 67.5 变异矩阵：12 条全灭，含本轮新增 3 条

`/private/tmp/r66`，独立 `go.work`（`go 1.25.0`，`use (./repo ./orion-go-common)`），`rsync -a --delete` 同步副本，工作树从未被碰过。`mutate.py` 每组：断言补丁真的改动了字节（改不动就 `sys.exit(1)`，坏补丁不会被误报成弱测试）→ 跑测试要求出现 `--- FAIL:` → 按字节从变异前备份还原 → 再跑确认还原后仍绿。

| # | 变异 | 抓到的测试 |
|---|---|---|
| A | `CreateTicket` 去掉 `*3600`，毫秒写回秒 | `TestCreateTicketWritesTheSLAWindowInMilliseconds` |
| B | 去掉 `slaTargetsFor` 的 medium 回退 | `TestCreateTicketFallsBackToMediumForAnUnknownPriority` |
| C | `GetTicketSLA` 响应窗口写死 1h | `TestGetTicketSLAReportsTheCriticalWindow` + `...FallsBackToMedium...` |
| D | 忽略落库的 resolution 目标 | `TestGetTicketSLAKeepsTheStoredResolutionTarget` |
| E | `GetBacklogAnalysis` 恢复 resolved/closed 重置为 0 | `TestBacklogAnalysisCountsEveryResolvedAndClosedTicket` |
| F | 去掉 `assignmentColumns` 的 `COALESCE(reason,'')` | `TestGetAssignmentsByTicketUsesTheExplicitProjection` |
| G | `TransitionStatus` 丢弃 SLA 更新错误 | `TestTransitionStatusPropagatesTheSLAUpdateError` |
| H | `CloseTicket` 丢弃绑定错误 | `TestHandler_CloseTicketRejectsAMalformedBody` |
| I | `EscalateTicket` 丢弃优先级更新错误 | `TestEscalateTicketPropagatesThePriorityUpdateError` |
| **J** | 恢复占位 `return &models.TicketSLAStatus{TicketID: ticketID}, nil` | **`TestGetTicketSLAStatusReportsTheComputedWindow`** |
| **K** | `sla, _ := s.GetTicketSLA(...); return sla, nil` 丢弃错误 | **`TestGetTicketSLAStatusPropagatesTheTrackingFailure`** |
| **J2** | 同 J，改跑 handler 包 | **`TestHandler_GET_TicketingSLATicketStatusReturnsTheComputedWindow`** |

`ALL MUTATIONS DISCRIMINATING: True`，12/12 行 `patch-fails=True restored-green=True`。J 和 K 是本轮针对这个缺陷专门加的：J 证明不是"改了个返回值但测试只查 200"，K 证明错误传播真的被断言了。J2 单独存在是因为同一个回归在 service 包和 handler 包各有独立守护，任何一条都能单独发现它。

### 67.6 写测试和 harness 时自己撞出的四个缺陷

这一轮四次撞到"没用或意外的信号"，四次里四次都是我的读法或断言太弱，不是代码：

1. **我自己的断言错了，产出一个真失败**：`TestGetTicketSLAStatusReportsTheComputedWindow` 里我写 `ResponseDue == created.Format(time.RFC3339)`——那只有 critical（0 小时窗口）才成立，而 fixture 是 high（1 小时）。实际拿到 `15:53:03Z`、期望 `14:53:03Z`。修的是断言，不是代码。这条也解释了为什么 fixture 必须避开 critical：critical 的 0 小时窗口会让"占位"和"真值"在断言里无法区分。
2. **我写了一条空断言的测试**：第一版 `TestGetTicketSLAStatusPropagatesTheTrackingFailure` 有 `want` 声明、`_ = want`、`_ = repo.overrideTracking` 三行死代码，注释说"断言错误被传播"而传播从未发生——因为 `wfRepo` 根本没有 `GetSLATracking` 的错误注入点。修法是给结构体加字段、在方法第一条语句注入，再重写测试用 `errors.Is` 断言。
3. **一条 sed 拼接错误让 Edit 失败**：我拿 `sed -n '40,60p' service.go` 的输出当锚点，以为 `RepositoryInterface` 第 48 行的邻居是 `DeleteSLAPolicy`/`GetTransferHistory`；实际是 `GetTicket`(47)/`GetTransferHistory`(49)。Edit 报 String to replace not found，重新 Read 后用正确锚点。
4. **一条 Edit 静默删了个寂寞还引入了空行**：`repository_interface.go` 那把编辑把 `GetSLABreaches`+`GetSLACompliance` 替换成同样两行加一个尾部空行——`GetTicketSLAStatus` 一行没删，第 51 行多了个空行。是前面那次 Edit 失败后重新 Read 才发现的；第二把编辑跨 `DeleteSLAPolicy` → `CreateAutomationRule` 把目标行和那个空行一起清掉。

另有两条环境坑（§66 已记过，本轮又各撞一次）：macOS 把 `/tmp` 规范化成 `/private/tmp`，`GOWORK=/private/tmp/...` 而 cwd 写成 `/tmp/r66/repo` 会让 workspace 模块根与包解析落在不同前缀，报 `contained in a module that is not one of the workspace modules`；以及 resync 时 `rm -rf internal/ticketing && cp -a .../internal internal/ticketing` 把整棵 `internal` 拷进了 `internal/ticketing`，改用 `rsync -a --delete` 两边同步、`diff -r` 确认。

### 67.7 验证

- `gofmt -l internal/ticketing/` 输出 0 行。
- 8 个改动文件字节扫描：U+201D = 0，控制字符为空。
- `go build ./internal/ticketing/...`、`go vet ./internal/ticketing/...`、`go test -count=1 ./internal/ticketing/...` 全绿。
- 变异 harness `ALL MUTATIONS DISCRIMINATING: True`（12/12）。
- 工作树 `git status` 只多出本轮预期的 6 个改动 + 1 个新文件；并行会话的 5 个 `internal/cmdb/*` 文件未动。

`cmd/server` 包本身当前无法构建：并行会话在 `internal/cmdb` 有未提交改动，报 `*Service does not implement ServiceInterface (wrong type for method Create)`。与本轮无关，未动。

### 67.8 提交

- 代码：`internal/ticketing/{handler/handler_sla_policy_test.go, handler/fake_repo_test.go, repository/repository.go, repository/repository_interface.go, service/service.go, service/ticket_sla_policy.go, service/ticket_sla_workflow_test.go}`
...
**本轮直接消掉一条**：§66.10 未验证列表里的 `GetTicketSLAStatus`（`repository.go`）自述 Placeholder——本轮删除。

**未动，结转：**

- `ticket_sla_tracking` 无 `tenant_id`，`GetSLATracking` / `UpdateSLATracking` 仍无法租户隔离（`ticket_assignments` 有）。
- `CreateTicket` 部分写入（工单行与历史行已提交，SLA upsert 才失败；需事务基础设施）。
- `GetTrendReport.Escalated` 与 `validTransitions` 缺 `"escalated"` → `GetExecutiveDashboard.Escalations` 结构上恒 0。
- 契约不一致：前端 `reason` / `comment` / `resolution` 三个 wire 字段在 3 条路由上对不上；`ResolveTicket` / `CloseTicket` 的 `from_state` 是空串（这两个方法从不调 `GetTicket`）。
- 死链集群（模块 A 与模块 B 各一套）：4 个克隆仓库 + `service/sla_policy.go` + 无引用的 `repository_interface.go`，查 686 的裸名表族，28 条无租户过滤的 `SELECT *`；`interfaces.go` 与 `testutil/mocks.go` 是活引用必须一起删。模块 A 的 `SLAPolicyHandler` 在 `wiring-core-domains.go:201` 被构造但 `router.go:351` 注明不注册；模块 B 的 `RegisterRoutes` 无调用点。
- `SLARepositoryInterface.policyID int`（`interfaces.go:36,71,72,73,76`）与 `MockSLARepository` 仍是 int。
- 模块 A `transfer.go` 的 `GetStats` 丢弃 `avgHold`；模块 B `TransferService.GetTransferStats`（`transfer_service.go:173`）把 `avg_hold_duration_ms` 标进 `AvgTransfers`——语义错位，且 `TransferService` 在 `wiring-core-domains.go:181` 是真的被构造的。
