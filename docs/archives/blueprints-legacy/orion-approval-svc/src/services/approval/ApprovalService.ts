...
export class ApprovalService {
  ...
  async approve(approvalId: string, userId: string): Promise<ApprovalRequest> {
    ...
    const matchingStep = steps.find((s: ApprovalStepEntity) => s.approverId === userId);
  ...
  async getApproval(id: string): Promise<ApprovalRequest | undefined> {
    ...
    const approverIds = steps.map((s: ApprovalStepEntity) => s.approverId ?? '');
  ...
  private entityToRequest(
    ...
    const approvals = steps?.filter((s: ApprovalStepEntity) => s.status === 'approved').map((s: ApprovalStepEntity) => s.approverId ?? '') ?? [];
    const rejections = steps?.filter((s: ApprovalStepEntity) => s.status === 'rejected').map((s: ApprovalStepEntity) => s.approverId ?? '') ?? [];
