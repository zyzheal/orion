...
import (
	...
	"orion/monitor-svc-go/internal/alert-silence/models"
	"orion/monitor-svc-go/internal/alert-silence/repository"
	"go.uber.org/zap"
)
...
func (s *AlertSilenceService) ExtendSilence(ctx context.Context, tenantID, id uuid.UUID, extendBy int) (*models.Silence, error) {
	...
	result, err := s.repo.db.Pool().Exec(ctx, query, extendBy, newExpiresAt, now, id, tenantID)
	if err != nil {
