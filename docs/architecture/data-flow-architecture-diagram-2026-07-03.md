# 数据流架构图（2026-07-03 自动生成）

> **生成方式**: 基于 `docs/architecture/当前系统架构.md` + `docs/architecture/架构设计详解.md` + 实际代码结构自动生成
> **对应任务**: Phase 2.33
...
| # | 路径 | 协议 | 数据量 | 实时性 |
|---|------|------|--------|--------|
| 1 | 前端 → API Gateway → Platform Service | HTTP/REST | 小 (KB) | 实时 |
| 2 | Platform Service → PostgreSQL | TCP/pg | 中 (KB-MB) | 实时 |
| 3 | Platform Service → Redis | TCP | 小 (KB) | 实时 |
| 4 | EventBus → 订阅者 | NATS/内存 | 小 (KB) | 近实时 |
| 5 | SSE → 前端 | HTTP/SSE | 中 (日志流) | 实时 |
| 6 | Plugin → Pipeline Engine | stdio/gRPC | 大 (MB-GB) | 异步 |
