...
> **⚠️⚠️ 深度核实（2026-09-08 Phase 300 差距扩展审计）—— 行数与能力深度实测**
>
> 全库 `find` + `grep` + `wc -l` 实测后发现，虽然 5 项模块**代码全部存在**，但**行数与能力深度**仍有重大差距：
>
> | 任务 | 文档声称 | 实测 | 关键差距 |
> |------|---------|------|---------|
> | T-AUDIT | 2829 行 | **1595 行 (-43%)** | ISO27001 endpoint 缺失；SOC2 已有 |
> | T-QUOTA | 1287 行 | **823 行 (-36%)** | **P0 BUG：`wiretenantquota` 未被 wiring.go 调用 → tqH 永远 nil → API 不可达** |
> | T-CONFIG-LEVEL | 408K | **1431 行** | **只有 TenantID，无 Level 字段（不支持三层覆盖）** |
> | T-SPI | 未标注 | **1827 行** | 只有 Category 分类，**无内置扩展点枚举** |
> | T-POSTMORTEM | 155+91+6 | **154+91+6** | ✅ 完全匹配 |
>
> **修正后结论**：TOP5 视角差距**不是"+24d 新增"，而是"6d 差距扩展"**。总工时 315.5d → **292.5d**（-23d）。

#### 4. 升级后的 Wave 总览

> ⚠️ **2026-09-08 Phase 300 最终修正**：原"新增 5 项 24d"经核实为"差距扩展 6d"。总工时 315.5d → **292.5d**（-23d 修正）。原"新增 5 项"表已作废，实际方案见 `docs/flagship-review-v3.6-delta-2026-09-08.md`。

| Wave | 原任务数 | 原人天 | 升级后任务数 | 升级后人天 | 变更 |
|------|---------|--------|-------------|-----------|------|
| Wave 1 | 8 | 11d | 10 | 18d | +T-19/T-18 前置（**T-AUDIT 已实现，删除新增**） |
| Wave 2 | 15 | 36d | 15 | 36d | **T-QUOTA/T-CONFIG-LEVEL/T-POSTMORTEM 已实现，删除新增** |
| Wave 3 | 13 | 26d | 13 | 26d | **T-SPI 已实现，删除新增** |
