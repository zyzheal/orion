...
import (
	...
	"orion/feature-flag-svc-go/internal/config"
	"orion/feature-flag-svc-go/internal/handler"
	"orion/feature-flag-svc-go/internal/repository"
	"orion/feature-flag-svc-go/internal/service"
	nats_subscriber "orion/feature-flag-svc-go/pkg/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-feature-flag-svc"))
	defer logger.Sync()
	...
	r.GET("/healthz", middleware.HealthCheck("orion-feature-flag-svc"))
	...
	logger.Info("feature-flag-svc listening", zap.String("addr", addr))
	...
	logger.Info("shutting down feature-flag-svc...")
	if natsSub != nil {
