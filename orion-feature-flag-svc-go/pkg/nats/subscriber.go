...
// NATSSubscriber subscribes to FeatureFlagEvent via NATS JetStream.
...
// Start subscribes to FeatureFlagEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	_, err := s.js.CreateOrUpdateConsumer(ctx, s.stream, jetstream.ConsumerConfig{
		Name:           "feature-flag-svc-consumer",
	...
	cons, err := s.js.Consumer(ctx, s.stream, "feature-flag-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received featureflag event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleFeatureFlagEvent(ctx, &event); err != nil {
		s.log.Error("handle featureflag event", zap.Error(err))
...
func (s *NATSSubscriber) handleFeatureFlagEvent(ctx context.Context, event *EventBusEvent) error {
