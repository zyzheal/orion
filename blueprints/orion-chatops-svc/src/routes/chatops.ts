...
import { EventBusService as ConcreteEventBusService } from '../services/event-bus-service';
import {
...
interface ChatOpsRoutesOptions {
  eventBus?: ConcreteEventBusService;
...
export default async function chatopsRoutes(
  ...
  const deployService = options.deployService || (new DeployService(new DeployRepository(db as any)));
  const monitoringService = options.monitoringService || (new MonitoringService(new MonitoringRepository(db as any)));
  const diagnosticService = options.diagnosticService || (new DiagnosticService(new DiagnosticRepository(db as any)));
  const selfHealingService = options.selfHealingService || (new SelfHealingService(new SelfHealingRepository(db as any)));
  ...
  const dataProvider = new RealDataProvider(db as any);
  const recommendationService = new RecommendationService(dataProvider);

  // Notification Preferences
  const notifPrefService = new NotificationPreferenceService(db as any);

  // DND Settings
  const dndService = new DNDService(db as any);

  // Alert States (with SE-9 ownership validation)
  const alertStateService = new AlertStateService(db as any);

  // Platform Config
  const platformConfigService = new PlatformConfigService(db as any);
