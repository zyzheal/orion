...
export class ExecutionService {
  ...
  async getAuditLogs(filter: ChatOpsAuditLogFilter = {}): Promise<{ logs: ChatOpsAuditLog[]; total: number }> {
    ...
    if (filter.traceId) {
      items = items.filter((l: ChatOpsAuditLogEntity) => l.traceId === filter.traceId);
    }
    if (filter.actor) {
      items = items.filter((l: ChatOpsAuditLogEntity) =>
        (l.actor as Record<string, string>).userId?.includes(filter.actor!) ||
        JSON.stringify(l.actor).includes(filter.actor!)
      );
    }
    if (filter.result) {
      items = items.filter((l: ChatOpsAuditLogEntity) => l.result === filter.result);
    }
    if (filter.startDate) {
      items = items.filter((l: ChatOpsAuditLogEntity) => l.timestamp >= filter.startDate!);
    }
    if (filter.endDate) {
      items = items.filter((l: ChatOpsAuditLogEntity) => l.timestamp <= filter.endDate!);
    }
    ...
    return { logs: paginated.map((e: ChatOpsAuditLogEntity) => this.entityToAuditLog(e)), total };
  }
