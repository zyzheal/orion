...
// Vendor 返回厂商
func (c *PostgreSQLCollector) Vendor() model.VendorType { return model.VendorPostgreSQL }
...
//   8. 组装 CIRaw
func (c *PostgreSQLCollector) Collect(ctx context.Context, config map[string]any) ([]model.CIRaw, error) {
	host := config["host"].(string)
	port := 5432
	if p, ok := config["port"]; ok {
		port = p.(int)
