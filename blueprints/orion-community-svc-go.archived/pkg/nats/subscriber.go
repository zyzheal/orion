...
// NATSSubscriber subscribes to CommunityEvent via NATS JetStream.
...
// Start subscribes to CommunityEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "community-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received community event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleCommunityEvent(ctx, &event); err != nil {
		s.log.Error("handle community event", zap.Error(err))
...
func (s *NATSSubscriber) handleCommunityEvent(ctx context.Context, event *EventBusEvent) error {
