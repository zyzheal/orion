...
// NATSSubscriber subscribes to ChatOpsEvent via NATS JetStream.
...
// Start subscribes to ChatOpsEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "chatops-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received chatops event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleChatOpsEvent(ctx, &event); err != nil {
		s.log.Error("handle chatops event", zap.Error(err))
...
func (s *NATSSubscriber) handleChatOpsEvent(ctx context.Context, event *EventBusEvent) error {
