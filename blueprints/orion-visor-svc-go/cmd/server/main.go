...
import (
	...
	"orion/visor-svc-go/internal/config"
	"orion/visor-svc-go/internal/handler"
	"orion/visor-svc-go/internal/repository"
	"orion/visor-svc-go/internal/service"
	"orion/visor-svc-go/pkg/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-visor-svc"))
	defer logger.Sync()
	...
	r.GET("/healthz", middleware.HealthCheck("orion-visor-svc"))

	addr := fmt.Sprintf(":%d", cfg.Port)
	logger.Info("visor-svc listening", zap.String("addr", addr))
	...
	logger.Info("shutting down visor-svc...")
	if natssub != nil {
		natssub.Close()
	}
