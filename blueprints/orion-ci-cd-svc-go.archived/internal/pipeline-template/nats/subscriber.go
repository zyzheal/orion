...
// NATSSubscriber subscribes to PipelineTemplateEvent via NATS JetStream.
...
// Start subscribes to PipelineTemplateEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	subject := fmt.Sprintf("%s.PipelineTemplateEvent.>", s.stream)

	_, err := s.js.CreateOrUpdateConsumer(ctx, s.stream, jetstream.ConsumerConfig{
		Name:           "pipeline-template-svc-consumer",
	...
	cons, err := s.js.Consumer(ctx, s.stream, "pipeline-template-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received PipelineTemplateEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handlePipelineTemplateEvent(ctx, &event); err != nil {
		s.log.Error("handle pipeline template event", zap.Error(err))
...
func (s *NATSSubscriber) handlePipelineTemplateEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的 PipelineTemplate 操作
	// 例如：TemplateCreated → 同步模板到模板市场
	//       TemplateUsed → 记录模板使用统计
	return nil
}
