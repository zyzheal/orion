...
// Vendor 返回厂商
func (c *OracleCollector) Vendor() model.VendorType { return model.VendorOracle }
...
//   7. 组装 CIRaw
func (c *OracleCollector) Collect(ctx context.Context, config map[string]any) ([]model.CIRaw, error) {
	host := config["host"].(string)
	port := 1521
	if p, ok := config["port"]; ok {
		port = p.(int)
