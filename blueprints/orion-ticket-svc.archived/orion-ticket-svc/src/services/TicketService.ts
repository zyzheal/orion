...
export class TicketService extends EventEmitter {
  ...
  private async attemptAutoDispatch(ticketId: string): Promise<void> {
    ...
    if (result) {
      const r = result as any;
      console.log(`[TicketService] Auto-dispatched ticket ${ticketId} to ${r.assignee} (score: ${r.score})`);
    }
  }
  ...
  async manualDispatch(
    ...
    // Record dispatch - using dispatch engine's method instead
    await this.dispatchEngine.dispatchTicket(ticketId, engineerId);
    await this.loadBalancer.recordAssignment({
      ticketId,
      engineerId,
      category: ticket.category,
  ...
  addDispatchRule(rule: DispatchRule): void {
    (this.dispatchEngine as any).addRule(rule);
  }
  ...
  getDispatchRules(): DispatchRule[] {
    return (this.dispatchEngine as any).getRules();
  }
  ...
  removeDispatchRule(ruleId: string): boolean {
    return (this.dispatchEngine as any).removeRule(ruleId);
  }
  ...
  async findBestEngineerForTicket(ticketId: string) {
    const ticket = await this.workflow.getTicket(ticketId);
    if (!ticket) return null;
    return (this.dispatchEngine as any).findBestEngineer(ticket);
  }
  ...
  async calculateDispatchScore(ticketId: string, engineerId: string) {
    ...
    const engineer = await this.dispatchEngine.getEngineer(engineerId);
    if (!engineer) return null;
    return (this.dispatchEngine as any).calculateDispatchScore(ticket, engineer);
  }
  ...
  getDispatchWeights(): DispatchWeights {
    return (this.dispatchEngine as any).getWeights();
  }
  ...
  updateDispatchWeights(weights: Partial<DispatchWeights>): void {
    (this.dispatchEngine as any).updateWeights(weights);
  }
