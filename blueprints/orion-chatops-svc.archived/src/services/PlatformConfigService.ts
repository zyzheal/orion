...
export class PlatformConfigService {
  ...
  constructor(private pool: DatabasePool) {
    this.repo = new ChatOpsPlatformConfigRepository(this.pool as any);
  }
  ...
  async batchUpdate(userId: string, configs: PlatformConfig[]): Promise<PlatformConfig[]> {
    // R-1: 使用 sequential upserts (简化实现，避免 Pool.transaction 类型问题)
    ...
    for (const config of configs) {
      ...
      results.push(this.entityToConfig(entity));
    }
    return results;
  }
