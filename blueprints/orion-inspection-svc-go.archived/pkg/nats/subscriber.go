...
// NATSSubscriber subscribes to InspectionEvent via NATS JetStream.
...
// Start subscribes to InspectionEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "inspection-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received inspection event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleInspectionEvent(ctx, &event); err != nil {
		s.log.Error("handle inspection event", zap.Error(err))
...
func (s *NATSSubscriber) handleInspectionEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理巡检相关事件
	return nil
}
