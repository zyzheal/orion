...
// NATSSubscriber subscribes to SkillConfigEvent via NATS JetStream.
...
// Start subscribes to SkillConfigEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	_, err := s.js.CreateOrUpdateConsumer(ctx, s.stream, jetstream.ConsumerConfig{
		Name:           "skill-config-svc-consumer",
	...
	cons, err := s.js.Consumer(ctx, s.stream, "skill-config-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received skill-config event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleSkillConfigEvent(ctx, &event); err != nil {
		s.log.Error("handle skill-config event", zap.Error(err))
...
func (s *NATSSubscriber) handleSkillConfigEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理技能配置相关事件
	return nil
}
