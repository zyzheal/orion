...
export class ApprovalRepository extends BaseRepository<ApprovalEntity> {
  ...
  /**
   * Conditional status update with optimistic locking.
   * Only succeeds if the current status matches expectedStatus.
   * Returns null if the status was already changed (prevents race conditions).
   */
  async updateStatusWithCondition(
    ...
    const result = await this.db.query(
      `UPDATE approvals SET status = $1, completed_at = $2,
       current_step = CASE WHEN $1 = 'approved' THEN total_steps ELSE current_step END
       WHERE id = $3 AND status = $4`,
      [newStatus, newStatus === 'approved' || newStatus === 'rejected' ? new Date() : null, id, expectedStatus],
    );
  ...
  /**
   * Atomic step update with optimistic locking.
   * Only succeeds if the step is currently in one of the expected statuses.
   * Returns null if the step was already acted upon (prevents race conditions).
   */
  async updateStepStatusWithLock(
    stepId: string,
    newStatus: string,
    expectedStatuses: string[],
    comment?: string,
    actedAt?: Date,
  ): Promise<ApprovalStepEntity | null> {
    const placeholders = expectedStatuses.map((_, i) => `$${i + 4}`).join(', ');
    const result = await this.db.query(
      `UPDATE approval_steps SET status = $1, comment = $2, acted_at = $3
       WHERE id = $4 AND status IN (${placeholders}) RETURNING *`,
      [newStatus, comment ?? null, actedAt ?? new Date(), stepId, ...expectedStatuses],
    );
    if (result.rows.length === 0) return null;
    return this.mapStepRowToEntity(result.rows[0]);
  }
