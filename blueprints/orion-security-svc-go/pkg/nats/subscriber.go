...
// NATSSubscriber subscribes to SecurityEvent via NATS JetStream.
...
// Start subscribes to SecurityEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "security-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received security event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleSecurityEvent(ctx, &event); err != nil {
		s.log.Error("handle security event", zap.Error(err))
...
func (s *NATSSubscriber) handleSecurityEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理安全相关事件
	return nil
}
