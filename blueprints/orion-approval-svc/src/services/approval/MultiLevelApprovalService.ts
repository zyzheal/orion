...
export class MultiLevelApprovalService {
  ...
  async review(
    ...
    if (entity.status !== 'pending') throw new Error(`Approval request is not pending (current status: ${entity.status})`);

    const steps = await this.repository.findStepsByApproval(requestId);
    const matchingStep = steps.find((s: ApprovalStepEntity) => s.approverId === reviewerId && (s.status === 'pending' || s.status === 'waiting'));
  ...
  async getApprovalChain(requestId: string): Promise<ApprovalChainInfo> {
    ...
    const stepDetails: ApprovalStepDetail[] = steps.map((s: ApprovalStepEntity, _idx: number) => {
  ...
  async getPendingApprovals(
    ...
    for (const entity of entities) {
      ...
      const userSteps = steps.filter((s: ApprovalStepEntity) => s.approverId === userId && (s.status === 'pending' || s.status === 'waiting'));
