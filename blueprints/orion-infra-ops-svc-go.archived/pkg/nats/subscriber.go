...
// NATSSubscriber subscribes to CapacityEvent via NATS JetStream.
...
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "capacity-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received CapacityEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleCapacityEvent(ctx, &event); err != nil {
		s.log.Error("handle capacity event", zap.Error(err))
...
func (s *NATSSubscriber) handleCapacityEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的 Capacity 操作
	// 例如：ResourceUsageThresholdExceeded → 更新容量预测模型
	//       ScaleOutTriggered → 记录扩容事件
	return nil
}
