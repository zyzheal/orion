...
import (
	...
	"orion/cron-svc-go/internal/config"
	"orion/cron-svc-go/internal/handler"
	"orion/cron-svc-go/internal/repository"
	"orion/cron-svc-go/internal/service"
	nats_subscriber "orion/cron-svc-go/pkg/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-cron-svc"))
	defer logger.Sync()
	...
	r.GET("/healthz", middleware.HealthCheck("orion-cron-svc"))
	...
	logger.Info("cron-svc listening", zap.String("addr", addr))
	...
	logger.Info("shutting down cron-svc...")
	if natsSub != nil {
