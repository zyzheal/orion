...
// NATSSubscriber subscribes to CronEvent via NATS JetStream.
...
// Start subscribes to CronEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "cron-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received cron event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleCronEvent(ctx, &event); err != nil {
		s.log.Error("handle cron event", zap.Error(err))
...
func (s *NATSSubscriber) handleCronEvent(ctx context.Context, event *EventBusEvent) error {
