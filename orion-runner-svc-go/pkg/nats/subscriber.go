...
// NATSSubscriber subscribes to RunnerEvent via NATS JetStream.
...
// Start subscribes to RunnerEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "runner-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received runner event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleRunnerEvent(ctx, &event); err != nil {
		s.log.Error("handle runner event", zap.Error(err))
...
func (s *NATSSubscriber) handleRunnerEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理 runner 相关事件
	return nil
}
