...
// NATSSubscriber subscribes to ArtifactEvent via NATS JetStream.
...
// Start subscribes to ArtifactEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "artifact-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received artifact event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleArtifactEvent(ctx, &event); err != nil {
		s.log.Error("handle artifact event", zap.Error(err))
...
func (s *NATSSubscriber) handleArtifactEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理制品相关事件
	// 例如：BuildCompleted -> 关联制品版本
	//       DeploymentCompleted -> 标记制品已部署
	return nil
}
