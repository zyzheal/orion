...
// NATSSubscriber subscribes to RiskEvent via NATS JetStream.
...
// Start subscribes to RiskEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "risk-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received risk event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleRiskEvent(ctx, &event); err != nil {
		s.log.Error("handle risk event", zap.Error(err))
...
func (s *NATSSubscriber) handleRiskEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理风险相关事件
	return nil
}
