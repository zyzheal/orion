...
// NATSSubscriber subscribes to AuthEvent via NATS JetStream.
...
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "auth-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received AuthEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleAuthEvent(ctx, &event); err != nil {
		s.log.Error("handle auth event", zap.Error(err))
...
func (s *NATSSubscriber) handleAuthEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的 Auth 操作
	// 例如：UserLoginSucceeded → 记录登录审计日志
	//       TokenRevoked → 清理 Redis 缓存
	return nil
}
