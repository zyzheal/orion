...
// NATSSubscriber subscribes to GovernanceEvent via NATS JetStream.
...
// Start subscribes to GovernanceEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "governance-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received governance event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleGovernanceEvent(ctx, &event); err != nil {
		s.log.Error("handle governance event", zap.Error(err))
...
func (s *NATSSubscriber) handleGovernanceEvent(ctx context.Context, event *EventBusEvent) error {
