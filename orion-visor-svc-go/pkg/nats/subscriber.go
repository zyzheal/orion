...
// NATSSubscriber subscribes to VisorEvent via NATS JetStream.
...
// Start subscribes to VisorEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "visor-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received visor event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleVisorEvent(ctx, &event); err != nil {
		s.log.Error("handle visor event", zap.Error(err))
...
func (s *NATSSubscriber) handleVisorEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理可视化相关事件
	return nil
}
