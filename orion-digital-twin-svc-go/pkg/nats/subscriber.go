...
// NATSSubscriber subscribes to DigitalTwinEvent via NATS JetStream.
...
// Start subscribes to DigitalTwinEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	_, err := s.js.CreateOrUpdateConsumer(ctx, s.stream, jetstream.ConsumerConfig{
		Name:           "digital-twin-svc-consumer",
	...
	cons, err := s.js.Consumer(ctx, s.stream, "digital-twin-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received digitaltwin event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleDigitalTwinEvent(ctx, &event); err != nil {
		s.log.Error("handle digitaltwin event", zap.Error(err))
...
func (s *NATSSubscriber) handleDigitalTwinEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理相关事件
	return nil
}
