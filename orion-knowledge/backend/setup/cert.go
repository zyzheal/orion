...
const (
	keyFile  = "/app/etc/nginx/ssl/orion-knowledge.key" // Key file path
	certFile = "/app/etc/nginx/ssl/orion-knowledge.crt" // Certificate file path
)
...
func createSelfSignedCerts() error {
	...
	certFile, err := os.Create("/app/etc/nginx/ssl/orion-knowledge.crt")
	if err != nil {
	...
	keyFile, err := os.Create("/app/etc/nginx/ssl/orion-knowledge.key")
	if err != nil {
