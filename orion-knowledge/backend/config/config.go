...
func NewConfig() (*Config, error) {
	...
	defaultConfig := &Config{
		...
		Redis: RedisConfig{
			Addr:     "orion-knowledge-redis:6379",
			Password: "",
		},
		Auth: AuthConfig{
			Type: "jwt",
...
func overrideWithEnv(c *Config) {
	if env := os.Getenv("POSTGRES_PASSWORD"); env != "" {
		c.PG.DSN = fmt.Sprintf("host=orion-knowledge-pg user=knowledge password=%s dbname=orion_knowledge port=5432 sslmode=disable TimeZone=Asia/Shanghai", env)
	}
