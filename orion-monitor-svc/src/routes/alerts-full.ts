...
export default async function alertRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/topology', async (_request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get('/deduplication/stats', async (_request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get('/suppression/stats', async (_request: FastifyRequest, reply: FastifyReply) => {
    const stats = suppressionService.getStats();
    return reply.send({ stats });
  });

  // GET /alert/suppression/maintenance-windows - 获取维护窗口
  app.get('/suppression/maintenance-windows', async (_request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get('/suppression/known-issues', async (_request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get('/suppression/alerts', async (_request: FastifyRequest, reply: FastifyReply) => {
    const groups = deduplication.getActiveGroups();
