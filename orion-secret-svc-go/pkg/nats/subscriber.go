...
// NATSSubscriber subscribes to SecretEvent via NATS JetStream.
...
// Start subscribes to SecretEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "secret-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received secret event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleSecretEvent(ctx, &event); err != nil {
		s.log.Error("handle secret event", zap.Error(err))
...
func (s *NATSSubscriber) handleSecretEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理密钥相关事件
	// 例如：DeploymentCompleted -> 自动轮转密钥
	//       SecurityAlert -> 标记密钥风险
	return nil
}
