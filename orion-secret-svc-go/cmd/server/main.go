...
import (
	...
	"orion/secret-svc-go/internal/config"
	"orion/secret-svc-go/internal/handler"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-secret-svc"))
	defer logger.Sync()
	...
	r.GET("/healthz", middleware.HealthCheck("orion-secret-svc"))

	addr := fmt.Sprintf(":%d", cfg.Port)
	logger.Info("secret-svc listening", zap.String("addr", addr))
