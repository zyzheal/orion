...
import (
	...
	"orion/skill-config-svc-go/internal/config"
	"orion/skill-config-svc-go/internal/handler"
	...
	nats_subscriber "orion/skill-config-svc-go/pkg/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-skill-config-svc"))
	defer logger.Sync()
	...
	go func() {
		logger.Info("skill-config-svc listening", zap.String("addr", addr))
	...
	<-quit
	logger.Info("shutting down skill-config-svc...")
