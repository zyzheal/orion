...
// NATSSubscriber subscribes to ReportDesignerEvent via NATS JetStream.
...
// Start subscribes to ReportDesignerEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	subject := fmt.Sprintf("%s.ReportDesignerEvent.>", s.stream)

	_, err := s.js.CreateOrUpdateConsumer(ctx, s.stream, jetstream.ConsumerConfig{
		Name:           "report-designer-svc-consumer",
	...
	cons, err := s.js.Consumer(ctx, s.stream, "report-designer-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received ReportDesignerEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleReportDesignerEvent(ctx, &event); err != nil {
		s.log.Error("handle report designer event", zap.Error(err))
...
func (s *NATSSubscriber) handleReportDesignerEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的 ReportDesigner 操作
	// 例如：ReportPublished → 刷新报表缓存
	//       ReportScheduleTriggered → 触发报表生成
	return nil
}
