...
// NATSSubscriber subscribes to WorkflowEvent via NATS JetStream.
...
// Start subscribes to WorkflowEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "workflow-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received workflow event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleWorkflowEvent(ctx, &event); err != nil {
		s.log.Error("handle workflow event", zap.Error(err))
...
func (s *NATSSubscriber) handleWorkflowEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理工作流相关事件
	return nil
}
