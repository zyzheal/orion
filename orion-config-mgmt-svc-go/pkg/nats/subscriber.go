...
// NATSSubscriber subscribes to ConfigMgmtEvent via NATS JetStream.
...
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "config-mgmt-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received ConfigMgmtEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleConfigMgmtEvent(ctx, &event); err != nil {
		s.log.Error("handle config mgmt event", zap.Error(err))
...
func (s *NATSSubscriber) handleConfigMgmtEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的 ConfigMgmt 操作
	// 例如：ConfigDriftDetected → 记录漂移事件
	//       FeatureFlagUpdated → 同步特性标志
	return nil
}
