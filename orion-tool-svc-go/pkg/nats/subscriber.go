...
// NATSSubscriber subscribes to ToolEvent via NATS JetStream.
...
// Start subscribes to ToolEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "tool-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received tool event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleToolEvent(ctx, &event); err != nil {
		s.log.Error("handle tool event", zap.Error(err))
...
func (s *NATSSubscriber) handleToolEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理工具相关事件
	return nil
}
