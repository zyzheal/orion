...
export class ConfigDriftRepository extends BaseRepository<ConfigDriftEntity> {
  ...
  protected mapRowToEntity(row: RawDriftRow): ConfigDriftEntity {
    return {
      ...
      expectedConfig: this._parseJson(row.expected_config) || {},
      actualConfig: this._parseJson(row.actual_config) || {},
      driftItems: this._parseJson(row.drift_items) || [],
      ...
      remediationLog: this._parseJson(row.remediation_log) || [],
