...
export class NotificationPolicyService {
  ...
  async evaluatePolicies(event: Record<string, unknown>): Promise<NotificationPolicyEntity[]> {
    ...
    logger.debug({ tenantId, totalPolicies: policies.length, matchedCount: matched.length }, 'Policy evaluation completed');
    return matched;
  }
  ...
  private evaluateCondition(fieldValue: unknown, operator: PolicyCondition['operator'], conditionValue: unknown): boolean {
    switch (operator) {
      case 'eq':
