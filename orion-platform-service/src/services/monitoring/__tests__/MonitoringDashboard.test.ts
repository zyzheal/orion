...
describe('MonitoringDashboard', () => {
  ...
  describe('widgetConfig', () => {
    it('should add a widget configuration', () => {
      dashboard.addWidgetConfig({
        title: 'CPU Usage',
        metrics: ['system.cpu.usage'],
        timeWindow: '1h',
      });

      const widgets = dashboard.getWidgetConfigs();
      expect(widgets.length).toBe(1);
      expect(widgets[0].title).toBe('CPU Usage');
    });

    it('should allow removing a widget configuration', () => {
      dashboard.addWidgetConfig({
        title: 'CPU',
        metrics: ['system.cpu.usage'],
        timeWindow: '1h',
      ...
      dashboard.removeWidgetConfig(0);
    ...
    it('should clear all widget configurations', () => {
      dashboard.addWidgetConfig({
        title: 'CPU',
        metrics: ['system.cpu.usage'],
        timeWindow: '1h',
      ...
      dashboard.clearWidgetConfigs();

      expect(dashboard.getWidgetConfigs().length).toBe(0);
    });
  });
  ...
  describe('getDashboardData', () => {
    ...
    it('should generate dashboard data with widgets', () => {
    ...
    it('should include active alert counts', () => {
      const data = dashboard.getDashboardData({
        critical: 2,
        warning: 5,
        info: 1,
    ...
    it('should calculate current value for widgets', () => {
      const data = dashboard.getDashboardData();
  ...
  describe('healthScore calculation', () => {
    it('should be 100 with no alerts and normal metrics', () => {
      collector.recordMetric('system.cpu.usage', 30);
      collector.recordMetric('system.memory.usage', 40);

      const data = dashboard.getDashboardData({ critical: 0, warning: 0, info: 0 });
      expect(data.healthScore).toBeGreaterThan(70);
    });

    it('should decrease with critical alerts', () => {
      collector.recordMetric('system.cpu.usage', 30);
      collector.recordMetric('system.memory.usage', 40);

      const highAlert = dashboard.getDashboardData({ critical: 5, warning: 0, info: 0 });
      const lowAlert = dashboard.getDashboardData({ critical: 0, warning: 0, info: 0 });

      expect(highAlert.healthScore).toBeLessThan(lowAlert.healthScore);
    });

    it('should decrease with high CPU', () => {
    ...
    it('should decrease with high memory', () => {
      collector.recordMetric('system.cpu.usage', 30);
    ...
    it('should be bounded between 0 and 100', () => {
      const data = dashboard.getDashboardData({ critical: 100, warning: 100, info: 100 });
      expect(data.healthScore).toBeGreaterThanOrEqual(0);
      expect(data.healthScore).toBeLessThanOrEqual(100);
    });
  });
