...
export default async function autonomousPipelineRoutes(
  ...
  app.post('/classify-error', async (request: FastifyRequest, reply: FastifyReply) => {
    return controller.classifyError(request, reply);
  });

  // GET /v1/autonomous/error-stats - 错误统计
  app.get('/error-stats', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get('/timeout/:stageName', async (request: FastifyRequest, reply: FastifyReply) => {
    return controller.getTimeoutForStage(request, reply);
  });

  // POST /v1/autonomous/record-execution - 记录执行数据
  app.post('/record-execution', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get('/retry-stats/:pipelineId', async (request: FastifyRequest, reply: FastifyReply) => {
    return controller.getRetryStats(request, reply);
  });

  // POST /v1/autonomous/configure-retry - 配置重试策略
  app.post('/configure-retry', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.post('/self-healing/recommend', async (request: FastifyRequest, reply: FastifyReply) => {
    return controller.recommendSelfHealing(request, reply);
  });

  // GET /v1/autonomous/self-healing/actions - 获取自修复动作目录
  app.get('/self-healing/actions', async (request: FastifyRequest, reply: FastifyReply) => {
