...
export class DigitalTwinController extends BaseController {
  ...
  async getTwinState(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!twin) throw new OrionError(`Twin '${params.id}' not found`, ErrorCode.NOT_FOUND);
  ...
  async createSandbox(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!twin) throw new OrionError(`Twin '${body.twinId}' not found`, ErrorCode.NOT_FOUND);
  ...
  async stopSandbox(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!sandbox) throw new OrionError(`Sandbox '${params.id}' not found or already stopped`, ErrorCode.NOT_FOUND);
      return sandbox;
  ...
  async destroySandbox(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!destroyed) throw new OrionError(`Sandbox '${params.id}' not found`, ErrorCode.NOT_FOUND);
  ...
  async getSandboxHealth(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!sandbox) throw new OrionError(`Sandbox '${params.id}' not found`, ErrorCode.NOT_FOUND);
      return sandbox;
    }, (sandbox) => this.sendSuccess(reply, sandbox));
  }
  ...
  async startRecording(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!body.name) throw new OrionError('Recording name is required', ErrorCode.VALIDATION_ERROR);
  ...
  async stopRecording(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!session) throw new OrionError(`Recording '${params.recordingId}' not found or not active`, ErrorCode.NOT_FOUND);
      return session;
  ...
  async getRecordingDetail(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!session) throw new OrionError(`Recording '${params.recordingId}' not found`, ErrorCode.NOT_FOUND);
      return {
        ...session,
  ...
  async startReplay(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!body.recordingSessionId) throw new OrionError('recordingSessionId is required', ErrorCode.VALIDATION_ERROR);
      if (!body.sandboxEndpoint) throw new OrionError('sandboxEndpoint is required', ErrorCode.VALIDATION_ERROR);

      const recordingSession = await this.trafficRecorder.getSession(body.recordingSessionId);
      if (!recordingSession) throw new OrionError(`Recording session '${body.recordingSessionId}' not found`, ErrorCode.NOT_FOUND);
  ...
  async getReplayStatus(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!replay) throw new OrionError(`Replay '${params.replayId}' not found`, ErrorCode.NOT_FOUND);
      return {
        id: replay.id,
  ...
  async cancelReplay(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    await this.tryExecute(reply, async () => {
      ...
      if (!replay) throw new OrionError(`Replay '${params.replayId}' not found or not running`, ErrorCode.NOT_FOUND);
      return replay;
