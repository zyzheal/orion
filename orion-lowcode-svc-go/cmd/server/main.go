...
import (
	...
	"orion/lowcode-svc-go/internal/config"
	"orion/lowcode-svc-go/internal/handler"
	...
	nats_subscriber "orion/lowcode-svc-go/pkg/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-lowcode-svc"))
	defer logger.Sync()
	...
	go func() {
		logger.Info("lowcode-svc listening", zap.String("addr", addr))
	...
	logger.Info("shutting down lowcode-svc...")
	if natsSub != nil {
