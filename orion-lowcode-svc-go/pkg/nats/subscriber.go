...
// NATSSubscriber subscribes to LowcodeEvent via NATS JetStream.
...
// Start subscribes to LowcodeEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "lowcode-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received lowcode event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleLowcodeEvent(ctx, &event); err != nil {
		s.log.Error("handle lowcode event", zap.Error(err))
...
func (s *NATSSubscriber) handleLowcodeEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理低代码相关事件
	return nil
}
