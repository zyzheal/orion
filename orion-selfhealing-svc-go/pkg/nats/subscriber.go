...
// Start subscribes to SelfHealingEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "selfhealing-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received selfhealing event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleSelfHealingEvent(ctx, &event); err != nil {
		s.log.Error("handle selfhealing event", zap.Error(err))
...
func (s *NATSSubscriber) handleSelfHealingEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理自愈相关事件
	return nil
}
