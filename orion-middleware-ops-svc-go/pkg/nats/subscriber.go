...
// NATSSubscriber subscribes to MiddlewareOpsEvent via NATS JetStream.
...
// Start subscribes to MiddlewareOpsEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	_, err := s.js.CreateOrUpdateConsumer(ctx, s.stream, jetstream.ConsumerConfig{
		Name:           "middleware-ops-svc-consumer",
	...
	cons, err := s.js.Consumer(ctx, s.stream, "middleware-ops-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received middleware-ops event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleMiddlewareOpsEvent(ctx, &event); err != nil {
		s.log.Error("handle middleware-ops event", zap.Error(err))
...
func (s *NATSSubscriber) handleMiddlewareOpsEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理中间件运维相关事件
	return nil
}
