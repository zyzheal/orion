...
// NATSSubscriber subscribes to all EventBus events via NATS JetStream.
...
// Start subscribes to all event subjects in the stream.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "eventbus-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received EventBus event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleEvent(ctx, &event); err != nil {
		s.log.Error("handle event", zap.Error(err))
...
func (s *NATSSubscriber) handleEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 将事件持久化到数据库
	// EventBus 服务作为事件中枢，接收所有服务的事件并存储
	return nil
}
