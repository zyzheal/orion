...
import (
	...
	"orion/inception-svc-go/internal/config"
	"orion/inception-svc-go/internal/handler"
	"orion/inception-svc-go/internal/repository"
	"orion/inception-svc-go/internal/service"
	nats_subscriber "orion/inception-svc-go/pkg/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-inception-svc"))
	defer logger.Sync()
	...
	r.GET("/healthz", middleware.HealthCheck("orion-inception-svc"))
	...
	logger.Info("inception-svc listening", zap.String("addr", addr))
	...
	logger.Info("shutting down inception-svc...")
	if natsSub != nil {
