...
// NATSSubscriber subscribes to InceptionEvent via NATS JetStream.
...
// Start subscribes to InceptionEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "inception-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received inception event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleInceptionEvent(ctx, &event); err != nil {
		s.log.Error("handle inception event", zap.Error(err))
...
func (s *NATSSubscriber) handleInceptionEvent(ctx context.Context, event *EventBusEvent) error {
