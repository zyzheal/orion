...
// NATSSubscriber subscribes to CanaryEvent via NATS JetStream.
...
// Start subscribes to CanaryEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "canary-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received canary event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleCanaryEvent(ctx, &event); err != nil {
		s.log.Error("handle canary event", zap.Error(err))
...
func (s *NATSSubscriber) handleCanaryEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理 canary 相关事件
	return nil
}
