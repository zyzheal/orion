...
// NATSSubscriber subscribes to ApprovalEvent via NATS JetStream.
...
// Start subscribes to ApprovalEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "approval-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received approval event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleApprovalEvent(ctx, &event); err != nil {
		s.log.Error("handle approval event", zap.Error(err))
...
func (s *NATSSubscriber) handleApprovalEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理审批相关事件
	// 例如：PipelineFailed → 自动创建审批单
	//       DeploymentCompleted → 触发部署审批
	return nil
}
