...
// NATSSubscriber subscribes to IntelligenceEvent via NATS JetStream.
...
// Start subscribes to IntelligenceEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	_, err := s.js.CreateOrUpdateConsumer(ctx, s.stream, jetstream.ConsumerConfig{
		Name:           "intelligence-svc-consumer",
	...
	cons, err := s.js.Consumer(ctx, s.stream, "intelligence-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received intelligence event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleIntelligenceEvent(ctx, &event); err != nil {
		s.log.Error("handle intelligence event", zap.Error(err))
...
func (s *NATSSubscriber) handleIntelligenceEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理智能分析相关事件
	return nil
}
