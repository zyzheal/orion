...
// NATSSubscriber subscribes to PluginEvent via NATS JetStream.
...
// Start subscribes to PluginEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "plugin-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received plugin event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handlePluginEvent(ctx, &event); err != nil {
		s.log.Error("handle plugin event", zap.Error(err))
...
func (s *NATSSubscriber) handlePluginEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 处理插件相关事件
	return nil
}
