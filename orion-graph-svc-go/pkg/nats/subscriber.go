...
// NATSSubscriber subscribes to GraphEvent via NATS JetStream.
...
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "graph-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received GraphEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleGraphEvent(ctx, &event); err != nil {
		s.log.Error("handle graph event", zap.Error(err))
...
func (s *NATSSubscriber) handleGraphEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的 Graph 操作
	// 例如：NodeAdded → 更新图谱节点
	//       EdgeCreated → 更新图谱边
	return nil
}
