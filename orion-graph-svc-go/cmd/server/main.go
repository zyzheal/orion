...
import (
	...
	"orion/graph-svc-go/internal/config"
	"orion/graph-svc-go/internal/handler"
	"orion/graph-svc-go/internal/repository"
	"orion/graph-svc-go/internal/service"
	"orion/go-common/pkg/auth"
	 nats_subscriber "orion/graph-svc-go/pkg/nats"
...
func main() {
	logger := orionlog.Must(orionlog.DefaultConfig("orion-graph-svc"))
	defer logger.Sync()
	...
	r.GET("/healthz", middleware.HealthCheck("orion-graph-svc"))

	addr := fmt.Sprintf(":%d", cfg.Port)
	logger.Info("graph-svc listening", zap.String("addr", addr))
