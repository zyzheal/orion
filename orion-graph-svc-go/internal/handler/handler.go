...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	r := rg.Group("/nodes")
	r.POST("", auth.RequirePermission("graph", "write"), h.Create); r.GET("", h.List); r.GET("/:id", h.Get)
	r.DELETE("/:id", auth.RequirePermission("graph", "delete"), h.Delete)
	r.GET("/count", h.Count)
}

func (h *Handler) Create(c *gin.Context) {
	tenantID := c.GetString("tenant_id")
	var req models.CreateGraphNodeRequest
