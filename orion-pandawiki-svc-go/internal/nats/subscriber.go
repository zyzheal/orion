...
// Start subscribes to PandaWikiEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "pandawiki-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received PandaWikiEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handlePandaWikiEvent(ctx, &event); err != nil {
		s.log.Error("handle pandawiki event", zap.Error(err))
...
func (s *NATSSubscriber) handlePandaWikiEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的 PandaWiki 操作
	// 例如：PageCreated → 同步页面到知识库
	//       PageUpdated → 更新页面缓存
	return nil
}
