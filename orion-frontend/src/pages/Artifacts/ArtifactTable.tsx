...
import {
  EditOutlined, DeleteOutlined, DownloadOutlined, TagOutlined,
  RocketOutlined, StopOutlined, SafetyCertificateOutlined, EyeOutlined,
} from '@ant-design/icons';
import Table, { type TableColumn } from '@/components/Table';
import type { Artifact, ArtifactStage, ArtifactStatus } from '@/api/artifacts';
import {
  stageColorMap, stageLabelMap, statusColorMap, typeLabelMap,
  formatSize, promotionStageOrder,
} from './constants';
...
const ArtifactTable: React.FC<ArtifactTableProps> = ({
  dataSource, loading, currentPage, pageSize, total,
  onDetail, onEdit, onPromote, onTag, onDownload,
  onDeprecate, onQuarantine, onDelete, onPaginationChange,
}) => {
  ...
  const columns: TableColumn<Artifact>[] = [
    {
      ...
      render: (v: unknown, record: Artifact) => (
        <Space direction="vertical" size={0}>
          <Text strong style={{ cursor: 'pointer' }} onClick={() => onDetail(record)}>{String(v)}</Text>
    ...
    },
    {
      ...
      render: (_: unknown, record: Artifact) => (
        <Space size="small" wrap>
          <Tooltip title="详情"><Button type="link" size="small" icon={<EyeOutlined />} onClick={() => onDetail(record)}>详情</Button></Tooltip>
          <Tooltip title="编辑"><Button type="link" size="small" icon={<EditOutlined />} onClick={() => onEdit(record)} /></Tooltip>
          {record.status === 'available' && nextAvailableStage(record.stage) && (
            <Tooltip title={`晋升到 ${stageLabelMap[nextAvailableStage(record.stage)!]}`}><Button type="link" size="small" icon={<RocketOutlined />} onClick={() => onPromote(record)} /></Tooltip>
          )}
          {record.status === 'available' && (
            <Tooltip title="管理标签"><Button type="link" size="small" icon={<TagOutlined />} onClick={() => onTag(record)} /></Tooltip>
          )}
          <Tooltip title="下载"><Button type="link" size="small" icon={<DownloadOutlined />} onClick={() => onDownload(record)} /></Tooltip>
          {record.status === 'available' && (
            <>
              <Tooltip title="废弃"><Popconfirm title="确认废弃该制品?" onConfirm={() => onDeprecate(record.id)}><Button type="link" size="small" danger icon={<StopOutlined />} /></Popconfirm></Tooltip>
              <Tooltip title="隔离"><Popconfirm title="确认隔离该制品?" onConfirm={() => onQuarantine(record.id)}><Button type="link" size="small" danger icon={<StopOutlined />}>隔离</Button></Popconfirm></Tooltip>
            </>
          )}
