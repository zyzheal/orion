...
import { Typography, Button, Space, Tag, Card, Modal, Form, Input, Select, InputNumber, message, Popconfirm } from 'antd';
...
const AlertConfig: React.FC = () => {
  ...
  const [rules, setRules] = useState<AlertRule[]>([
    { id: 'r1', name: '日费用超预算 80%', metric: 'daily_cost', operator: '>', threshold: 80, severity: 'warning', channels: ['dingtalk'], enabled: true },
    { id: 'r2', name: '日费用超预算 95%', metric: 'daily_cost', operator: '>', threshold: 95, severity: 'critical', channels: ['dingtalk', 'email'], enabled: true },
    { id: 'r3', name: 'Token 用量异常', metric: 'token_usage', operator: '>', threshold: 1000000, severity: 'info', channels: ['inapp'], enabled: true },
  ]);
  ...
  const alertColumns: TableColumn<CostAlert>[] = [
    ...
    { key: 'threshold', title: '阈值', dataIndex: 'threshold', width: 80, render: (v: unknown) => <Tag>{Number(v)}%</Tag> },
  ...
  const ruleColumns: TableColumn<AlertRule>[] = [
    ...
    { key: 'severity', title: '级别', dataIndex: 'severity', width: 80, render: (v: unknown) => <Tag color={severityColorMap[String(v)]}>{String(v)}</Tag> },
    { key: 'channels', title: '通知渠道', dataIndex: 'channels', width: 160, render: (v: unknown) => <Space size={4}>{Array.isArray(v) ? v.map((c) => <Tag key={c}>{c}</Tag>) : null}</Space> },
    { key: 'enabled', title: '状态', dataIndex: 'enabled', width: 80, render: (v: unknown) => <StatusBadge status={v ? 'success' as any : 'warning' as any} size="small" /> },
    { key: 'actions', title: '操作', width: 100, render: (_: unknown, record: any) => (
      <Popconfirm title="确认删除?" onConfirm={() => setRules((prev) => prev.filter((r) => r.id !== record.id))}>
  ...
  return (
    <div style={{ padding: 0 }}>
      <div style={{ marginBottom: 24 }}>
        <Title level={3} style={{ margin: 0 }}>告警设置</Title>
      ...
      {/* Create Rule Modal */}
      <Modal title="创建告警规则" open={createModalVisible} onCancel={() => setCreateModalVisible(false)} onOk={handleCreateRule} confirmLoading={submitting}>
        <Form form={createForm} layout="vertical">
          <Form.Item name="name" label="规则名称" rules={[{ required: true }]}><Input placeholder="日费用超预算告警" /></Form.Item>
          <Form.Item name="metric" label="监控指标" rules={[{ required: true }]}><Select options={metricOptions} /></Form.Item>
          ...
          <Form.Item name="severity" label="告警级别" rules={[{ required: true }]} initialValue="warning">
            <Select options={[{ label: 'Info', value: 'info' }, { label: 'Warning', value: 'warning' }, { label: 'Critical', value: 'critical' }]} />
          </Form.Item>
          <Form.Item name="channels" label="通知渠道" rules={[{ required: true }]} initialValue={['dingtalk']}>
