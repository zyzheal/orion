...
import { Typography, Button, Space, Tag, Card, DatePicker, Select, message, Table as AntTable, Row, Col, Statistic } from 'antd';
...
const CostDetail: React.FC = () => {
  ...
  const [dateRange, setDateRange] = useState<[dayjs.Dayjs | null, dayjs.Dayjs | null] | null>([dayjs().subtract(7, 'day'), dayjs()]);
  ...
  const columns: ColumnsType<CostRecord> = [
    ...
    { key: 'inputTokens', title: '输入 Token', dataIndex: 'inputTokens', render: (v: unknown) => <Text>{Number(v).toLocaleString()}</Text> },
    { key: 'outputTokens', title: '输出 Token', dataIndex: 'outputTokens', render: (v: unknown) => <Text>{Number(v).toLocaleString()}</Text> },
    { key: 'totalCost', title: '费用', dataIndex: 'totalCost', render: (v: unknown) => <Text strong>${Number(v).toFixed(4)}</Text> },
  ...
  return (
    <div style={{ padding: 0 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: spacing[6] }}>
        ...
        <Space>
          <Select value={groupBy} onChange={setGroupBy} style={{ width: 120 }} options={groupByOptions} />
      ...
      <Card title="成本明细记录">
        <AntTable columns={columns} dataSource={costs} loading={loading} rowKey="id" size="middle" scroll={{ x: 1200 }} />
      </Card>
    </div>
  );
};
