...
import {
  Typography, Tag, Space, Descriptions, Button, Table as AntTable,
  Popconfirm, Card, Input, message,
} from 'antd';
import {
  PlusOutlined, ReloadOutlined, SearchOutlined,
  RocketOutlined, TeamOutlined, CheckCircleOutlined,
  LinkOutlined, WarningOutlined,
} from '@ant-design/icons';
import type { TableColumn } from '@/components/Table';
import type {
  InternalLibrary, LibraryVersion, LibraryDependent,
  LibraryLanguage, LibraryStatus, VersionStatus,
...
const DependencyCheckTool: React.FC = () => {
  ...
  return (
    <Card size="small" title={<Space><LinkOutlined /> 依赖检查工具</Space>}>
      <Space wrap>
        <Input
...
const InfoTab: React.FC<{ library: InternalLibrary }> = ({ library }) => (
  <Descriptions column={2} bordered size="small">
    ...
    <Descriptions.Item label="描述" span={2}>{library.description || '-'}</Descriptions.Item>
    <Descriptions.Item label="语言"><Tag color="cyan">{languageLabels[library.language]}</Tag></Descriptions.Item>
    <Descriptions.Item label="状态"><Tag color={statusColorMap[library.status]}>{library.status}</Tag></Descriptions.Item>
    <Descriptions.Item label="团队"><TeamOutlined /> {library.owner}</Descriptions.Item>
    <Descriptions.Item label="维护者">{library.maintainers?.join(', ') || '-'}</Descriptions.Item>
    <Descriptions.Item label="仓库" span={2}><Text code>{library.repository}</Text></Descriptions.Item>
    <Descriptions.Item label="文档">{library.documentation ? <a href={library.documentation} target="_blank" rel="noopener noreferrer">{library.documentation}</a> : '-'}</Descriptions.Item>
    <Descriptions.Item label="当前版本"><Text code>{library.currentVersion}</Text></Descriptions.Item>
    <Descriptions.Item label="最新稳定版"><Text code>{library.latestStableVersion}</Text></Descriptions.Item>
    ...
    <Descriptions.Item label="使用最新版">{library.dependents?.reposUsingLatest ?? 0}</Descriptions.Item>
    <Descriptions.Item label="需要升级">{library.dependents?.reposNeedingUpgrade ?? 0}</Descriptions.Item>
    <Descriptions.Item label="创建时间">{dayjs(library.createdAt).format('YYYY-MM-DD HH:mm')}</Descriptions.Item>
    <Descriptions.Item label="更新时间">{dayjs(library.updatedAt).format('YYYY-MM-DD HH:mm')}</Descriptions.Item>
  </Descriptions>
);
...
const VersionTab: React.FC<VersionTabProps> = ({ versions, onOpenPublish, onOpenDeprecateVersion }) => {
  const versionColumns: TableColumn<LibraryVersion>[] = [
    ...
    { key: 'actions', title: '操作', width: 100, render: (_: unknown, record: LibraryVersion) => (
      record.status !== 'deprecated' ? (
        <Popconfirm title="确认废弃此版本?" onConfirm={() => onOpenDeprecateVersion(record.version)}>
  ...
  return (
    <div>
      <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'space-between' }}>
        ...
        <Button type="primary" size="small" icon={<PlusOutlined />} onClick={onOpenPublish}>发布新版本</Button>
      </div>
...
const DependentsTab: React.FC<DependentsTabProps> = ({ dependents, onUpdateStats, onOpenAddDependent, onUpdateDependent }) => {
  const dependentColumns: TableColumn<LibraryDependent>[] = [
    ...
    { key: 'actions', title: '操作', width: 120, render: (_: unknown, record: LibraryDependent) => (
      record.upgradeAvailable && record.latestCompatibleVersion ? (
        <Popconfirm title={`确认升级到 ${record.latestCompatibleVersion}?`} onConfirm={() => onUpdateDependent(record.repoName, record.latestCompatibleVersion!)}>
    ...
    <div>
      <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Space>
          ...
          <Button size="small" icon={<ReloadOutlined />} onClick={onUpdateStats}>刷新统计</Button>
        </Space>
        <Button type="primary" size="small" icon={<PlusOutlined />} onClick={onOpenAddDependent}>添加依赖</Button>
      </div>
...
export function getLibraryTabItems(
  ...
  const items: LibraryTabItem[] = [
    ...
    { key: 'versions', label: '版本管理', children: <VersionTab versions={versions} onOpenPublish={onOpenPublish} onOpenDeprecateVersion={onOpenDeprecateVersion} /> },
    { key: 'dependents', label: '依赖者管理', children: <DependentsTab dependents={dependents} onUpdateStats={onUpdateStats} onOpenAddDependent={onOpenAddDependent} onUpdateDependent={onUpdateDependent} /> },
