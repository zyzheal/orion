  ...
  const columns: TableColumn<InternalLibrary>[] = [
    ...
    },
    {
      ...
      render: (_: unknown, record: InternalLibrary) => (
        <Tag color="cyan">{languageLabels[record.language] || record.language}</Tag>
      ),
    },
    {
      key: 'version',
    ...
    },
    {
      ...
      render: (_: unknown, record: InternalLibrary) => (
        <Space>
          <TeamOutlined /> <Text>{record.owner}</Text>
        </Space>
      ),
    },
    {
      ...
      render: (_: unknown, record: InternalLibrary) => {
        ...
        return (
          <Space direction="vertical" size={0}>
            ...
            <Text
              type={
                q.securityScore && q.securityScore >= 90
                  ? 'success'
                  : q.securityScore && q.securityScore >= 70
                    ? 'warning'
                    : 'danger'
              }
              style={{ fontSize: 12 }}
      ...
      render: (_: unknown, record: InternalLibrary) => (
        <Space size="small" wrap>
          ...
          {record.status === 'deprecated' || record.status === 'archived' ? (
            <Tooltip title="激活">
              <Popconfirm title="确认激活?" onConfirm={() => onActivate(record.id)}>
                <Button type="link" size="small" icon={<PlayCircleOutlined />} />
              </Popconfirm>
            </Tooltip>
          ) : record.status === 'active' ? (
            <Tooltip title="废弃">
              <Popconfirm title="确认废弃?" onConfirm={() => onDeprecate(record)}>
