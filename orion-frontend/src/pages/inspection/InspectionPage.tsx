...
const RulesTab: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'operator',
      key: 'operator',
      render: (op: string, r: InspectionRule) => `${op} ${r.threshold}`,
    },
    { title: '调度', dataIndex: 'schedule', key: 'schedule' },
    {
      ...
      key: 'enabled',
      render: (e: boolean) => (
        <Tag color={e ? colors.success[500] : colors.neutral[400]}>{e ? '启用' : '禁用'}</Tag>
      ),
    },
    {
      title: '操作',
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          ...
          <Form.Item label="目标" name="target" rules={[{ required: true }]}>
            <Input placeholder="如: host-001, db-primary" />
          </Form.Item>
...
const ReportsTab: React.FC = () => {
  ...
  const columns = [
    { title: '报告标题', dataIndex: 'title', key: 'title' },
    {
      ...
      render: (s: InspectionReport['summary']) => (
        <Progress
          percent={s.score}
          size="small"
          status={s.score >= 80 ? 'success' : s.score >= 60 ? 'normal' : 'exception'}
          style={{ width: 100 }}
        />
      ),
    },
    {
      ...
      dataIndex: 'summary',
      key: 'total',
      render: (s: InspectionReport['summary']) => s.total,
    },
    {
      title: '通过',
      dataIndex: 'summary',
      key: 'passed',
      render: (s: InspectionReport['summary']) => (
        <span style={{ color: colors.success[500] }}>{s.passed}</span>
      ),
    },
    {
      title: '失败',
      ...
      key: 'failed',
      render: (s: InspectionReport['summary']) => (
        <span style={{ color: colors.error[500] }}>{s.failed}</span>
      ),
    },
    {
      title: '生成时间',
...
const InspectionPage: React.FC = () => {
  ...
  const loadScore = async () => {
    try {
      ...
      const data = (res.data as { data?: { score?: number; details?: Record<string, number> } })
        ?.data;
