...
const DiagnosticSessions: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        message.error(`加载诊断会话失败：${error.message}`);
      } else {
        message.error('加载诊断会话失败，请稍后重试');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleAddSymptom = async (values: any) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        message.error(`添加症状失败：${error.message}`);
      } else {
        message.error('添加症状失败，请稍后重试');
      }
    }
  };
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          if (error instanceof Error) {
            message.error(`完成会话失败：${error.message}`);
          } else {
                ...
                <Space direction="vertical" style={{ width: '100%' }}>
                  {sessionDetail.symptoms.map((symptom: DiagnosticSymptom, idx: number) => (
                    <div
                      ...
                      style={{
                        padding: spacing[3],
                        background: isDark ? 'rgba(255,255,255,0.05)' : colors.neutral[50],
                        borderRadius: 6,
                      }}
                    >
                      <Space>
