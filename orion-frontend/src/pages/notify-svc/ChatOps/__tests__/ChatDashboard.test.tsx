...
describe('ChatDashboard', () => {
  ...
  it('should render metric cards when data is loaded', async () => {
    (chatopsApi.getDashboardStats as ReturnType<typeof vi.fn>).mockResolvedValue({
      data: mockStats,
    });
  ...
  it('should render empty state on API error', async () => {
    (chatopsApi.getDashboardStats as ReturnType<typeof vi.fn>).mockRejectedValue(
      new Error('API error')
    );
  ...
  it('should show loading skeleton on initial load', () => {
    (chatopsApi.getDashboardStats as ReturnType<typeof vi.fn>).mockReturnValue(
      new Promise(() => {})
    );
