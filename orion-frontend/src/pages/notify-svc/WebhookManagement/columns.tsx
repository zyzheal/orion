...
export const buildWebhookColumns = (deps: WebhookColumnsDeps): TableColumn<Webhook>[] => [
  {
    key: 'url',
    title: 'URL',
    dataIndex: 'url',
      ...
      <Space size="small">
        ...
        <Tooltip title="日志">
          <Button
            ...
            size="small"
            icon={<EyeOutlined />}
            onClick={() => deps.onViewLogs(record)}
          />
        </Tooltip>
        <Tooltip title="编辑">
          <Button
        ...
        <Popconfirm title="确认删除该 Webhook?" onConfirm={() => deps.onDelete(record.id)}>
          <Tooltip title="删除">
...
export const buildLogColumns = (): TableColumn<WebhookLog>[] => [
  {
    key: 'event',
    title: '事件',
    dataIndex: 'event',
