...
import type { Artifact, PromotionRecord, Tag as TagType } from '@/api/artifacts';
import {
  stageColorMap,
  stageLabelMap,
  statusColorMap,
  ...
  return (
    <Descriptions column={2} bordered size="small">
      ...
      <Descriptions.Item label="阶段">
        <Tag color={stageColorMap[a.stage]}>{stageLabelMap[a.stage]}</Tag>
      </Descriptions.Item>
      <Descriptions.Item label="状态">
        <Tag color={statusColorMap[a.status]}>{a.status}</Tag>
      </Descriptions.Item>
          ...
          <Row gutter={16}>
            <Col span={6}>
              <Card size="small">
                <Statistic
                  title="严重"
                  value={artifact.security.scanResults.critical}
                  valueStyle={{
                    color:
                      artifact.security.scanResults.critical > 0
                        ? colors.error[600]
                        : colors.success[600],
                  }}
                />
            ...
            <Col span={6}>
              <Card size="small">
                <Statistic
                  title="高危"
                  value={artifact.security.scanResults.high}
                  valueStyle={{
                    color:
                      artifact.security.scanResults.high > 0
                        ? colors.error[500]
                        : colors.success[600],
                  }}
                />
            ...
            <Col span={6}>
              <Card size="small">
                <Statistic
                  title="中危"
                  value={artifact.security.scanResults.medium}
                  valueStyle={{
                    color:
                      artifact.security.scanResults.medium > 0
                        ? colors.warning[500]
                        : colors.success[600],
                  }}
                />
            ...
            <Col span={6}>
              <Card size="small">
                <Statistic
                  title="低危"
                  value={artifact.security.scanResults.low}
      ...
      {artifact.tests.unitTests && (
        ...
        >
          ...
          <Descriptions.Item label="覆盖率">
            {artifact.tests.unitTests.coverage ? `${artifact.tests.unitTests.coverage}%` : '-'}
          </Descriptions.Item>
        </Descriptions>
      )}
      {artifact.tests.integrationTests && (
        ...
        >
          ...
          <Descriptions.Item label="失败">
            {artifact.tests.integrationTests.failed}
          </Descriptions.Item>
        </Descriptions>
      )}
    </div>
...
const DeploymentsTab: React.FC<{ artifact: Artifact }> = ({ artifact }) => {
  ...
  return (
    <Timeline
      items={artifact.deployments.map((d) => ({
        children: (
          <Space direction="vertical" size={0}>
            <Text strong>{d.environment}</Text>
            <Text type="secondary">
              <Tag
                color={d.status === 'success' ? 'green' : d.status === 'failed' ? 'red' : 'orange'}
              >
                {d.status}
              </Tag>
...
export function getArtifactTabItems(
  ...
  return [
    ...
    { key: 'deployments', label: '部署历史', children: <DeploymentsTab artifact={a} /> },
    {
      key: 'tags',
      label: '标签',
      children: <TagsTab artifact={a} tags={tags} onAddTags={onAddTags} />,
    },
