...
import { Alert, Card, Col, Descriptions, Progress, Row, Space, Statistic, Tag } from 'antd';
...
import type { BudgetAlert, BudgetConfig, BudgetUsage } from '../types';

function getProgressStatus(percent: number): 'exception' | 'normal' | 'success' {
...
export const UsageCard: React.FC<UsageCardProps> = ({ usage, config }) => (
  <Card
    title={
      <>
  ...
  >
    {usage?.alerts && usage.alerts.length > 0 && (
      <Space
        ...
        size={8}
      >
        {usage.alerts.map((alert: BudgetAlert, i: number) => {
          ...
          return (
            <Alert
              ...
              type={cfg.type}
              showIcon
              icon={alert.level === 'critical' ? <WarningOutlined /> : <CheckCircleOutlined />}
            />
          );
        })}
      </Space>
    ...
    <Row gutter={16}>
      <Col span={12}>
        ...
        <Progress
          ...
          status={getProgressStatus(usage?.time_percent ?? 0)}
      ...
      <Col span={12}>
        ...
        <Progress
          ...
          status={getProgressStatus(usage?.cost_percent ?? 0)}
          style={{ marginTop: spacing.sm }}
        />
      </Col>
    </Row>
