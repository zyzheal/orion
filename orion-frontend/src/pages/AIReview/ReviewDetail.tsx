...
import { Typography, Card, Descriptions, Tag, Spin, Space, Button, Alert, Row, Col, Statistic, Table, message } from 'antd';
import { ArrowLeftOutlined, ReloadOutlined, WarningOutlined, CloseCircleOutlined, InfoCircleOutlined } from '@ant-design/icons';
...
const AIReviewDetail: React.FC = () => {
  ...
  const issueColumns = [
    { title: '严重程度', dataIndex: 'severity', key: 'severity', render: (s: string) => <Tag color={s === 'critical' ? 'red' : s === 'warning' ? 'orange' : 'blue'}>{s}</Tag> },
  ...
  const mockIssues = Array.from({ length: Math.min(detail.totalIssues, 20) }, (_, i) => ({
    key: `issue-${i}`,
    severity: i < detail.criticalCount ? 'critical' : i < detail.criticalCount + detail.warningCount ? 'warning' : 'info',
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
        <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/ai-review/history')}>返回</Button>
      ...
      <Alert
        ...
        type={detail.status === 'completed' ? 'success' : detail.status === 'failed' ? 'error' : 'info'}
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={6}>
          <Card>
            <Statistic title="总问题数" value={detail.totalIssues} prefix={<CloseCircleOutlined />} />
          </Card>
        </Col>
        <Col span={6}>
          <Card>
