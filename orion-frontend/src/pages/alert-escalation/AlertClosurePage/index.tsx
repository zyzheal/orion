...
import {
  ...
  listClosures, acknowledgeAlert, resolveAlert, getMetrics,
  type EscalationPolicy, type EscalationTrigger, type AlertClosure, type AlertMetrics,
...
const AlertClosurePage: React.FC = () => {
  ...
  const handleRefresh = async () => {
    setLoading(true);
    try {
      if (activeTab === 'policies') { await Promise.all([loadPolicies(), loadTriggers()]); }
      else if (activeTab === 'closures') await loadClosures();
      else await loadMetrics();
    } finally { setLoading(false); }
  };

  useEffect(() => { handleRefresh(); }, [activeTab]);

  // --- Policy CRUD ---
  const handleCreate = () => { setSelectedItem(null); form.resetFields(); setModalOpen(true); };
  const handleEdit = (r: EscalationPolicy) => { setSelectedItem(r); form.setFieldsValue(r); setModalOpen(true); };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (typeof rulesObj === 'string') {
        try { rulesObj = JSON.parse(rulesObj); } catch { message.error('Rules 格式无效'); return; }
      }
      if (!Array.isArray(rulesObj) || rulesObj.length === 0) { message.error('至少需要一条升级规则'); return; }
  ...
  const handleViewDetail = (r: EscalationPolicy) => { setSelectedItem(r); setDetailOpen(true); };
    ...
    Modal.confirm({
      ...
      content: (
        <TextArea
          rows={3}
          placeholder="请填写解决说明"
          onChange={(e) => { closure.resolutionNote = e.target.value; }}
        />
      ),
      onOk: async () => {
        try {
  ...
  const policyColumns = [
    ...
    { title: '严重度', dataIndex: 'severity', key: 'severity', render: (v: string) => <Tag color={SEVERITY_MAP[v]?.color}>{SEVERITY_MAP[v]?.label || v}</Tag> },
    { title: '规则数', key: 'rulesCount', render: (_: any, r: EscalationPolicy) => (r.rules?.length || 0) },
    ...
    { title: '创建时间', dataIndex: 'createdAt', key: 'createdAt', width: 160, render: (v: string) => v ? new Date(v).toLocaleString() : '-' },
    {
      ...
      render: (_: any, r: EscalationPolicy) => (
        <Space>
          ...
          <Button type="link" size="small" onClick={() => handleEvaluate(r)}>模拟</Button>
  ...
  const triggerColumns = [
    ...
    { title: '触发时间', dataIndex: 'triggeredAt', key: 'triggeredAt', width: 160, render: (v: string) => v ? new Date(v).toLocaleString() : '-' },
    {
      ...
      render: (_: any, r: EscalationTrigger) => (
        r.status === 'pending' ? (
          <Button type="link" size="small" onClick={() => handleTriggerResolve(r)}>解决</Button>
        ) : null
      ),
    },
  ];
  ...
  const closureColumns = [
    ...
    { title: '状态', dataIndex: 'status', key: 'status', width: 90, render: (v: string) => <Tag color={CLOSURE_STATUS[v]?.color}>{CLOSURE_STATUS[v]?.label || v}</Tag> },
    { title: '确认人', dataIndex: 'acknowledgedBy', key: 'acknowledgedBy', width: 90, render: (v: string) => v || '-' },
    { title: '确认时间', dataIndex: 'acknowledgedAt', key: 'acknowledgedAt', width: 160, render: (v: string) => v ? new Date(v).toLocaleString() : '-' },
    { title: '解决人', dataIndex: 'resolvedBy', key: 'resolvedBy', width: 90, render: (v: string) => v || '-' },
    { title: '解决时间', dataIndex: 'resolvedAt', key: 'resolvedAt', width: 160, render: (v: string) => v ? new Date(v).toLocaleString() : '-' },
    { title: 'MTTR', dataIndex: 'mttrSeconds', key: 'mttrSeconds', width: 90, render: (v: number) => v > 0 ? `${v}s` : '-' },
    {
      title: '操作', key: 'action', width: 160,
      render: (_: any, r: AlertClosure) => (
        <Space>
          {r.status === 'pending' && <Button type="link" size="small" onClick={() => handleAcknowledge(r)}>确认</Button>}
          {(r.status === 'pending' || r.status === 'acknowledged') && <Button type="link" size="small" onClick={() => handleResolve(r)}>解决</Button>}
        </Space>
      ),
    },
  ];
    ...
    <div style={{ padding: spacing.lg }}>
          ...
          <Card
            ...
            extra={
              <Space>
                ...
                <Button type="primary" size="small" onClick={handleCreate}>新建策略</Button>
              </Space>
            }
          >
            <Table columns={policyColumns} dataSource={policies} rowKey="id" loading={loading} size="small"
              locale={{ emptyText: <Empty description="暂无升级策略" /> }}
              pagination={{ pageSize: 10 }}
            />
          </Card>
          <Card title="升级触发器" extra={<Button icon={<ReloadOutlined />} size="small" onClick={loadTriggers}>刷新</Button>}>
            <Table columns={triggerColumns} dataSource={triggers} rowKey="id" size="small"
      ...
      {activeTab === 'closures' && (
        <Card
          ...
          extra={
            <Space>
              <Select style={{ width: 120 }} value={policyStatus} onChange={setPolicyStatus} allowClear placeholder="状态">
                <Option value="pending">待确认</Option>
                <Option value="acknowledged">已确认</Option>
                <Option value="resolved">已解决</Option>
              </Select>
              <Button icon={<ReloadOutlined />} size="small" onClick={loadClosures} loading={loading}>刷新</Button>
            </Space>
          }
        >
          <Table columns={closureColumns} dataSource={closures} rowKey="id" loading={loading} size="small"
      ...
      {activeTab === 'metrics' && metrics && (
        <Row gutter={[spacing.md, spacing.md]}>
          <Col span={6}><Card><Typography.Text type="secondary">总告警数</Typography.Text><div style={{ fontSize: 24, fontWeight: 600, color: colors.primary[500] }}>{metrics.totalAlerts}</div></Card></Col>
          <Col span={6}><Card><Typography.Text type="secondary">待确认</Typography.Text><div style={{ fontSize: 24, fontWeight: 600, color: colors.warning[500] }}>{metrics.openCount}</div></Card></Col>
          <Col span={6}><Card><Typography.Text type="secondary">已确认</Typography.Text><div style={{ fontSize: 24, fontWeight: 600, color: colors.info[500] }}>{metrics.acknowledgedCount}</div></Card></Col>
          <Col span={6}><Card><Typography.Text type="secondary">已解决</Typography.Text><div style={{ fontSize: 24, fontWeight: 600, color: colors.success[500] }}>{metrics.resolvedCount}</div></Card></Col>
          <Col span={24}>
            <Card>
              <Typography.Text type="secondary">平均 MTTR</Typography.Text>
              <div style={{ fontSize: 32, fontWeight: 700, color: colors.purple[500], marginTop: 4 }}>
      ...
      <Modal title={selectedItem ? '编辑策略' : '新建策略'} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)} width={640}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="rules" label="升级规则 (JSON 数组)" rules={[{ required: true, message: '请配置升级规则' }]}>
            <TextArea rows={6} placeholder={JSON.stringify([
      ...
      <Modal title="策略详情" open={detailOpen} onCancel={() => setDetailOpen(false)} footer={<Button onClick={() => setDetailOpen(false)}>关闭</Button>} width={600}>
        {selectedItem && (
          <Descriptions bordered column={1} size="small">
            <Descriptions.Item label="策略名称">{selectedItem.name}</Descriptions.Item>
            <Descriptions.Item label="严重度">
              <Tag color={SEVERITY_MAP[selectedItem.severity]?.color}>{SEVERITY_MAP[selectedItem.severity]?.label || selectedItem.severity}</Tag>
            </Descriptions.Item>
            <Descriptions.Item label="创建者">{selectedItem.createdBy}</Descriptions.Item>
            <Descriptions.Item label="创建时间">{selectedItem.createdAt ? new Date(selectedItem.createdAt).toLocaleString() : '-'}</Descriptions.Item>
