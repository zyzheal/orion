...
import { Typography, Card, Table, Tag, Space, Button, Input, Select, Modal, Form, message } from 'antd';
...
const IncidentList: React.FC = () => {
  ...
  const columns = [
    ...
    { title: '类型', dataIndex: 'type', key: 'type', render: (text: string) => <Text strong>{text}</Text> },
    {
      title: '严重程度',
      dataIndex: 'severity',
      key: 'severity',
    ...
    },
    {
      ...
      render: (_: any, record: SelfHealingIncident) => (
        <Button type="link" size="small" icon={<EyeOutlined />} onClick={() => navigate(`/self-healing/incidents/${record.id}`)}>
          详情
        </Button>
      ),
    },
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <Title level={3} style={{ margin: 0 }}>事件列表</Title>
      ...
      >
        <Form form={createForm} layout="vertical" onFinish={handleCreate}>
          ...
          <Form.Item name="environment" label="环境" rules={[{ required: true, message: '请输入环境' }]}>
            <Select>
