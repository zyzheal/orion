...
export const ALERT_LEVEL_CONFIG: Record<string, AlertLevelConfig> = {
...
export const TIME_POLICY_OPTIONS: { value: string; label: string }[] = [
  { value: 'warn', label: '仅警告 - 记录警告但不中断' },
  { value: 'block', label: '阻断执行 - 超过预算后阻止运行' },
  { value: 'rollback', label: '回滚 - 超时后自动回滚' },
];

export const COST_POLICY_OPTIONS: { value: string; label: string }[] = [
  { value: 'warn', label: '仅警告 - 记录警告但不中断' },
  { value: 'block', label: '阻断执行 - 超过预算后阻止运行' },
];
