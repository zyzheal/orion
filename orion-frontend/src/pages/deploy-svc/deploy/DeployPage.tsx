...
const DeployPage: React.FC = () => {
  ...
  // Artifact version picker state
  const [artifactVersions, setArtifactVersions] = useState<{ label: string; value: string }[]>([]);
  ...
  const handleCreate = async () => {
    try {
      ...
      envVars.forEach((e) => { if (e.key.trim()) envOverrides[e.key.trim()] = e.value; });
      const payload = {
  ...
  // Fetch artifact versions when appName changes
  const handleAppNameChange = async (appName: string) => {
    if (!appName) return;
      ...
      >
        <Form form={createForm} layout="vertical">
          <Form.Item name="appName" label="应用名称" rules={[{ required: true, message: '请输入应用名称' }]}>
            <Input
              placeholder="如: orion-platform"
              onChange={(e) => handleAppNameChange(e.target.value)}
            />
          </Form.Item>
          <Form.Item name="version" label="版本" rules={[{ required: true, message: '请选择或输入版本号' }]}>
            <Select
              showSearch
              placeholder="请选择或输入版本号"
              loading={versionLoading}
              options={artifactVersions.length > 0 ? artifactVersions : undefined}
              notFoundContent="暂无可用制品版本，可手动输入"
              allowClear
            />
          </Form.Item>
