...
import {
  AppstoreOutlined, PlusOutlined, ReloadOutlined, EditOutlined, DeleteOutlined,
  EyeOutlined, BellOutlined,
} from '@ant-design/icons';
import { colors, spacing } from '@/tokens';
import {
  listCatalogItems, createCatalogItem,
  updateCatalogItem, deleteCatalogItem, getSLABreaches,
  type ServiceCatalog, type SLABreach,
...
const ServiceCatalogPage: React.FC = () => {
  ...
  const loadBreaches = useCallback(async () => {
    ...
    } catch { setBreaches([]); setTotalBreaches(0); }
  }, []);
  ...
  const handleEdit = (r: ServiceCatalog) => { setSelectedItem(r); form.setFieldsValue(r); setModalOpen(true); };
  const handleViewDetail = (r: ServiceCatalog) => { setSelectedItem(r); setDetailOpen(true); };
  ...
  const handleDelete = (id: string) => {
    Modal.confirm({
      ...
      content: '删除后不可恢复',
      onOk: async () => {
        try { await deleteCatalogItem(id); message.success('删除成功'); loadItems(); }
  ...
  const breachColumns = [
    ...
    { title: '状态', dataIndex: 'status', key: 'status', width: 100,
      render: (v: string) => <Tag color={STATUS_CONFIG[v]?.color || 'default'}>{STATUS_CONFIG[v]?.label || v}</Tag> },
  ];
  ...
  return (
    <div style={{ padding: spacing.lg }}>
      ...
      <Row gutter={[spacing.md, spacing.md]} style={{ marginBottom: spacing.md }}>
        <Col span={6}>
          <Card>
            ...
            <div style={{ fontSize: 24, fontWeight: 600, color: colors.primary[500] }}>{items.length}</div>
          </Card>
        </Col>
        <Col span={6}>
          <Card>
            <Typography.Text type="secondary">已启用</Typography.Text>
            <div style={{ fontSize: 24, fontWeight: 600, color: colors.success[500] }}>{enabledCount}</div>
          </Card>
        </Col>
        <Col span={6}>
          <Card>
            <Typography.Text type="secondary">SLA 违约总数</Typography.Text>
            <div style={{ fontSize: 24, fontWeight: 600, color: colors.error[500] }}>{totalBreaches}</div>
          </Card>
        </Col>
        <Col span={6}>
          <Card>
            <Typography.Text type="secondary">活跃违约</Typography.Text>
            <div style={{ fontSize: 24, fontWeight: 600, color: colors.warning[500] }}>{breaches.length}</div>
          </Card>
        </Col>
      </Row>
        ...
        <Card
          ...
          extra={
            <Space>
              ...
              <Button type="primary" size="small" icon={<PlusOutlined />} onClick={handleCreate}>新建服务</Button>
            </Space>
          }
        >
          {items.length === 0 ? <Empty description="暂无服务，请注册第一个服务" /> :
      ...
      {activeTab === 'sla' && (
        <Card
          title={<><BellOutlined /> SLA 违约记录</>}
          extra={<Button icon={<ReloadOutlined />} size="small" onClick={loadBreaches}>刷新</Button>}
        >
          {breaches.length === 0 ? <Empty description="暂无 SLA 违约记录" /> :
            <Table columns={breachColumns} dataSource={breaches} rowKey="requestId" size="small"
              pagination={{ pageSize: 10 }} />
          }
        </Card>
      )}

      <Modal title={selectedItem ? '编辑服务' : '新建服务'} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)} width={560}>
      ...
      <Modal title="服务详情" open={detailOpen} onCancel={() => setDetailOpen(false)} footer={<Button onClick={() => setDetailOpen(false)}>关闭</Button>} width={560}>
        {selectedItem && (
          <Descriptions bordered column={1} size="small">
            ...
            <Descriptions.Item label="服务值">
              <pre style={{ margin: 0, fontSize: 12, maxHeight: 120, overflow: 'auto' }}>{selectedItem.value}</pre>
            </Descriptions.Item>
            <Descriptions.Item label="启用状态">
              <Tag color={selectedItem.enabled ? 'green' : 'default'}>{selectedItem.enabled ? '已启用' : '已禁用'}</Tag>
            </Descriptions.Item>
            <Descriptions.Item label="创建时间">{selectedItem.createdAt ? new Date(selectedItem.createdAt).toLocaleString() : '-'}</Descriptions.Item>
            <Descriptions.Item label="更新时间">{selectedItem.updatedAt ? new Date(selectedItem.updatedAt).toLocaleString() : '-'}</Descriptions.Item>
          </Descriptions>
        )}
      </Modal>
    </div>
