...
const defaultModules: Record<string, MenuModuleConfig> = {
  ...
  '/delivery': {
    ...
    children: [
      ...
      // 流水线配置
      {
        key: '/pipeline/global-params',
        label: '全局参数',
        description: '跨 Pipeline 共享参数管理',
        category: '流水线配置',
        enabled: true,
      },
      {
        key: '/pipeline/env-profiles',
        label: '环境配置',
        description: '环境变量与 Profile 管理',
        category: '流水线配置',
        enabled: true,
      },
      {
        key: '/pipeline/script-versions',
        label: '脚本版本',
        description: '脚本内容版本追踪与对比',
        category: '流水线配置',
        enabled: true,
      },
      {
        key: '/pipeline/audit-logs',
        label: '审计日志',
        description: 'Pipeline 执行审计轨迹',
        category: '流水线配置',
        enabled: true,
      },
      // 代码与制品
  ...
  '/observability': {
    ...
    children: [
      ...
      {
        key: '/observability/self-healing',
        label: '自愈系统',
        description: '自动化故障恢复',
        category: '智能诊断',
        enabled: true,
      ...
      // 混沌工程
      {
        key: '/chaos-experiments',
        label: '混沌实验',
        description: '故障注入实验管理',
        category: '混沌工程',
        enabled: true,
      },
      {
        key: '/chaos-runs',
        label: '运行记录',
        description: '实验运行历史与时间线',
        category: '混沌工程',
        enabled: true,
      },
      {
        key: '/resilience-score',
        label: '弹性评分',
        description: '系统弹性评估与趋势',
        category: '混沌工程',
        enabled: true,
      },
      {
      ...
      // APM 链路追踪
      {
        key: '/apm/dashboard',
        label: 'APM 仪表盘',
        description: '应用性能监控总览',
        category: 'APM',
        enabled: true,
      },
      {
        key: '/apm/slow-requests',
        label: '慢请求分析',
        description: '慢请求追踪与优化',
        category: 'APM',
        enabled: true,
      },
      {
        key: '/apm/error-tracking',
        label: '错误追踪',
        description: '异常与错误聚合分析',
        category: 'APM',
        enabled: true,
      },
      {
        key: '/apm/service-topology',
        label: '服务拓扑',
        description: '服务依赖拓扑图',
        category: 'APM',
        enabled: true,
      ...
      // 服务治理 (Task 6.5 + 6.8)
      {
        key: '/observability/service-registry',
        label: '服务注册中心',
        description: '微服务注册与发现管理',
        category: '服务治理',
        enabled: true,
      },
      {
        key: '/observability/health-dashboard',
        label: '健康仪表盘',
        description: '系统健康状态总览',
        category: '服务治理',
        enabled: true,
      },
    ],
  ...
  '/ai': {
    ...
    children: [
      ...
      {
        key: '/vector-store/index-management',
        label: '索引管理',
        description: '向量索引配置与优化',
        category: '知识管理',
        enabled: true,
      },
      {
        key: '/vector-store/auto-vectorize',
        label: '自动向量化',
        description: '文档自动向量化规则',
        category: '知识管理',
        enabled: true,
      },
      {
  ...
  '/infra': {
    ...
    children: [
      ...
      },
      {
        key: '/cmdb/topology',
        label: '拓扑图',
        description: 'CI 关系可视化',
        category: 'CMDB',
        enabled: true,
      },
      {
        key: '/cmdb/integration',
        label: '集成资源',
        description: '主机 + K8s 资源',
        category: 'CMDB',
        enabled: true,
      },
      {
      ...
      // 多云管理
      {
        key: '/multi-cloud',
        label: '多云管理',
        description: '多云资源统一管理',
        category: '多云管理',
        enabled: true,
      },
      {
        key: '/multi-cloud/advanced',
        label: '高级多云',
        description: '多云高级配置与同步',
        category: '多云管理',
        enabled: true,
      },
      {
    ...
    children: [
      {
        key: '/efficacy-metrics',
        label: '度量总览',
        description: '六域整体评分与趋势',
        category: '总览',
        enabled: true,
      },
      {
        key: '/efficacy-metrics/e2e',
        label: '端到端链路',
        description: 'Commit→Prod 全链路周期',
        category: '端到端',
        enabled: true,
      },
      {
        key: '/efficacy-metrics/management',
        label: '管理域',
        description: '团队/产品线效能对标',
        category: '管理域',
        enabled: true,
      },
      {
        key: '/efficacy-metrics/engineering',
        label: '工程域',
        description: 'DORA + 工程效率深度',
        category: '工程域',
        enabled: true,
      },
      {
        key: '/efficacy-metrics/compliance',
        label: '合规域',
        description: '合规率与SLA达成度量',
        category: '合规域',
        enabled: true,
      },
      {
        key: '/efficacy-metrics/ai-efficiency',
        label: 'AI智研提效',
        description: 'AI辅助研发效能度量',
        category: 'AI提效',
        enabled: true,
      },
      {
        key: '/efficacy-metrics/risk',
        label: '风险看板',
        description: '风险+技术债务+质量门禁',
        category: '风险域',
        enabled: true,
      },
    ],
...
export const useMenuConfigStore = create<MenuConfigState>((set, get) => ({
      ...
      if (stored) {
        ...
        for (const [key, storedModule] of Object.entries(parsed)) {
          ...
          if (defModule && storedModule) {
            // 迁移旧路径
            const migratedChildren = ((storedModule.children as MenuChildConfig[]) || []).map(
              (c) => ({
                ...c,
            ...
            const userAddedChildren = migratedChildren.filter((c) => !defaultKeys.has(c.key));
  ...
  moveChild: (fromModuleKey, toModuleKey, childKey, index) => {
    set((state) => {
      ...
      const insertIndex =
        index !== undefined ? Math.min(index, newToChildren.length) : newToChildren.length;
...
export const getVisibleChildren = (moduleKey: string): MenuChildConfig[] => {
  ...
  const userRoles =
    currentUser?.roles || currentUser?.role ? currentUser?.roles || [currentUser?.role || ''] : [];
