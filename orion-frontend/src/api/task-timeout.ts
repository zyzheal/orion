...
/**
 * 超时任务信息（来自后端 GET /task-timeouts/timed-out）
 */
...
/**
 * 手动触发检查响应（来自后端 POST /task-timeouts/check-now）
 */
...
/**
 * 检查器状态（来自后端 GET /task-timeouts/status）
 */
...
export async function getTimedOutTasks(): Promise<TimedOutTask[]> {
  const res = await api.get('/task-timeouts/timed-out');
...
export async function triggerCheckNow(): Promise<CheckNowResult> {
  const res = await api.post('/task-timeouts/check-now');
...
export async function getTimeoutStatus(): Promise<TimeoutStatus> {
  const res = await api.get('/task-timeouts/status');
