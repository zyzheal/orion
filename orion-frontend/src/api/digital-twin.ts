...
export const digitalTwinApi = {
  ...
  listTwins: async (params?: { environment?: string; status?: string }) => {
    const response = await apiClient.get('/digital-twins', { params });
    return response.data;
  },

  getTwin: async (twinId: string) => {
    const response = await apiClient.get(`/digital-twins/${twinId}`);
    return response.data as DigitalTwin;
  },

  registerTwin: async (data: { name: string; environment: string }) => {
    const response = await apiClient.post('/digital-twins', data);
    return response.data as DigitalTwin;
  },

  // Snapshots
  listSnapshots: async (params?: { environment?: string; status?: string }) => {
    const response = await apiClient.get('/digital-twin/snapshots', { params });
    return response.data;
  },

  getSnapshot: async (snapshotId: string) => {
    const response = await apiClient.get(`/digital-twin/snapshots/${snapshotId}`);
    return response.data as TwinSnapshot;
  },

  createSnapshot: async (data: { environment: string; note?: string }) => {
    const response = await apiClient.post('/digital-twin/snapshots', data);
    return response.data;
  },

  deleteSnapshot: async (snapshotId: string) => {
    const response = await apiClient.delete(`/digital-twin/snapshots/${snapshotId}`);
    return response.data;
  },

  exportSnapshot: async (snapshotId: string) => {
    const response = await apiClient.get(`/digital-twin/snapshots/${snapshotId}/export`);
    return response.data;
  },

  // Traffic Recording
  startRecording: async (data: { source_env: string; path_prefixes?: string[] }) => {
    const response = await apiClient.post('/digital-twin/traffic/recording', data);
    return response.data as TrafficRecording;
  },

  getRecording: async (recordingId: string) => {
    const response = await apiClient.get(`/digital-twin/traffic/recording/${recordingId}`);
    return response.data as TrafficRecording;
  },

  stopRecording: async (recordingId: string) => {
    const response = await apiClient.post(
      `/digital-twin/traffic/recording/${recordingId}/stop`
    );
    return response.data;
  },

  listRecordings: async (params?: { source_env?: string }) => {
    const response = await apiClient.get('/digital-twin/traffic/recordings', { params });
    return response.data;
  },
  ...
  startReplay: async (data: {
    ...
    const response = await apiClient.post('/digital-twin/traffic/replay', data);
    return response.data as TrafficReplay;
  },

  getReplay: async (replayId: string) => {
    const response = await apiClient.get(`/digital-twin/traffic/replay/${replayId}`);
    return response.data as TrafficReplay;
  },

  getReplayReport: async (replayId: string) => {
    const response = await apiClient.get(`/digital-twin/traffic/replay/${replayId}/report`);
    return response.data;
  },

  listReplays: async (params?: { target_env?: string }) => {
    const response = await apiClient.get('/digital-twin/traffic/replays', { params });
    return response.data;
  },

  // Sandbox
  createSandbox: async (data: { name: string; snapshot_id?: string; description?: string }) => {
    const response = await apiClient.post('/digital-twins/sandbox', data);
