...
export function getExecutiveDashboard(params?: { days?: number }) {
  return api.get<ExecutiveDashboardData>('/api/tickets/bi/dashboard/executive', { params });
}
...
export function getManagerDashboard(params?: { teamId?: string; days?: number }) {
  return api.get<ManagerDashboardData>('/api/tickets/bi/dashboard/manager', { params });
}
...
export function getEngineerDashboard(engineerId: string, params?: { days?: number }) {
  return api.get<EngineerDashboardData>(`/api/tickets/bi/dashboard/engineer/${engineerId}`, {
    params,
  });
}
