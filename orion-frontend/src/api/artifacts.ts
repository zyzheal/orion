...
export function getArtifacts(params?: ArtifactListParams) {
  return api.get<Artifact[]>('/v1/artifacts', { params });
}

export function getArtifact(id: string) {
  return api.get<Artifact>(`/v1/artifacts/${id}`);
}

export function createArtifact(data: CreateArtifactInput) {
  return api.post<Artifact>('/v1/artifacts', data);
}

export function updateArtifact(id: string, data: UpdateArtifactInput) {
  return api.put<Artifact>(`/v1/artifacts/${id}`, data);
}
...
export function searchArtifacts(query: string, params?: { type?: string; namespace?: string }) {
  return api.get<Artifact[]>('/v1/artifacts/search', { params: { q: query, ...params } });
}

// ---- Tags ----

export function getArtifactTags(id: string) {
  return api.get<Tag[]>(`/v1/artifacts/${id}/tags`);
}

export function addArtifactTags(id: string, tags: string[]) {
  return api.post<Tag[]>(`/v1/artifacts/${id}/tags`, { tags });
}

export function removeArtifactTags(id: string, tags: string[]) {
  return api.delete(`/v1/artifacts/${id}/tags`, { data: { tags } });
}

// ---- Download ----

export function downloadArtifact(id: string) {
  return api.get<{ url: string }>(`/v1/artifacts/${id}/download`);
}

export function getDownloadHistory(id: string) {
  return api.get<DownloadRecord[]>(`/v1/artifacts/${id}/downloads`);
}
...
export function getCurrentStage(id: string) {
  return api.get<{ stage: ArtifactStage }>(`/v1/artifacts/${id}/stage`);
}

export function getPromotionHistory(id: string) {
  return api.get<PromotionRecord[]>(`/v1/artifacts/${id}/history`);
}

// ---- Lifecycle ----

export function deprecateArtifact(id: string) {
  return api.post(`/v1/artifacts/${id}/deprecate`);
}

export function quarantineArtifact(id: string) {
  return api.post(`/v1/artifacts/${id}/quarantine`);
}

// ---- Stats ----

export function getArtifactStats() {
  return api.get<ArtifactStats>('/v1/artifacts/stats');
}

export function getArtifactTypeStats() {
  return api.get<ArtifactTypeStats>('/v1/artifacts/types');
}

export function getNamespaces() {
  return api.get<string[]>('/v1/artifacts/namespaces');
}
