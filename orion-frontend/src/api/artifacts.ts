...
export function getArtifacts(params?: ArtifactListParams) {
  return api.get<Artifact[]>('/artifacts', { params });
}
...
export function createArtifact(data: CreateArtifactInput) {
  return api.post<Artifact>('/artifacts', data);
}

export function updateArtifact(id: string, data: UpdateArtifactInput) {
  return api.put<Artifact>(`/artifacts/${id}`, data);
}
...
export function searchArtifacts(query: string, params?: { type?: string; namespace?: string }) {
  return api.get<Artifact[]>('/artifacts/search', { params: { q: query, ...params } });
}

// ---- Tags ----

export function getArtifactTags(id: string) {
  return api.get<Tag[]>(`/artifacts/${id}/tags`);
}

export function addArtifactTags(id: string, tags: string[]) {
  return api.post<Tag[]>(`/artifacts/${id}/tags`, { tags });
}

export function removeArtifactTags(id: string, tags: string[]) {
  return api.delete(`/artifacts/${id}/tags`, { data: { tags } });
}

// ---- Download ----

export function downloadArtifact(id: string) {
  return api.get<{ url: string }>(`/artifacts/${id}/download`);
}

export function getDownloadHistory(id: string) {
  return api.get<DownloadRecord[]>(`/artifacts/${id}/downloads`);
}
...
export function promoteArtifact(
  ...
  return api.post<PromotionRecord>(`/artifacts/${id}/promote`, data);
}

export function getCurrentStage(id: string) {
  return api.get<{ stage: ArtifactStage }>(`/artifacts/${id}/stage`);
}

export function getPromotionHistory(id: string) {
  return api.get<PromotionRecord[]>(`/artifacts/${id}/history`);
}

// ---- Lifecycle ----

export function deprecateArtifact(id: string) {
  return api.post(`/artifacts/${id}/deprecate`);
}

export function quarantineArtifact(id: string) {
  return api.post(`/artifacts/${id}/quarantine`);
}
...
export function getArtifactTypeStats() {
  return api.get<ArtifactTypeStats>('/artifacts/types');
}

export function getNamespaces() {
  return api.get<string[]>('/artifacts/namespaces');
}
