...
export const capabilityApi = {
  // List capabilities
  list: (category?: string) => api.get('/capabilities', { params: { category } }),

  // Get capability tree
  getTree: () => api.get('/capabilities/tree'),
  ...
  // Delete capability
  delete: (id: string) => api.delete(`/capabilities/${id}`),
  ...
  mapCommand: (data: {
    ...
    capability_id: string;
    environment_suffix?: string;
  }) => api.post('/capabilities/commands/mapping', data),
  ...
  checkPermission: (userId: string, userRoles: string[], capabilityId: string) =>
    api.post('/capabilities/check', { userId, userRoles, capabilityId }),
  ...
  grantTemporary: (data: {
    ...
    reason?: string;
    expires_in_hours: number;
  }) => api.post('/capabilities/temporary', data),
  ...
  getAuditLogs: (params?: {
    ...
    limit?: number;
    offset?: number;
  }) => api.get('/capabilities/audit', { params }),
  ...
  requestPermission: (data: {
    ...
    user_id?: string;
    tenant_id?: string;
  }) => api.post('/capabilities/request', data),
  ...
  cleanup: () => api.post('/capabilities/cleanup'),

  // ==================== 用户有效能力 ====================

  // 获取用户有效能力（角色 + 覆盖）
  getEffectiveCapabilities: (params?: { user_id?: string; roles?: string }) =>
    api.get('/capabilities/user/effective', { params }),
  ...
  updateRoleCapabilities: (data: UpdateRoleCapabilitiesInput) =>
    api.put('/capabilities/roles', data),

  // 获取权限矩阵（全部角色）
  getCapabilityMatrix: () => api.get('/capabilities/matrix'),
  ...
  grantPermission: (data: {
    ...
    environment_suffix?: string;
    tenant_id?: string;
  }) => api.post('/capabilities/grant', data),
  ...
  submitPermissionRequest: (data: RequestPermissionInput) =>
    api.post('/capabilities/request/permission', data),
