...
export const capabilityApi = {
  // List capabilities
  list: (category?: string) => api.get('/api/capabilities', { params: { category } }),

  // Get capability tree
  getTree: () => api.get('/api/capabilities/tree'),
  ...
  create: (data: {
    ...
    requires_approval?: boolean;
    approval_role?: string;
  }) => api.post('/api/capabilities', data),
  ...
  mapCommand: (data: {
    ...
    capability_id: string;
    environment_suffix?: string;
  }) => api.post('/api/capabilities/commands/mapping', data),

  // Get capability for command
  getCommandCapability: (command: string, action: string, environment?: string) =>
    api.get(`/capabilities/commands/${command}/actions/${action}`, { params: { environment } }),

  // Check permission
  checkPermission: (userId: string, userRoles: string[], capabilityId: string) =>
    api.post('/api/capabilities/check', { userId, userRoles, capabilityId }),
  ...
  grantTemporary: (data: {
    ...
    reason?: string;
    expires_in_hours: number;
  }) => api.post('/api/capabilities/temporary', data),

  // Get user's active temporary permissions
  getUserTemporaryPermissions: (userId: string, tenantId?: string) =>
    api.get(`/capabilities/temporary/${userId}`, { params: { tenant_id: tenantId } }),

  // Revoke temporary permission
  revokeTemporary: (id: number, reason?: string) =>
    api.delete(`/capabilities/temporary/${id}`, { data: { reason } }),
  ...
  getAuditLogs: (params?: {
    ...
    limit?: number;
    offset?: number;
  }) => api.get('/api/capabilities/audit', { params }),
  ...
  requestPermission: (data: {
    ...
    user_id?: string;
    tenant_id?: string;
  }) => api.post('/api/capabilities/request', data),
  ...
  cleanup: () => api.post('/api/capabilities/cleanup'),

  // ==================== 用户有效能力 ====================

  // 获取用户有效能力（角色 + 覆盖）
  getEffectiveCapabilities: (params?: { user_id?: string; roles?: string }) =>
    api.get('/api/capabilities/user/effective', { params }),

  // ==================== 角色能力管理 ====================

  // 获取角色的能力列表
  getRoleCapabilities: (roleId: string) => api.get(`/capabilities/roles/${roleId}`),

  // 批量更新角色能力
  updateRoleCapabilities: (data: UpdateRoleCapabilitiesInput) =>
    api.put('/api/capabilities/roles', data),

  // 获取权限矩阵（全部角色）
  getCapabilityMatrix: () => api.get('/api/capabilities/matrix'),

  // ==================== 用户能力覆盖 ====================

  // 获取用户能力覆盖列表
  getUserOverrides: (userId: string, params?: { tenant_id?: string }) =>
    api.get(`/capabilities/users/${userId}/overrides`, { params }),
  ...
  ) => api.post(`/capabilities/users/${userId}/overrides`, data),

  // 删除用户能力覆盖
  removeUserOverride: (userId: string, capabilityId: string) =>
    api.delete(`/capabilities/users/${userId}/overrides/${capabilityId}`),
  ...
  grantPermission: (data: {
    ...
    environment_suffix?: string;
    tenant_id?: string;
  }) => api.post('/api/capabilities/grant', data),
  ...
  submitPermissionRequest: (data: RequestPermissionInput) =>
    api.post('/api/capabilities/request/permission', data),
  ...
  approveRequest: (ticketId: number, tenantId?: string) =>
    api.post(`/capabilities/request/${ticketId}/approve`, { tenant_id: tenantId }),

  // 拒绝权限申请
  rejectRequest: (ticketId: number, reason?: string) =>
    api.post(`/capabilities/request/${ticketId}/reject`, { reason }),
};
