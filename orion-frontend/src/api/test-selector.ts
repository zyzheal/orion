...
// GET /test-selector/cases - 获取测试用例列表
export async function getTestCases(filters?: { suite?: string; status?: string }) {
  ...
  return api.get<TestCase[]>(`/test-selector/cases${qs ? '?' + qs : ''}`);
}

// GET /test-selector/suites - 获取测试套件列表
export async function getTestSuites() {
  return api.get<TestSuite[]>('/test-selector/suites');
}

// GET /test-selector/coverage - 获取测试覆盖率
export async function getTestCoverage() {
  return api.get<Record<string, number>>('/test-selector/coverage');
}
...
// GET /test-selector/flaky - 获取抖动测试
export async function getFlakyTests(threshold?: number) {
  const params = threshold ? `?threshold=${threshold}` : '';
  return api.get<{ flakyTests: FlakyTest[]; threshold: number }>(
    `/test-selector/flaky${params}`
  );
}

// GET /test-selector/history - 获取测试历史
export async function getTestHistory() {
  return api.get<TestHistoryEntry[]>('/test-selector/history');
}

// 兼容旧接口
export async function getTestStats() {
  const [casesRes, suitesRes] = await Promise.all([
    api.get<TestCase[]>('/test-selector/cases'),
    api.get<TestSuite[]>('/test-selector/suites'),
  ]);
...
export async function runTests(testIds: string[]) {
  return api.post<{ runId: string }>('/test-selector/run', { testIds });
}
