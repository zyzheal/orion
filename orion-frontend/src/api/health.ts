...
export function getHealthDashboard(): Promise<HealthDashboardData> {
  return api.get<HealthDashboardData>('/v1/health/dashboard').then((res) => res.data);
}

export function getHealthScore(): Promise<HealthScore> {
  return api.get<HealthScore>('/v1/health/score').then((res) => res.data);
}

export function getServiceHealthList(): Promise<ServiceHealthRow[]> {
  return api.get<ServiceHealthRow[]>('/v1/health/services').then((res) => res.data);
}

export function getHealthAlerts(params?: { status?: string; limit?: number }): Promise<HealthAlert[]> {
  return api.get<HealthAlert[]>('/v1/health/alerts', { params }).then((res) => res.data);
}

export function getHealthTrend(since?: string): Promise<TrendPoint[]> {
  return api.get<TrendPoint[]>('/v1/health/trend', { params: { since } }).then((res) => res.data);
}
