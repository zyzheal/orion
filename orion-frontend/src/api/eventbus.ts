...
export async function getEventBusStatus() {
  return api.get<EventBusStatus>('/v1/eventbus/status');
}
...
export async function getStats() {
  return api.get<{ stats: Record<string, number> }>('/v1/eventbus/stats');
}

export async function getJetStreamMetrics() {
  return api.get<JetStreamMetrics>('/v1/eventbus/jetstream/metrics');
}
...
export async function getStreamConsumers(streamName: string) {
  return api.get<{ stream: string; consumers: Array<{ name: string; pending: number }> }>(
    `/v1/eventbus/jetstream/streams/${streamName}/consumers`
  );
}

export async function getDLQEvents(limit?: number) {
  const qs = limit ? `?limit=${limit}` : '';
  return api.get<DLQEvent>(`/v1/eventbus/dlq${qs}`);
}
