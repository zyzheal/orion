...
export async function getEventBusStatus() {
  return api.get<EventBusStatus>('/eventbus/status');
}

export async function connectEventBus() {
  return api.post<void>('/eventbus/connect');
}
...
export async function publishEvent(
  ...
  return api.post<void>('/eventbus/publish', { subject, data, tenantId, publishedBy });
}
...
export async function getEvents(options?: { eventType?: string; status?: string; limit?: number }) {
  ...
  return api.get<EventBusEvent[]>(`/eventbus/events${qs ? '?' + qs : ''}`);
}

export async function getSubscriptions(tenantId?: string) {
  const qs = tenantId ? `?tenantId=${tenantId}` : '';
  return api.get<EventBusSubscription[]>(`/eventbus/subscriptions${qs}`);
}

export async function getStats() {
  return api.get<{ stats: Record<string, number> }>('/eventbus/stats');
}

export async function getJetStreamMetrics() {
  return api.get<JetStreamMetrics>('/eventbus/jetstream/metrics');
}
...
export async function getStreamConsumers(streamName: string) {
  return api.get<{ stream: string; consumers: Array<{ name: string; pending: number }> }>(
    `/eventbus/jetstream/streams/${streamName}/consumers`
  );
}

export async function getDLQEvents(limit?: number) {
  const qs = limit ? `?limit=${limit}` : '';
  return api.get<DLQEvent>(`/eventbus/dlq${qs}`);
}
