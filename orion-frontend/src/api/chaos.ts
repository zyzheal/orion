...
export const chaosApi = {
  listExperiments: async (params?: { status?: string }) => {
    const response = await apiClient.get('/v1/chaos/experiments', { params });
    return response.data;
  },

  getExperiment: async (experimentId: string) => {
    const response = await apiClient.get(`/v1/chaos/experiments/${experimentId}`);
  ...
  createExperiment: async (data: { name: string; scope: Record<string, unknown>; faults: ChaosFault[] }) => {
    const response = await apiClient.post('/v1/chaos/experiments', data);
    return response.data as ChaosExperiment;
  },

  runExperiment: async (experimentId: string, dryRun?: boolean) => {
    const response = await apiClient.post(`/v1/chaos/experiments/${experimentId}/run`, { dry_run: dryRun });
  ...
  rollbackRun: async (runId: string, reason?: string) => {
    const response = await apiClient.post(`/v1/chaos/runs/${runId}/rollback`, { reason });
    return response.data;
  },
};
...
export const resilienceApi = {
  getScore: async (params?: { serviceId?: string }) => {
    const response = await apiClient.get('/v1/resilience-score', { params });
    return response.data as ResilienceScore;
  },

  getHistory: async (params?: { serviceId?: string; days?: number }) => {
    const response = await apiClient.get('/v1/resilience-score/history', { params });
    return response.data;
  },
};
