...
export function explainDecision(data: ExplainRequest) {
  return api.post<DecisionExplanation>('/ai-decisions/explain', data);
}

export function getFeatureImportance(decisionId: string, features?: string) {
  return api.get<FeatureImportance>(`/ai-decisions/${decisionId}/feature-importance`, {
    params: { features },
  });
}

export function getConfidenceExplanation(level: string, score?: number) {
  return api.get<ConfidenceExplanation>(`/ai-decisions/confidence/${level}`, {
    params: { score },
  });
}
...
export function listModels(params?: { status?: string; framework?: string; name?: string }) {
  return api.get<{ models: ModelVersion[] }>('/ai-models', { params });
}

export function getModel(modelId: string) {
  return api.get<ModelVersion>(`/ai-models/${modelId}`);
}
...
export function registerModel(data: {
  ...
  return api.post<ModelVersion>('/ai-models', data);
}

export function activateModel(modelId: string) {
  return api.post(`/ai-models/${modelId}/activate`);
}

export function deprecateModel(modelId: string) {
  return api.post(`/ai-models/${modelId}/deprecate`);
}

export function getModelVersions(modelName: string, includeDeprecated?: boolean) {
  return api.get<{ versions: ModelVersion[] }>(`/ai-models/${modelName}/versions`, {
    params: { include_deprecated: includeDeprecated },
  });
}

export function getABTestResults(modelName: string) {
  return api.get<ABTestResult>(`/ai-models/${modelName}/ab-test`);
}

export function getModelPerformance(modelName: string) {
  return api.get<ModelPerformance>(`/ai-models/${modelName}/performance`);
}
...
export function getExplanationById(decisionId: string) {
  return api.get<ExplanationWithRules>(`/ai-decisions/explanations/${decisionId}`);
}

export function getExplanationHistory(params?: { limit?: number; decisionType?: string }) {
  return api.get<ExplanationWithRules[]>('/ai-decisions/explanations/history', { params });
}

// ---- Model Rollback API ----

export function rollbackModel(modelId: string, targetVersion?: string) {
  return api.post(`/ai-models/${modelId}/rollback`, { targetVersion });
}
