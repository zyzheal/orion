 ...
 * - POST /feature-flags/:id/evaluate - evaluate flag for tenant/user
 * - POST /feature-flags/:id/toggle - enable/disable flag
 */
...
export async function getFeatureFlags(params?: { tenantId?: string; enabled?: boolean }) {
  return api.get<FeatureFlag[]>('/feature-flags', { params });
}

export async function createFeatureFlag(
  data: Omit<FeatureFlag, 'id' | 'createdAt' | 'updatedAt' | 'evaluationCount'>
) {
  return api.post<FeatureFlag>('/feature-flags', data);
}

export async function updateFeatureFlag(id: string, data: Partial<FeatureFlag>) {
  return api.put<FeatureFlag>(`/feature-flags/${id}`, data);
}

export async function deleteFeatureFlag(id: string) {
  return api.delete<void>(`/feature-flags/${id}`);
}

export async function toggleFeatureFlag(id: string, enabled: boolean) {
  return api.post<FeatureFlag>(`/feature-flags/${id}/toggle`, { enabled });
}
...
export async function evaluateFeatureFlag(
  ...
  return api.post<FlagEvaluationResult>(`/feature-flags/${id}/evaluate`, context);
}

export async function getFeatureFlagStats() {
  return api.get<FeatureFlagStats>('/feature-flags/stats');
}
