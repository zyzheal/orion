...
/**
 * 获取 SPI 统计信息
 * GET /plugins-spi/stats
 */
export async function getSPIStats() {
  const res = await api.get<SPIStats>('/plugins-spi/stats');
  return res.data;
}

/**
 * 获取插件列表（按能力分组）
 * GET /plugins-spi/plugins
 */
export async function getExtensionPoints() {
  const res = await api.get<PluginInfo[]>('/plugins-spi/plugins');
...
export async function createRegistration(input: Omit<PluginRegistration, 'id' | 'createdAt'>) {
  ...
  const res = await api.post<PluginInfo>('/plugins-spi/plugins', {
    manifest,
    config: input.config,
  });
  return res.data;
...
export async function deleteRegistration(id: string) {
  await api.delete(`/plugins-spi/plugins/${id}`);
}
...
export async function getPluginDetails(pluginName: string) {
  const res = await api.get<PluginInfo>(`/plugins-spi/plugins/${pluginName}`);
  return res.data;
}
...
export async function updatePluginConfig(pluginName: string, config: Record<string, unknown>) {
  const res = await api.put<PluginInfo>(`/plugins-spi/plugins/${pluginName}/config`, {
    config,
  });
  return res.data;
}
...
export async function enablePlugin(pluginName: string) {
  const res = await api.post<PluginInfo>(`/plugins-spi/plugins/${pluginName}/enable`);
  return res.data;
}
...
export async function disablePlugin(pluginName: string) {
  const res = await api.post<PluginInfo>(`/plugins-spi/plugins/${pluginName}/disable`);
  return res.data;
}
...
export async function uninstallPlugin(pluginName: string) {
  const res = await api.post<PluginInfo>(`/plugins-spi/plugins/${pluginName}/uninstall`);
  return res.data;
}
...
export async function getPluginHealth(pluginName: string) {
  const res = await api.get<{ status: 'healthy' | 'unhealthy'; message?: string }>(
    `/plugins-spi/plugins/${pluginName}/health`
  );
  return res.data;
}
...
export async function executePlugin(
  ...
  const res = await api.post<PluginExecutionResult>(
    `/plugins-spi/plugins/${pluginName}/execute`,
    { input, timeout }
  );
  return res.data;
}
