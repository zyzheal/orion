...
export const createTicketFromAlert = (data: {
  ...
  tags?: Record<string, string>;
  ruleName?: string;
}) => api.post<Ticket>('/tickets/from-alert', data);
...
) => api.post(`/tickets/${id}/assign`, typeof data === 'string' ? { assignee: data } : data);
...
) => api.post(`/tickets/${id}/close`, typeof data === 'string' ? { reason: data } : data);

export const getWorkflowHistory = (ticketId: string) =>
  api.get<{ items: WorkflowHistoryEntry[] }>(`/tickets/${ticketId}/history`);

// ==================== Assignment Rules ====================

export const addAssignmentRule = (rule: Omit<AssignmentRule, 'id'>) =>
  api.post<AssignmentRule>('/ticketing/rules', rule);

export const getAssignmentRules = () =>
  api.get<{ items: AssignmentRule[] }>('/ticketing/rules');

export const removeAssignmentRule = (ruleId: string) =>
  api.delete(`/ticketing/rules/${ruleId}`);
...
export const getRelations = (ticketId: string) =>
  api.get<{ items: TicketRelation[] }>(`/tickets/${ticketId}/relations`);
...
) => api.get<{ items: TicketRelation[] }>(`/tickets/${ticketId}/related`, { params });

export const detectDuplicates = (ticketId: string, params?: { threshold?: number }) =>
  api.get<{ items: TicketRelation[] }>(`/tickets/${ticketId}/duplicates`, { params });

export const correlateRootCause = (ticketIds: string[]) =>
  api.post('/tickets/correlate', { ticketIds });

// ==================== Comments & Attachments ====================

export const getComments = (ticketId: string) =>
  api.get<{ items: unknown[] }>(`/tickets/${ticketId}/comments`);

export const createComment = (ticketId: string, data: { content: string; type?: 'comment' | 'internal-note' }) =>
  api.post<unknown>(`/tickets/${ticketId}/comments`, data);

export const getAttachments = (ticketId: string) =>
  api.get<{ items: unknown[] }>(`/tickets/${ticketId}/attachments`);

// ==================== SLA ====================

export const addSLATarget = (target: SLATarget) => api.post('/ticketing/sla', target);

export const getTicketSLA = (ticketId: string) => api.get(`/tickets/${ticketId}/sla`);

// ==================== Reports ====================

export const getSLACompliance = (params?: { periodStart?: string; periodEnd?: string }) =>
  api.get('/tickets/reports/sla', { params });

export const getResolutionStats = () => api.get('/tickets/reports/resolution');

export const getBacklogAnalysis = () => api.get('/tickets/reports/backlog');

export const getTrendReport = (params?: { days?: number; granularity?: string }) =>
  api.get('/tickets/reports/trends', { params });

export const getStatistics = () => api.get('/tickets/reports/statistics');

// ==================== Dispatch ====================

export const registerEngineer = (profile: Omit<EngineerProfile, 'id'>) =>
  api.post<EngineerProfile>('/tickets/dispatch/engineers', profile);

export const listEngineers = () =>
  api.get<{ items: EngineerProfile[] }>('/tickets/dispatch/engineers');

export const getEngineer = (engineerId: string) =>
  api.get<EngineerProfile>(`/tickets/dispatch/engineers/${engineerId}`);
...
) => api.post<DispatchResult>(`/tickets/dispatch/auto/${ticketId}`, options);

export const manualDispatch = (ticketId: string, engineerId: string, reason?: string) =>
  api.post<DispatchResult>(`/tickets/dispatch/manual/${ticketId}`, { engineerId, reason });

export const getBestMatch = (ticketId: string) =>
  api.get<DispatchResult>(`/tickets/dispatch/best-match/${ticketId}`);

export const calculateDispatchScore = (ticketId: string, engineerId: string) =>
  api.post('/tickets/dispatch/score', { ticketId, engineerId });

export const getDispatchQueueStatus = () => api.get('/tickets/dispatch/queue/status');

export const getDispatchQueue = () =>
  api.get<{ entries: unknown[] }>('/tickets/dispatch/queue/entries');

export const getSLAAlerts = (params?: { type?: string; limit?: number }) =>
  api.get('/tickets/dispatch/sla-alerts', { params });

export const addDispatchRule = (rule: Omit<DispatchRule, 'id'>) =>
  api.post<DispatchRule>('/tickets/dispatch/rules', rule);

export const getDispatchRules = () =>
  api.get<{ items: DispatchRule[] }>('/tickets/dispatch/rules');

export const getLoadBalanceReport = () => api.get('/tickets/dispatch/load-balance/report');

export const getReassignmentSuggestions = () =>
  api.get('/tickets/dispatch/load-balance/suggestions');

export const getDispatchMetrics = (params?: { periodStart?: string; periodEnd?: string }) =>
  api.get('/tickets/dispatch/reports/metrics', { params });

export const getAssignmentSuccessMetrics = (params?: {
  periodStart?: string;
  periodEnd?: string;
}) => api.get('/tickets/dispatch/reports/assignment-success', { params });

export const getTimeToAssignmentStats = (params?: { periodStart?: string; periodEnd?: string }) =>
  api.get('/tickets/dispatch/reports/time-to-assignment', { params });

export const getEngineerPerformance = (engineerId: string) =>
  api.get(`/tickets/dispatch/reports/performance/${engineerId}`);

export const getAllEngineerPerformances = () =>
  api.get('/tickets/dispatch/reports/performance');

export const updateDispatchWeights = (weights: Partial<DispatchWeights>) =>
  api.put('/tickets/dispatch/weights', weights);

export const getDispatchWeights = () =>
  api.get<DispatchWeights>('/tickets/dispatch/weights');
...
) => api.post(`/tickets/transfer/${ticketId}`, data);

export const getTransferHistory = (ticketId: string) =>
  api.get<{ items: TransferRecord[] }>(`/tickets/transfer/${ticketId}/history`);

export const getTransferStats = (params?: { periodStart?: string; periodEnd?: string }) =>
  api.get('/tickets/transfer/stats', { params });
...
export const createSuspend = (data: {
  ...
  notes?: string;
  createdBy: string;
}) => api.post<SuspendRecord>('/tickets/suspend', data);

export const activateSuspend = (suspendId: string) =>
  api.post<SuspendRecord>(`/tickets/suspend/${suspendId}/activate`);

export const endSuspend = (suspendId: string) =>
  api.post<SuspendRecord>(`/tickets/suspend/${suspendId}/end`);

export const cancelSuspend = (suspendId: string) =>
  api.post<SuspendRecord>(`/tickets/suspend/${suspendId}/cancel`);

export const listSuspensions = (params?: { status?: string }) =>
  api.get<{ items: SuspendRecord[] }>('/tickets/suspend', { params });

export const getSuspend = (suspendId: string) =>
  api.get<SuspendRecord>(`/tickets/suspend/${suspendId}`);

export const getEngineerSuspensions = (engineerId: string) =>
  api.get<{ items: SuspendRecord[] }>(`/tickets/suspend/engineer/${engineerId}`);

export const getEngineerSuspendImpact = (engineerId: string) =>
  api.get<SuspensionImpact>(`/tickets/suspend/engineer/${engineerId}/impact`);

// ==================== BI Analytics ====================

export const getExecutiveDashboard = (params?: { periodStart?: string; periodEnd?: string }) =>
  api.get('/tickets/bi/dashboard/executive', { params });

export const getManagerDashboard = (params?: { periodStart?: string; periodEnd?: string }) =>
  api.get('/tickets/bi/dashboard/manager', { params });

export const getEngineerDashboard = (
  engineerId: string,
  params?: { periodStart?: string; periodEnd?: string }
) => api.get(`/tickets/bi/dashboard/engineer/${engineerId}`, { params });
...
) => api.get(`/tickets/bi/efficiency/${engineerId}`, { params });
...
) => api.get(`/tickets/bi/score/${engineerId}`, { params });
...
export const comparePeriods = (params: {
  ...
  previousEnd: string;
}) => api.get('/tickets/bi/compare', { params });
...
export const exportBIData = (data: {
  ...
  periodStart?: string;
  periodEnd?: string;
}) => api.post('/tickets/bi/export', data);
...
export const getTimeTrend = (params?: {
  ...
  granularity?: string;
}) => api.get('/tickets/bi/trend', { params });

// ==================== Service Control ====================

export const startTicketingService = () => api.post('/ticketing/start');

export const stopTicketingService = () => api.post('/ticketing/stop');

export const getTicketingHealth = () => api.get('/ticketing/health');
