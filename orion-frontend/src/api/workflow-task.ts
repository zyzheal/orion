...
export async function getTasks(params?: TaskListQuery): Promise<TaskListResponse> {
  ...
  const response = await api.get<TaskListResponse>(
    `/v1/workflow-tasks${query.toString() ? `?${query}` : ''}`
  );
...
export async function getTask(id: string): Promise<WorkflowTask> {
  const response = await api.get<TaskResponse>(`/v1/workflow-tasks/${id}`);
...
export async function claimTask(id: string, data?: ClaimTaskInput): Promise<WorkflowTask> {
  const response = await api.post<TaskResponse>(`/v1/workflow-tasks/${id}/claim`, data);
  return response.data.data;
}
...
export async function completeTask(
  ...
  const response = await api.post<TaskResponse>(`/v1/workflow-tasks/${id}/complete`, data);
  return {
