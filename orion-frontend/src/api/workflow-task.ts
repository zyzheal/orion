 ...
 * 工作流人工任务 API 客户端
 *
 * Backend routes: /api/v1/workflow-tasks
 * - GET /api/v1/workflow-tasks - 任务列表
 * - GET /api/v1/workflow-tasks/:id - 任务详情
 * - POST /api/v1/workflow-tasks/:id/claim - 认领任务
...
export async function getTasks(params?: TaskListQuery): Promise<TaskListResponse> {
  ...
  const res = await api.get(`/api/v1/workflow-tasks${query.toString() ? `?${query}` : ''}`);
...
export async function getTask(id: string): Promise<WorkflowTask> {
  const res = await api.get(`/api/v1/workflow-tasks/${id}`);
...
export async function claimTask(id: string, data?: ClaimTaskInput): Promise<WorkflowTask> {
  const res = await api.post(`/api/v1/workflow-tasks/${id}/claim`, data);
  return (res.data as unknown as TaskResponse).data;
}
...
export async function completeTask(
  ...
  const res = await api.post(`/api/v1/workflow-tasks/${id}/complete`, data);
