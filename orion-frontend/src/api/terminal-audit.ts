...
export const getConnectLogs = async (params?: ConnectLogListParams) => {
  const response = await apiClient.get('/cmdb/terminal-audit/connect-logs', { params });
...
export const getConnectLog = async (id: string) => {
  const response = await apiClient.get(`/cmdb/terminal-audit/connect-logs/${id}`);
...
export const getFileLogs = async (params?: FileLogListParams) => {
  const response = await apiClient.get('/cmdb/terminal-audit/file-logs', { params });
  return response.data as {
    success: boolean;
...
export const getFileLog = async (id: string) => {
  const response = await apiClient.get(`/cmdb/terminal-audit/file-logs/${id}`);
...
export const getTerminalAuditStats = async () => {
  const response = await apiClient.get('/cmdb/terminal-audit/stats');
