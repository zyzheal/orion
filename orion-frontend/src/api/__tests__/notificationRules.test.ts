...
describe('Notification Rules API', () => {
  ...
  it('should get IM notification rules', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/notifications/im-rules');
  ...
  it('should create an IM notification rule', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/notifications/im-rules', {
      platform: 'wecom',
      name: 'Test Rule',
      webhookUrl: 'https://qyapi.weixin.qq.com/cgi-bin/webhook/send',
      events: ['pipeline.failed'],
  ...
  it('should update an IM notification rule', async () => {
    ...
    expect(api.put).toHaveBeenCalledWith('/notifications/im-rules/1', { name: 'Updated' });
  });
  ...
  it('should delete an IM notification rule', async () => {
    ...
    expect(api.delete).toHaveBeenCalledWith('/notifications/im-rules/1');
  });
  ...
  it('should toggle IM notification rule', async () => {
    ...
    expect(api.put).toHaveBeenCalledWith('/notifications/im-rules/1/toggle', {
      enabled: false,
    });
  ...
  it('should test an IM notification rule', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/notifications/im-rules/1/test');
