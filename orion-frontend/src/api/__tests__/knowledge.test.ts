...
describe('Knowledge API', () => {
  ...
  it('should get a knowledge item', async () => {
    vi.mocked(api.get).mockResolvedValue({ data: { item: { id: '1', title: 'Test' } } });
    const result = await getKnowledge('1');
    expect(api.get).toHaveBeenCalledWith('/v1/knowledge/1');
    expect(result.data.item.title).toBe('Test');
  });

  it('should create a knowledge item', async () => {
    vi.mocked(api.post).mockResolvedValue({ data: { item: { id: '1', title: 'New' } } });
  ...
  it('should update a knowledge item', async () => {
    vi.mocked(api.put).mockResolvedValue({ data: { item: { id: '1' } } });
