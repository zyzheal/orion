...
export function executeCommand(data: ExecCommandInput) {
  return api.post('/api/visor/exec/command', data);
}

export function getCommandLog(id: string) {
  return api.get(`/api/visor/exec/command-log/${id}`);
}

export function getCommandLogDetails(id: string) {
  return api.get(`/api/visor/exec/command-log/${id}/details`);
}

export function listCommandLogs(page = 1, pageSize = 20) {
  return api.get('/api/visor/exec/command-log', { params: { page, pageSize } });
}
...
export function listTemplates() {
  return api.get('/api/visor/exec/template');
}

export function getTemplate(id: string) {
  return api.get(`/api/visor/exec/template/${id}`);
}

export function createTemplate(data: CreateTemplateInput) {
  return api.post('/api/visor/exec/template', data);
}

export function updateTemplate(id: string, data: Partial<CreateTemplateInput>) {
  return api.put(`/api/visor/exec/template/${id}`, data);
}

export function deleteTemplate(id: string) {
  return api.delete(`/api/visor/exec/template/${id}`);
}
...
export function uploadFile(file: File, hostIds: string[], targetPath: string) {
  ...
  return api.post('/api/visor/exec/upload', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
}

export function listUploadTasks() {
  return api.get('/api/visor/exec/upload-task');
}

export function getUploadTask(id: string) {
  return api.get(`/api/visor/exec/upload-task/${id}`);
}

export function cancelUploadTask(id: string) {
  return api.post(`/api/visor/exec/upload-task/${id}/cancel`);
}
