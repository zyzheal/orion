...
export function executeCommand(data: ExecCommandInput) {
  return api.post('/api/v1/visor/exec/command', data);
}

export function getCommandLog(id: string) {
  return api.get(`/api/v1/visor/exec/command-log/${id}`);
}

export function getCommandLogDetails(id: string) {
  return api.get(`/api/v1/visor/exec/command-log/${id}/details`);
}

export function listCommandLogs(page = 1, pageSize = 20) {
  return api.get('/api/v1/visor/exec/command-log', { params: { page, pageSize } });
}
...
export function listTemplates() {
  return api.get('/api/v1/visor/exec/template');
}

export function getTemplate(id: string) {
  return api.get(`/api/v1/visor/exec/template/${id}`);
}

export function createTemplate(data: CreateTemplateInput) {
  return api.post('/api/v1/visor/exec/template', data);
}

export function updateTemplate(id: string, data: Partial<CreateTemplateInput>) {
  return api.put(`/api/v1/visor/exec/template/${id}`, data);
}

export function deleteTemplate(id: string) {
  return api.delete(`/api/v1/visor/exec/template/${id}`);
}
...
export function listCronJobs() {
  return api.get('/api/v1/visor/exec/job');
}

export function getCronJob(id: string) {
  return api.get(`/api/v1/visor/exec/job/${id}`);
}

export function createCronJob(data: CreateCronJobInput) {
  return api.post('/api/v1/visor/exec/job', data);
}

export function updateCronJob(id: string, data: Partial<CreateCronJobInput>) {
  return api.put(`/api/v1/visor/exec/job/${id}`, data);
}

export function deleteCronJob(id: string) {
  return api.delete(`/api/v1/visor/exec/job/${id}`);
}

export function toggleCronJob(id: string, enabled: boolean) {
  return api.patch(`/api/v1/visor/exec/job/${id}/toggle`, { enabled });
}

export function runCronJobNow(id: string) {
  return api.post(`/api/v1/visor/exec/job/${id}/run-now`);
}

export function getCronJobLogs(id: string, page = 1, pageSize = 20) {
  return api.get(`/api/v1/visor/exec/job/${id}/logs`, { params: { page, pageSize } });
}
...
export function uploadFile(file: File, hostIds: string[], targetPath: string) {
  ...
  return api.post('/api/v1/visor/exec/upload', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
}

export function listUploadTasks() {
  return api.get('/api/v1/visor/exec/upload-task');
}

export function getUploadTask(id: string) {
  return api.get(`/api/v1/visor/exec/upload-task/${id}`);
}

export function cancelUploadTask(id: string) {
  return api.post(`/api/v1/visor/exec/upload-task/${id}/cancel`);
}
