...
export function getPipelineVersions(name: string) {
  return api.get(`/pipelines/versions/${name}`);
}

export function validatePipelineYaml(yamlDefinition: string) {
  return api.post('/pipelines/validate', { yamlDefinition });
}
...
export function triggerPipeline(
  ...
  return api.post(`/pipelines/${id}/runs`, data);
}

export function getPipelineRuns(pipelineId: string, params?: PipelineRunListParams) {
  return api.get(`/pipeline-runs`, { params: { pipelineId, ...params } });
}

export function getPipelineRun(runId: string) {
  return api.get(`/pipeline-runs/${runId}`);
}
...
export function retryPipelineRun(runId: string) {
  return api.post(`/pipeline-runs/${runId}/retry`);
}

// ---- Cache Management ----

export function saveCache(runId: string, stageId: string, data: { key: string; paths: string[] }) {
  return api.post(`/pipeline-runs/${runId}/stages/${stageId}/cache`, data);
}

export function restoreCache(runId: string, stageId: string, key: string) {
  return api.get(`/pipeline-runs/${runId}/stages/${stageId}/cache`, { params: { key } });
}

// Note: Cache management is under /build-cache endpoints
export function deleteCache(cacheKey: string) {
  return api.delete(`/build-cache/entries/${cacheKey}`);
}

export function listCaches(params?: { stageName?: string }) {
  return api.get('/build-cache/entries', { params });
}

// ---- Artifact Management ----

export function uploadArtifact(runId: string, stageId: string, data: FormData) {
  return api.post(`/pipeline-runs/${runId}/stages/${stageId}/artifacts`, data, {
...
export function batchUpdatePipelines(ids: string[], action: 'activate' | 'deactivate' | 'delete') {
  return api.post('/pipelines/batch', {
    ids,
    action,
  });
}
 ...
 * Maps to GET /pipelines/:runId/error-detail
 */
export function getPipelineErrorDetail(runId: string) {
  return api.get<{ data: PipelineErrorDetailResponse }>(`/pipelines/${runId}/error-detail`);
}
