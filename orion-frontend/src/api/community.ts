...
export const communityApi = {
  ...
  listContributions: async (params?: { type?: string; status?: string; userId?: string }) => {
    const response = await apiClient.get('/community/contributions', { params });
    return response.data as Contribution[];
  },

  getContribution: async (contributionId: string) => {
    const response = await apiClient.get(`/community/contributions/${contributionId}`);
    return response.data as Contribution;
  },
  ...
  createContribution: async (data: {
    ...
    const response = await apiClient.post('/community/contributions', data);
    return response.data as Contribution;
  },

  // Contributors
  getContributor: async (userId: string) => {
    const response = await apiClient.get(`/community/contributors/${userId}`);
  ...
  submitPlugin: async (data: {
    ...
    const response = await apiClient.post('/community/plugins', data);
    return response.data;
  },

  reviewPlugin: async (pluginId: string, data: { score: number; feedback: string }) => {
    const response = await apiClient.post(`/community/plugins/${pluginId}/review`, data);
  ...
  listMentorship: async (params?: { status?: string }) => {
    const response = await apiClient.get('/community/mentorship', { params });
    return response.data as Mentorship[];
  },

  createMentorship: async (data: { mentor_id: string; mentee_id: string; skill_area: string }) => {
    const response = await apiClient.post('/community/mentorship', data);
    return response.data as Mentorship;
  },
};
