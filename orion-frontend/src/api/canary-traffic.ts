...
export const canaryTrafficApi = {
  createCanaryDeployment: async (data: {
    ...
    const response = await apiClient.post('/canary/deployments', data);
    return response.data as CanaryDeployment;
  },

  listCanaryDeployments: async (params?: { serviceName?: string; status?: string }) => {
    const response = await apiClient.get('/canary/deployments', { params });
    return response.data as CanaryDeployment[];
  },

  getCanaryDeployment: async (id: string) => {
    const response = await apiClient.get(`/canary/deployments/${id}`);
    return response.data as CanaryDeployment;
  },

  configureTrafficSplit: async (id: string, data: TrafficSplitConfig) => {
    const response = await apiClient.put(`/canary/deployments/${id}/traffic`, data);
    return response.data as CanaryDeployment;
  },

  promoteCanary: async (id: string, data?: { reason?: string }) => {
    const response = await apiClient.post(`/canary/deployments/${id}/promote`, data);
    return response.data;
  },

  rollbackCanary: async (id: string, data?: { reason?: string }) => {
    const response = await apiClient.post(`/canary/deployments/${id}/rollback`, data);
    return response.data;
  },

  deleteCanaryDeployment: async (id: string) => {
    const response = await apiClient.delete(`/canary/deployments/${id}`);
    return response.data;
  },
};
