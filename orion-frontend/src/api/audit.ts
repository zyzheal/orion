...
export function getAuditLogs(filters?: AuditLogFilters) {
  return api.get<{ entries: AuditLogEntry[]; total: number }>('/audit/logs', {
    params: filters,
  });
}
...
export function createAuditLog(data: {
  ...
  return api.post<AuditLogEntry>('/audit/logs', data);
}

export function verifyAuditLog(id: string) {
  return api.get<{ entry: AuditLogEntry; isValid: boolean }>(`/audit/logs/${id}/verify`);
}
...
export function getChainInfo() {
  return api.get<ChainInfo>('/audit/chain/info');
}

export function getChainGenesis() {
  return api.get<{ genesisHash: string }>('/audit/chain/genesis');
}

export function getChainLatest() {
  return api.get<AuditLogEntry>('/audit/chain/latest');
}

// ==================== Storage Management ====================

export function getStorageStats() {
  return api.get<StorageStats>('/audit/storage/stats');
}

export function flushStorage() {
  return api.post<{ status: string }>('/audit/storage/flush');
}

// ==================== Reports ====================

export function generateReport() {
  return api.post<{ report: IntegrityReport }>('/audit/report/generate');
}

export function getReports() {
  return api.get<{ reports: IntegrityReport[] }>('/audit/reports');
}
