...
export function classifyError(data: {
  ...
  return api.post<ErrorClassification>('/autonomous/classify-error', data);
}

export function getErrorStats(params?: { pipelineId?: string; days?: number }) {
  return api.get<ErrorStats>('/autonomous/error-stats', { params });
}

// ---- Adaptive Timeout API ----

export function getTimeoutForStage(stageName: string) {
  return api.get<TimeoutConfig>(`/autonomous/timeout/${stageName}`);
}
...
export function recordExecution(data: {
  ...
  return api.post<ExecutionRecord>('/autonomous/record-execution', data);
}

// ---- Auto Retry API ----

export function getRetryStats(pipelineId: string) {
  return api.get<RetryStats>(`/autonomous/retry-stats/${pipelineId}`);
}

export function configureRetry(data: RetryConfig) {
  return api.post<RetryConfig>('/autonomous/configure-retry', data);
}
...
export function recommendSelfHealing(data: {
  ...
  }>('/autonomous/self-healing/recommend', data);
}
