...
export async function getModules() {
  return api.get<ModuleDescriptor[]>('/system/modules');
}

/** Get single module detail */
export async function getModule(id: string) {
  return api.get<ModuleDetailResponse>(`/system/modules/${id}`);
}

/** Enable or disable a module */
export async function toggleModule(id: string, enabled: boolean) {
  return api.put<ToggleModuleResponse>(`/system/modules/${id}/toggle`, { enabled });
}

/** Validate module dependencies */
export async function validateDependencies() {
  return api.get<ValidationResponse>('/system/modules/validate');
}

/** Get module startup order (topological sort) */
export async function getStartupOrder() {
  return api.get<StartupOrderResponse>('/system/modules/startup-order');
}
