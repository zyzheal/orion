...
// POST /env-profiles — 创建
export function createEnvProfile(data: CreateEnvProfileInput) {
  return api.post<EnvProfile>('/env-profiles', data);
}

// GET /env-profiles — 列表查询
export function getEnvProfiles(params?: { name?: string; environment?: string }) {
  return api.get<EnvProfile[]>('/env-profiles', { params });
}

// GET /env-profiles/:id — 获取详情
export function getEnvProfile(id: string) {
  return api.get<EnvProfile>(`/env-profiles/${id}`);
}

// PUT /env-profiles/:id — 更新
export function updateEnvProfile(id: string, data: UpdateEnvProfileInput) {
  return api.put<EnvProfile>(`/env-profiles/${id}`, data);
}

// DELETE /env-profiles/:id — 删除
export function deleteEnvProfile(id: string) {
  return api.delete(`/env-profiles/${id}`);
}

// GET /env-profiles/:name/environments — 列出环境
export function getEnvironmentsForProfile(name: string) {
  return api.get<string[]>(`/env-profiles/${encodeURIComponent(name)}/environments`);
}

// POST /env-profiles/resolve — 解析变量
export function resolveEnvVariables(data: ResolveVariablesInput) {
  return api.post<ResolvedVariablesResult>('/env-profiles/resolve', data);
}
