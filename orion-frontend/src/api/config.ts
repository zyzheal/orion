...
export function getConfigs(filters?: ConfigFilters) {
  return api.get<{ configs: ConfigItem[]; total: number }>('/api/config/configs', {
    params: filters,
  });
}

export function getConfig(id: string) {
  return api.get<ConfigItem>(`/api/config/configs/${id}`);
}
...
export function createConfig(data: {
  ...
  return api.post<ConfigItem>('/api/config/configs', data);
}

export function updateConfig(id: string, data: { value: unknown; changeReason?: string }) {
  return api.put<ConfigItem>(`/api/config/configs/${id}`, data);
}

export function deleteConfig(id: string) {
  return api.delete(`/api/config/configs/${id}`);
}

export function getConfigVersions(id: string) {
  return api.get<{ versions: ConfigVersion[] }>(`/api/config/configs/${id}/versions`);
}

export function rollbackConfig(id: string, version: number) {
  return api.post<ConfigItem>(`/api/config/configs/${id}/rollback`, { version });
}

// ==================== GitOps ====================

export function getGitOpsConfig() {
  return api.get<GitOpsConfig>('/api/config/gitops');
}

export function updateGitOpsConfig(config: Partial<GitOpsConfig>) {
  return api.put<GitOpsConfig>('/api/config/gitops', config);
}
...
export function syncFromGit() {
  ...
  return api.post<{ status: string; syncedAt: string }>('/api/config/gitops/drift');
}

export function getGitOpsStatus() {
  return api.get<GitOpsConfig>('/api/config/gitops/sync-status');
}
...
export function submitForApproval(id: string, reviewers: string[]) {
  ...
  return api.post<ApprovalWorkflow>(`/api/config/change-requests`, { configId: id, reviewers });
}

export function approveConfig(id: string, comment?: string) {
  // Backend uses /change-requests/:changeRequestId/approve
  return api.post<ApprovalWorkflow>(`/api/config/change-requests/${id}/approve`, { comment });
}

export function rejectConfig(id: string, comment: string) {
  return api.post<ApprovalWorkflow>(`/api/config/change-requests/${id}/reject`, { comment });
}

export function getApprovalWorkflow(id: string) {
  return api.get<ApprovalWorkflow>(`/api/config/change-requests/${id}`);
}
...
export function compareEnvironments(sourceEnv: string, targetEnv: string) {
  return api.get<EnvDiffResult>(`/api/config/diff/${sourceEnv}/${targetEnv}`);
}
...
export function getDiffReport(configId?: string) {
  ...
  return api.get<DiffReport>('/api/config/diff/report', { params });
}
...
export function detectDrift() {
  return api.get<DriftResult>('/api/config/gitops/drift');
}
...
export function compareConfigs(id: string, version1: number, version2: number) {
  ...
  return api.get<ConfigDiff>(`/api/config/configs/${id}/versions/diff`, {
...
export function diffConfigWithCurrent(id: string, newValue: unknown) {
  ...
  return api.post<ConfigDiff>(`/api/config/configs/${id}/versions/diff`, { newValue });
}
