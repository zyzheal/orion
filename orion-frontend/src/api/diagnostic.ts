...
export function triggerDiagnostic(data: {
  ...
  return api.post<{ sessionId: string; status: string }>('/diagnostic/trigger', data);
}
...
export function getReports(params?: { sessionId?: string; from?: string; to?: string }) {
  return api.get<DiagnosticReport[]>('/diagnostic/reports', { params });
}
...
export function searchPatterns(params?: { category?: string; keyword?: string }) {
  return api.get<DiagnosticPattern[]>('/diagnostic/knowledge/patterns', { params });
}

export function getPattern(id: string) {
  return api.get<DiagnosticPattern>(`/diagnostic/knowledge/patterns/${id}`);
}

export function addPattern(data: Omit<DiagnosticPattern, 'id' | 'frequency'>) {
  return api.post<DiagnosticPattern>('/diagnostic/knowledge/patterns', data);
}
...
export function getKnowledgeStats() {
  ...
  }>('/diagnostic/knowledge/stats');
}

export function recordOutcome(data: { sessionId: string; success: boolean; notes?: string }) {
  return api.post<{ recorded: boolean }>('/diagnostic/knowledge/outcomes', data);
}
