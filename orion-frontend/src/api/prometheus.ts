...
export async function queryMetrics(query: string, time?: number) {
  ...
  return api.get<{ result: MetricResult[] }>(`/api/v1/metrics/query${qs}`);
}
...
export async function queryRangeMetrics(query: string, start: number, end: number, step: number) {
  return api.get<{ result: MetricResult[] }>(
    `/api/v1/metrics/query/range?query=${encodeURIComponent(query)}&start=${start}&end=${end}&step=${step}`
  );
}

export async function getDashboardData() {
  return api.get<DashboardData>('/api/v1/metrics/dashboard');
}
