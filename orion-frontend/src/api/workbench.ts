...
export async function getWorkbenchFallback(): Promise<WorkbenchData> {
  ...
  const pipelineItems =
    pipelines.status === 'fulfilled'
      ? (((
          ...
          (pipelines.value.data as { data?: { data?: Array<Record<string, unknown>> } })?.data
            ?.data ||
  ...
  const ticketItems =
    tickets.status === 'fulfilled'
      ? (((
          ...
          (tickets.value.data as { data?: { data?: Array<Record<string, unknown>> } })?.data
            ?.data ||
          []) as Array<Record<string, unknown>>)
      : [];
  ...
  const deploymentItems =
    deployments.status === 'fulfilled'
      ? (((
          ...
          (deployments.value.data as { data?: { data?: Array<Record<string, unknown>> } })?.data
            ?.data ||
          []) as Array<Record<string, unknown>>)
      : [];
  ...
  return {
    myPipelines: {
      ...
      successRate: 0,
      totalRuns24h: 0,
      failedRuns: pipelineItems.filter((r: Record<string, unknown>) => r.status === 'failed')
        .length,
    },
    myAlerts: {
      ...
      recent: alertItems.map((alert: Record<string, unknown>) => ({
        ...
        acknowledged: (alert.status === 'acknowledged' ||
          (alert.acknowledged as boolean) ||
          false) as boolean,
      })),
    },
    myTickets: {
      active: ticketItems.length,
      overdue: ticketItems.filter(
        (t: Record<string, unknown>) => t.isOverdue || (t.slaRemainingHours as number) < 0
      ).length,
...
export function getMyPipelineRuns(limit = 5) {
  return api.get('/pipeline-runs', { params: { limit, status: 'all' } });
}

/** Fetch my active alerts */
export function getMyAlerts(limit = 5) {
  return api.get('/alerts', { params: { limit, status: 'active' } });
}

/** Fetch my active tickets */
export function getMyTickets(limit = 5) {
  return api.get('/tickets', { params: { limit, status: 'active' } });
}

/** Fetch my recent deployments */
export function getMyDeployments(limit = 5) {
  return api.get('/deployments', { params: { limit } });
}
