...
export function getAlertRules(params?: { enabled?: boolean }) {
  return api.get<{ rules: AlertRule[] }>('/observability/alert-rules', { params });
}

export function createAlertRule(data: AlertRuleInput) {
  return api.post<AlertRule>('/observability/alert-rules', data);
}

export function updateAlertRule(ruleId: string, data: Partial<AlertRuleInput>) {
  return api.put<AlertRule>(`/observability/alert-rules/${ruleId}`, data);
}

export function deleteAlertRule(ruleId: string) {
  return api.delete(`/observability/alert-rules/${ruleId}`);
}

export function toggleAlertRule(ruleId: string) {
  return api.post(`/observability/alert-rules/${ruleId}/toggle`);
}

// ---- Silence Rules API ----

export function getSilenceRules(params?: { status?: string }) {
  return api.get<{ rules: SilenceRule[] }>('/observability/silence-rules', { params });
}

export function createSilenceRule(data: SilenceRuleInput) {
  return api.post<SilenceRule>('/observability/silence-rules', data);
}

export function deleteSilenceRule(ruleId: string) {
  return api.delete(`/observability/silence-rules/${ruleId}`);
}

// ---- Root Cause Analysis API ----

export function getRootCauseAnalyses(params?: { incidentId?: string; status?: string }) {
  return api.get<{ analyses: RootCauseAnalysis[] }>('/observability/rca', { params });
}

export function getRootCauseAnalysis(analysisId: string) {
  return api.get<RootCauseAnalysis>(`/observability/rca/${analysisId}`);
}

export function triggerRCA(data: { incidentId: string; serviceIds?: string[] }) {
  return api.post<RootCauseAnalysis>('/observability/rca/trigger', data);
}
...
export function getTraces(params?: {
  ...
  return api.get<{ traces: TraceData[] }>('/observability/traces', { params });
}

export function getTrace(traceId: string) {
  return api.get<TraceData>(`/observability/traces/${traceId}`);
}

// ---- Service Health API ----

export function getServiceHealth(params?: { serviceName?: string }) {
  return api.get<{ services: ServiceHealth[] }>('/observability/health', { params });
}
...
export function getAlertRuleTemplates(params?: { category?: string }) {
  return api.get<{ data: AlertRuleTemplate[]; total: number }>(
    '/observability/alert-rule-templates',
    { params }
  );
}
...
export function createAlertRuleFromTemplate(data: {
  ...
  return api.post('/observability/alert-rules/from-template', data);
}
...
export function getRcaTimeline(deploymentId: string, params?: { start?: string; end?: string }) {
  return api.get<{ timeline: TimelineReport }>(
    `/observability/rca/${deploymentId}/timeline`,
    { params }
  );
}
...
export function getDependencyGraph() {
  return api.get<{ data: ServiceDependency[] }>('/observability/dependency-graph');
}

export function analyzeDependencyRootCause(affectedServices: string[]) {
  return api.post<{ data: string[] }>('/observability/dependency-graph/analyze', {
    affectedServices,
  });
}
...
export function analyzeTemporalCorrelation(
  ...
  return api.post<{ data: TemporalCorrelationResult }>(
    '/observability/temporal-correlation',
    { alerts, windowMs }
  );
}
