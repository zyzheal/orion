...
export const orchestrationApi = {
  create: async (data: CreateOrchestrationInput) => {
    const response = await apiClient.post('/orchestration', data);
  ...
  getById: async (id: string) => {
    const response = await apiClient.get(`/orchestration/${id}`);
    return response.data as OrchestrationFlow;
  },

  execute: async (id: string) => {
    const response = await apiClient.post(`/orchestration/${id}/execute`);
    return response.data as OrchestrationFlow;
  },

  pause: async (id: string) => {
    const response = await apiClient.post(`/orchestration/${id}/pause`);
    return response.data as OrchestrationFlow;
  },

  resume: async (id: string) => {
    const response = await apiClient.post(`/orchestration/${id}/resume`);
    return response.data as OrchestrationFlow;
  },

  abort: async (id: string) => {
    const response = await apiClient.post(`/orchestration/${id}/abort`);
    return response.data as OrchestrationFlow;
  },
};
