...
export function executeAIRequest(request: AIRequest) {
  return api.post<AIResponse>('/api/ai-gateway/execute', request);
}

// ==================== Health Monitoring ====================

export function getScenarioHealth(scenario: AIScenario) {
  return api.get<AIGatewayHealth>(`/api/ai-gateway/health/${scenario}`);
}

export function getAllHealth() {
  return api.get<{ health: AIGatewayHealth[] }>('/api/ai-gateway/health/all');
}

export function getGatewayStatus() {
  return api.get<{ status: string }>('/api/ai-gateway/status');
}
...
export function getRules() {
  return api.get<{ rules: RuleSet[] }>('/api/ai-gateway/rules');
}

export function getEngineStatus() {
  return api.get<{ cacheEnabled: boolean; auditEnabled: boolean }>('/api/ai-gateway/engine/status');
}

// ==================== Configuration ====================

export function getConfig() {
  return api.get<{ config: AIGatewayConfig }>('/api/ai-gateway/config');
}

export function updateConfig(config: Partial<AIGatewayConfig>) {
  return api.put('/api/ai-gateway/config', config);
}
