...
export const disasterRecoveryApi = {
  createDRPlan: async (data: {
    ...
    const response = await apiClient.post('/api/disaster-recovery/plans', data);
    return response.data as DRPlan;
  },

  listDRPlans: async (params?: { status?: string }) => {
    const response = await apiClient.get('/api/disaster-recovery/plans', { params });
    return response.data as DRPlan[];
  },

  executeFailoverTest: async (planId: string) => {
    const response = await apiClient.post(`/api/disaster-recovery/plans/${planId}/failover-test`);
    return response.data as FailoverTest;
  },

  createBackup: async (data: { sourceService: string; type?: string }) => {
    const response = await apiClient.post('/api/disaster-recovery/backups', data);
    return response.data as BackupRecord;
  },

  executeFailover: async (planId: string, data?: { reason?: string; dryRun?: boolean }) => {
    const response = await apiClient.post(`/api/disaster-recovery/plans/${planId}/failover`, data);
    return response.data;
  },

  getDRStatus: async () => {
    const response = await apiClient.get('/api/disaster-recovery/status');
