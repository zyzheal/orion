 ...
 * Prefix: /api/v1/tickets/:id/to-knowledge
 */
...
export const createTicketKnowledgeToKnowledge = async (id: string, data?: Partial<TicketKnowledge>): Promise<TicketKnowledge> => {
  const response = await api.post<TicketKnowledge>('/api/v1/tickets/:id/to-knowledge/' + id + '/to-knowledge', data);
  return response.data;
};

export const getTicketKnowledge = async (id: string): Promise<TicketKnowledge> => {
  const response = await api.get<TicketKnowledge>('/api/v1/tickets/:id/to-knowledge/' + id + '/to-knowledge/preview');
  return response.data;
};
