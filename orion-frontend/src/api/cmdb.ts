...
export const getCIs = async (params?: ListCIsParams) => {
  const response = await apiClient.get('/api/cmdb/cis', { params });
  return response.data as { data?: CIItem[] };
};

export const getCI = async (id: string) => {
  const response = await apiClient.get(`/api/cmdb/cis/${id}`);
  return response.data as { ci?: CIItem };
};

export const createCI = async (input: CreateCIInput) => {
  const response = await apiClient.post('/api/cmdb/cis', input);
  return response.data as { ci?: CIItem };
};

export const updateCI = async (id: string, input: UpdateCIInput) => {
  const response = await apiClient.put(`/api/cmdb/cis/${id}`, input);
  return response.data as { ci?: CIItem };
};

export const deleteCI = async (id: string) => {
  const response = await apiClient.delete(`/api/cmdb/cis/${id}`);
...
export const startK8sSync = async () => {
  const response = await apiClient.post('/api/cmdb/k8s/sync/start');
  return response.data;
};

export const stopK8sSync = async () => {
  const response = await apiClient.post('/api/cmdb/k8s/sync/stop');
  return response.data;
};
...
export const executeScript = async (input: {
  ...
  const response = await apiClient.post('/api/cmdb/execute', input);
  return response.data;
};
...
export const getImpactAnalysis = async (ciId: string) => {
  const response = await apiClient.get(`/api/cmdb/topology/${ciId}/impact`);
...
export const getCIVersions = async (ciId: string) => {
  const response = await apiClient.get(`/api/cmdb/cis/${ciId}/versions`);
  return response.data as { versions?: unknown[] };
};

export const getCICurrentVersion = async (ciId: string) => {
  const response = await apiClient.get(`/api/cmdb/cis/${ciId}/versions/current`);
  return response.data as { version?: number };
};

export const restoreCIVersion = async (ciId: string, version: string, user?: string) => {
  const response = await apiClient.post(`/api/cmdb/cis/${ciId}/versions/restore`, { version, user });
  return response.data as { ci?: CIItem };
};
...
export const getCIByCiId = async (ciId: string, tenantId?: string) => {
  const response = await apiClient.get(`/api/cmdb/cis/by-id/${ciId}`, { params: { tenantId } });
  return response.data as { ci?: CIItem };
};
...
export const getCIDependencies = async (ciId: string) => {
  const response = await apiClient.get(`/api/cmdb/topology/${ciId}/dependencies`);
...
export const getCMDBHealth = async () => {
  const response = await apiClient.get('/api/cmdb/health');
  return response.data;
};
