...
export const getCIs = async (params?: ListCIsParams) => {
  const response = await apiClient.get('/cmdb/cis', { params });
  return response.data as { data?: CIItem[] };
};

export const getCI = async (id: string) => {
  const response = await apiClient.get(`/cmdb/cis/${id}`);
  return response.data as { ci?: CIItem };
};

export const createCI = async (input: CreateCIInput) => {
  const response = await apiClient.post('/cmdb/cis', input);
  return response.data as { ci?: CIItem };
};

export const updateCI = async (id: string, input: UpdateCIInput) => {
  const response = await apiClient.put(`/cmdb/cis/${id}`, input);
  return response.data as { ci?: CIItem };
};

export const deleteCI = async (id: string) => {
  const response = await apiClient.delete(`/cmdb/cis/${id}`);
...
export const startK8sSync = async () => {
  const response = await apiClient.post('/cmdb/k8s/sync/start');
  return response.data;
};

export const stopK8sSync = async () => {
  const response = await apiClient.post('/cmdb/k8s/sync/stop');
  return response.data;
};
...
export const executeScript = async (input: {
  ...
  const response = await apiClient.post('/cmdb/execute', input);
  return response.data;
};
...
export const getImpactAnalysis = async (ciId: string) => {
  const response = await apiClient.get(`/cmdb/topology/${ciId}/impact`);
...
export const getCIByCiId = async (ciId: string, tenantId?: string) => {
  const response = await apiClient.get(`/cmdb/cis/by-id/${ciId}`, { params: { tenantId } });
  return response.data as { ci?: CIItem };
};
...
export const getCIDependencies = async (ciId: string) => {
  const response = await apiClient.get(`/cmdb/topology/${ciId}/dependencies`);
...
export const getRecommendations = async (params?: {
  ...
  const response = await apiClient.get('/cmdb/recommendations', { params });
  return response.data as { data?: RecommendationResult };
};

export const actionRecommendation = async (id: string, action: 'accept' | 'reject') => {
  const response = await apiClient.post(`/cmdb/recommendations/${id}/action`, { action });
  return response.data;
};

export const getCMDBHealth = async () => {
  const response = await apiClient.get('/cmdb/health');
  return response.data;
};
