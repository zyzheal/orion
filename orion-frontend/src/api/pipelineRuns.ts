 ...
 * Maps to GET /pipeline-runs
 */
export function getAllPipelineRuns(params?: GetAllPipelineRunsParams) {
  return api.get<PipelineRunListResponse>('/pipeline-runs', { params });
}

/**
 * Retry a pipeline run
 * Maps to POST /pipeline-runs/:id/retry
 */
export function retryPipelineRun(
  runId: string,
  options?: { fromStage?: string; onlyFailed?: boolean }
) {
  return api.post(`/pipeline-runs/${runId}/retry`, null, { params: options });
}

/**
 * Cancel a pipeline run
 * Maps to POST /pipeline-runs/:id/cancel
 */
 ...
 * Maps to POST /pipeline-runs/:id/retry?fromStage=stageId
 */
export function retryFromStage(runId: string, stageId: string) {
  return api.post(`/pipeline-runs/${runId}/retry`, null, {
 ...
 * Maps to GET /pipeline-runs/:id/stages
 */
export function getPipelineRunStages(runId: string) {
  return api.get(`/pipeline-runs/${runId}/stages`);
}
