...
export function getCostOverview(params?: { projectId?: string; period?: string }) {
  return api.get<CostOverview>('/v1/cost-operations/overview', { params });
}
...
export function getCostTrend(params?: {
  ...
  return api.get<{ trends: CostTrendPoint[] }>('/v1/cost-operations/trend', { params });
}

export function getCostByService(params?: { period?: string }) {
  return api.get<{ services: CostByService[] }>('/v1/cost-operations/by-service', { params });
}
...
export function getCostAnomalies(params?: {
  ...
  return api.get<{ anomalies: CostAnomaly[] }>('/v1/cost-operations/anomalies', { params });
}
...
export function getOptimizationSuggestions(params?: {
  ...
  return api.get<{ suggestions: OptimizationSuggestion[] }>(
    '/v1/cost-operations/optimizations',
    { params }
  );
}

export function applyOptimization(suggestionId: string) {
  return api.post(`/v1/cost-operations/optimizations/${suggestionId}/apply`);
}

export function rejectOptimization(suggestionId: string) {
  return api.post(`/v1/cost-operations/optimizations/${suggestionId}/reject`);
}

// ---- Budget API ----

export function getBudgets(params?: { projectId?: string }) {
  return api.get<{ budgets: BudgetConfig[] }>('/v1/cost-operations/budgets', { params });
}
...
export function createBudget(data: {
  ...
  return api.post<BudgetConfig>('/v1/cost-operations/budgets', data);
}

export function updateBudget(budgetId: string, data: Partial<typeof createBudget.arguments>) {
  return api.put<BudgetConfig>(`/v1/cost-operations/budgets/${budgetId}`, data);
}

export function deleteBudget(budgetId: string) {
  return api.delete(`/v1/cost-operations/budgets/${budgetId}`);
}

export function checkBudgetGate(pipelineId: string, estimatedCost: number) {
  return api.post<BudgetGateResult>('/v1/cost-operations/budget-gate/check', {
    pipelineId,
    estimatedCost,
  });
}
...
export function getBudgetGuards(params?: { tenantId?: string }) {
  return api.get<{ success: boolean; data: BudgetGuard[] }>(
    '/v1/cost-operations/budget-guards',
    { params },
  );
}

export function createBudgetGuard(data: BudgetGuardInput) {
  return api.post<{ success: boolean; data: BudgetGuard }>('/v1/cost-operations/budget-guards', data);
}
...
export function evaluateBudgetGuard(pipelineId: string, estimatedCost: number, options?: {
  ...
  return api.post<{ success: boolean; data: EvaluationResult }>('/v1/cost-operations/evaluate', {
