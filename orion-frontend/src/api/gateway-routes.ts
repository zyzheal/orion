...
export async function getGatewayRoutes(): Promise<GatewayRoute[]> {
  const response = await api.get<GatewayRoute[]>('/gateway/routes');
  return response.data;
}

export async function getGatewayRoute(id: string): Promise<GatewayRoute> {
  const response = await api.get<GatewayRoute>(`/gateway/routes/${id}`);
  return response.data;
}

export async function createGatewayRoute(data: GatewayRouteInput): Promise<GatewayRoute> {
  const response = await api.post<GatewayRoute>('/gateway/routes', data);
  return response.data;
}
...
export async function updateGatewayRoute(
  ...
  const response = await api.put<GatewayRoute>(`/gateway/routes/${id}`, data);
  return response.data;
}

export async function deleteGatewayRoute(id: string): Promise<void> {
  await api.delete<void>(`/gateway/routes/${id}`);
}

export async function toggleGatewayRoute(id: string, enabled: boolean): Promise<GatewayRoute> {
  const response = await api.patch<GatewayRoute>(`/gateway/routes/${id}/toggle`, {
    enabled,
  });
  return response.data;
}
...
export async function getGatewayRouteStats(): Promise<GatewayRouteStats> {
  const response = await api.get<GatewayRouteStats>('/gateway/routes/stats');
  return response.data;
}
