...
export function getTrace(traceId: string) {
  return api.get<LLMTrace>(`/v1/llm/traces/${traceId}`);
}
...
export function getDailyStats(params: DailyStatsQuery) {
  return api.get<DailyStats>('/v1/llm/stats/daily', { params });
}

// ---- Cost Analysis ----

export function getCostBreakdown(params: CostBreakdownQuery) {
  return api.get<CostBreakdown>('/v1/llm/cost/breakdown', { params });
}

export function getTrackingAccuracy() {
  return api.get<TrackingAccuracy>('/v1/llm/tracking/accuracy');
}

// ---- Pricing ----

export function getPricing() {
  return api.get<PricingTable>('/v1/llm/pricing');
}

export function estimateCost(data: CostEstimateInput) {
  return api.post<CostEstimate>('/v1/llm/cost/estimate', data);
}