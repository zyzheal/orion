...
export function getTrace(traceId: string) {
  return api.get<LLMTrace>(`/llm/traces/${traceId}`);
}

export function getTraces(params?: TraceQuery) {
  return api.get<LLMTrace[]>('/llm/traces', { params });
}

// ---- Statistics ----

export function getDailyStats(params: DailyStatsQuery) {
  return api.get<DailyStats>('/llm/stats/daily', { params });
}

// ---- Cost Analysis ----

export function getCostBreakdown(params: CostBreakdownQuery) {
  return api.get<CostBreakdown>('/llm/cost/breakdown', { params });
}

export function getTrackingAccuracy() {
  return api.get<TrackingAccuracy>('/llm/tracking/accuracy');
}

// ---- Pricing ----

export function getPricing() {
  return api.get<PricingTable>('/llm/pricing');
}

export function estimateCost(data: CostEstimateInput) {
  return api.post<CostEstimate>('/llm/cost/estimate', data);
}
