...
export const userApi = {
  ...
  getTeams: (userId: string) => {
    return api.get<UserTeam[]>(`/users/${userId}/teams`);
  },
  ...
  getPermissions: (userId: string) => {
    return api.get<UserPermission[]>(`/users/${userId}/permissions`);
  },
  ...
  getActivities: (userId: string, page = 1, pageSize = 20) => {
    return api.get<UserActivity[]>(`/users/${userId}/activities`, {
  ...
  getTokens: (userId: string) => {
    return api.get<UserToken[]>(`/users/${userId}/tokens`);
  },
  ...
  createToken: (userId: string, name: string, expiresInDays?: number) => {
    return api.post<UserToken>(`/users/${userId}/tokens`, { name, expiresInDays });
  },
  ...
  deleteToken: (userId: string, tokenId: string) => {
    return api.delete<void>(`/users/${userId}/tokens/${tokenId}`);
  },
  ...
  getNotificationPreferences: (userId: string) => {
    return api.get<NotificationPreferences>(`/users/${userId}/notifications`);
  },
  ...
  updateNotificationPreferences: (userId: string, data: NotificationPreferences) => {
    return api.put<NotificationPreferences>(`/users/${userId}/notifications`, data);
  },
