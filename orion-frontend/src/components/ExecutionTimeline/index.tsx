    ...
    <Card
      ...
      extra={
        <Space>
          ...
          <Button icon={<FastForwardOutlined />} size="small">
            2x
          </Button>
        </Space>
      }
    >
          ...
          children: (
            <div
              style={{
                cursor: 'pointer',
                padding: '4px 8px',
                background: selectedStep?.id === step.id ? 'colors.primary[50]' : 'transparent',
              }}
              onClick={() => setSelectedStep(step)}
            >
              <strong>{step.stepName}</strong>
              <Tag color={getStatusColor(step.status)} style={{ marginLeft: spacing.sm }}>
                {step.status}
              </Tag>
      ...
      {selectedStep && (
        <div style={{ marginTop: spacing.md }}>
          ...
          <p>
            <strong>Started At:</strong> {new Date(selectedStep.startedAt).toLocaleString()}
          </p>
          {selectedStep.durationMs && (
            <p>
              <strong>Duration:</strong> {formatDuration(selectedStep.durationMs)}
            </p>
          )}
          {selectedStep.errorMessage && (
            <p>
              <strong>Error:</strong> {selectedStep.errorMessage}
            </p>
          )}
          {diagnosis && (
            <div
            ...
            >
              ...
              <p>
                <strong>Suggested Fix:</strong> {diagnosis.suggestedFix}
              </p>
