...
// NATSSubscriber subscribes to ComplianceEvent via NATS JetStream.
...
// Start subscribes to ComplianceEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "compliance-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received compliance event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleComplianceEvent(ctx, &event); err != nil {
		s.log.Error("handle compliance event", zap.Error(err))
...
func (s *NATSSubscriber) handleComplianceEvent(ctx context.Context, event *EventBusEvent) error {
