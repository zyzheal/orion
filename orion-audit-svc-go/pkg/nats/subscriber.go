...
// NATSSubscriber subscribes to AuditEvent via NATS JetStream.
...
// Start subscribes to AuditEvent subjects.
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "audit-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received audit event",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleAuditEvent(ctx, &event); err != nil {
		s.log.Error("handle audit event", zap.Error(err))
...
func (s *NATSSubscriber) handleAuditEvent(ctx context.Context, event *EventBusEvent) error {
