...
// NATSSubscriber subscribes to CMDBEvent via NATS JetStream.
...
func (s *NATSSubscriber) Start(ctx context.Context) error {
	...
	cons, err := s.js.Consumer(ctx, s.stream, "cmdb-svc-consumer")
	if err != nil {
...
func (s *NATSSubscriber) handleMessage(ctx context.Context, msg jetstream.Msg) {
	...
	s.log.Info("received CMDBEvent",
		zap.String("event_type", event.EventType),
		zap.String("source", event.Source),
	)

	if err := s.handleCMDBEvent(ctx, &event); err != nil {
		s.log.Error("handle cmdb event", zap.Error(err))
...
func (s *NATSSubscriber) handleCMDBEvent(ctx context.Context, event *EventBusEvent) error {
	// TODO: 根据 event.EventType 触发相应的 CMDB 操作
	// 例如：CIUpdated → 同步 CI 拓扑关系
	//       RelationCreated → 更新关系缓存
	return nil
}
