...
export class DeploymentWorkflow {
  ...
  /**
   * List deployments with optional filters (used by API routes)
   */
  async listDeployments(query: {
    ...
    if (query.pipelineId) {
      deployments = deployments.filter(
        (d) => d.notes === query.pipelineId || d.changeRequestId === query.pipelineId
      );
    }
    return deployments;
  }
  ...
  /**
   * Execute a deployment from API route parameters (used by API routes)
   */
  async executeDeploy(input: {
    pipelineId?: string;
    runId?: string;
    environment: string;
    strategy?: string;
    ...
    const config: DeployConfig = {
      appName: input.pipelineId || 'unknown',
      version: input.runId || 'latest',
      environment: input.environment,
      strategy: (input.strategy || 'rolling') as Deployment['strategy'],
      initiatedBy: input.triggeredBy || 'api',
      notes: input.pipelineId,
      envOverrides: input.envOverrides,
    };
    return this.startDeployment(config);
  }

  /**
   * Get a single deployment by ID (used by API routes)
   */
  async getDeployment(id: string): Promise<Deployment | null> {
    return this.historyService.getDeployment(id);
  }

  /**
   * Rollback a deployment by ID (used by API routes)
   */
  async rollbackDeployment(
    id: string,
    options?: { reason?: string; triggeredBy?: string }
  ): Promise<{ deployment: Deployment; rollback: RollbackInfo }> {
    const deployment = await this.historyService.getDeployment(id);
    if (!deployment) {
    ...
    await this.historyService.updateDeployment(id, {
    ...
    return { deployment: result.deployment, rollback: rollbackInfo as unknown as RollbackInfo };
  }
}
