...
type AlertCorrelationError struct {
	Code    string
	Message string
	Cause   error
}
...
func (e *AlertCorrelationError) Is(target error) bool {
	_, ok := target.(*AlertCorrelationError)
	return ok
}
...
func NewAlertCorrelationError(code, msg string) error {
	return &AlertCorrelationError{Code: code, Message: msg}
}
