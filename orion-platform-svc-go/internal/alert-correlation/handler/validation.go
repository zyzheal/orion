...
func (v *AlertCorrelationValidator) ValidateName(name string) error {
	if strings.TrimSpace(name) == "" {
		return ErrAlertCorrelationInvalidInput
	}
	if len(name) > v.MaxNameLength {
		return fmt.Errorf("alert-correlation: name too long")
	}
...
func (v *AlertCorrelationValidator) ValidateDescription(desc string) error {
	if len(desc) > v.MaxDescLength {
		return fmt.Errorf("alert-correlation: description too long")
	}
	return nil
}
...
type AlertCorrelationValidationResult struct {
	Valid  bool
	Errors []string
}
...
func (v *AlertCorrelationValidator) Validate(name, desc, typ, id string) *AlertCorrelationValidationResult {
	...
	for _, c := range checks {
		if err := c.fn(); err != nil {
			r.Errors = append(r.Errors, c.field+": "+err.Error())
		}
	}
