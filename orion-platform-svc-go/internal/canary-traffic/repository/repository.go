...
func (r *Repository) Create(ctx context.Context, entity *models.CanaryTraffic) error {
	entity.ID = uuid.New().String()
	now := time.Now().UTC()
	entity.CreatedAt = now
	entity.UpdatedAt = now
	_, err := r.db.NamedExecContext(ctx,
		`INSERT INTO canary_traffics (id, tenant_id, name, created_at, updated_at)
...
func (r *Repository) GetByID(ctx context.Context, id, tenantID string) (*models.CanaryTraffic, error) {
	var entity models.CanaryTraffic
	err := r.db.GetContext(ctx, &entity,
		`SELECT * FROM canary_traffics WHERE id=$1 AND tenant_id=$2`, id, tenantID)
	if err != nil {
		return nil, err
	}
	return &entity, nil
...
func (r *Repository) List(ctx context.Context, tenantID string) ([]models.CanaryTraffic, error) {
	...
	err := r.db.SelectContext(ctx, &entities,
		`SELECT * FROM canary_traffics WHERE tenant_id=$1 ORDER BY created_at DESC`, tenantID)
	if err != nil {
		return nil, err
	}
	return entities, nil
...
func (r *Repository) Update(ctx context.Context, id, tenantID string, attrs map[string]interface{}) (*models.CanaryTraffic, error) {
	...
	query := fmt.Sprintf("UPDATE canary_traffics SET %s WHERE id=$%d AND tenant_id=$%d",
...
func (r *Repository) Delete(ctx context.Context, id, tenantID string) (bool, error) {
	result, err := r.db.ExecContext(ctx,
		`DELETE FROM canary_traffics WHERE id=$1 AND tenant_id=$2`, id, tenantID)
	if err != nil {
		return false, err
	}
	n, _ := result.RowsAffected()
