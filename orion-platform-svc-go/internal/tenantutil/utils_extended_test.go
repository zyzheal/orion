...
func TestTenantutilCounter(t *testing.T) {
	...
	c.Decrement()
	if c.Value() != 1 {
		t.Fatal("counter should remain 1")
	}
	c.Reset()
	if c.Value() != 0 {
...
func TestTenantutilConfigStore(t *testing.T) {
	...
	if !ok || val != "localhost" {
		t.Fatal("get should return localhost")
	}
}
...
func TestTenantutilUnique(t *testing.T) {
	...
	if len(result) != 3 {
		t.Fatal("unique should remove dupes")
	}
}

func TestTenantutilContains(t *testing.T) {
	items := []string{"a", "b", "c"}
	if !TenantutilContains(items, "b") {
		t.Fatal("should find b")
	}
	if TenantutilContains(items, "d") {
		t.Fatal("should not find d")
	}
}
