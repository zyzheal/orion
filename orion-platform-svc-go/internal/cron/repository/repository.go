...
func (r *Repository) Create(ctx context.Context, m *models.CronJob) error {
	...
	_, err := r.db.NamedExecContext(ctx,
		`INSERT INTO cron_jobs (id, tenant_id, name, created_at, updated_at) VALUES (:id, :tenant_id, :name, :created_at, :updated_at)`, m)
	return err
}
...
func (r *Repository) List(ctx context.Context, tenantID string, limit, offset int) ([]models.CronJob, error) {
	...
	err := r.db.SelectContext(ctx, &items, `SELECT * FROM cron_jobs WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`, tenantID, limit, offset)
	if err != nil {
		return nil, err
	}
	return items, nil
...
func (r *Repository) Update(ctx context.Context, tenantID, id string, updates map[string]interface{}) error {
	...
	_, err := r.db.ExecContext(ctx, `UPDATE cron_jobs SET updated_at = NOW() WHERE id=$1 AND tenant_id=$2`, id, tenantID)
	if err != nil {
