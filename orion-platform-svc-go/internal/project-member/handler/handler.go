...
type Handler struct{ svc service.ServiceInterface }

func NewHandler(svc service.ServiceInterface) *Handler { return &Handler{svc: svc} }
...
func (h *Handler) Create(c *gin.Context) {
	...
	if err != nil {
		switch err {
		case service.ErrBadRequest, service.ErrInvalidRole:
			goerr.WriteError(c, goerr.ErrBadRequest, err.Error(), 400)
		case service.ErrDuplicateMember:
...
func (h *Handler) Delete(c *gin.Context) {
	...
	tenantID := c.GetString("tenant_id")
	id := c.Param("id")
	if err := h.svc.DeleteMember(ctx, tenantID, id); err != nil {
		goerr.WriteError(c, goerr.ErrNotFound, "not found", 404)
		return
	}
