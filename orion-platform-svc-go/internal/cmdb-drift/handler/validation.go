...
func (v *CmdbDriftValidator) ValidateName(name string) error {
	if strings.TrimSpace(name) == "" {
		return ErrCmdbDriftInvalidInput
	}
	if len(name) > v.MaxNameLength {
		return fmt.Errorf("cmdb-drift: name too long")
	}
...
func (v *CmdbDriftValidator) ValidateDescription(desc string) error {
	if len(desc) > v.MaxDescLength {
		return fmt.Errorf("cmdb-drift: description too long")
	}
	return nil
}
...
type CmdbDriftValidationResult struct {
	Valid  bool
	Errors []string
}
