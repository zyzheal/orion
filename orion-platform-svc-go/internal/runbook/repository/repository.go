...
func (r *Repository) Create(ctx context.Context, m *models.Runbook) error {
	...
	_, err := r.db.NamedExecContext(ctx, `
		INSERT INTO runbook (id, tenant_id, name, value, enabled, created_at, updated_at)
...
func (r *Repository) List(ctx context.Context, tenantID string) ([]models.Runbook, error) {
	var items []models.Runbook
	err := r.db.SelectContext(ctx, &items, `SELECT * FROM runbook WHERE tenant_id = $1 ORDER BY created_at DESC`, tenantID)
	return items, err
}
...
func (r *Repository) Update(ctx context.Context, tenantID, id string, updates map[string]interface{}) (*models.Runbook, error) {
	...
	_, err := r.db.ExecContext(ctx,
		"UPDATE runbook SET "+strings.Join(setParts, ", ")+
