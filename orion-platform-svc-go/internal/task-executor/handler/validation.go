...
func (v *TaskExecutorValidator) ValidateName(name string) error {
	if strings.TrimSpace(name) == "" {
		return ErrTaskExecutorInvalidInput
	}
	if len(name) > v.MaxNameLength {
		return fmt.Errorf("task-executor: name too long")
	}
...
func (v *TaskExecutorValidator) ValidateDescription(desc string) error {
	if len(desc) > v.MaxDescLength {
		return fmt.Errorf("task-executor: description too long")
	}
	return nil
}
...
type TaskExecutorValidationResult struct {
	Valid  bool
	Errors []string
}
