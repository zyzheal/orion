...
func ruleRow(id, tenant, name, value string, enabled bool) *sqlmock.Rows {
	return sqlmock.NewRows([]string{
		"id", "tenant_id", "name", "value", "enabled", "created_at", "updated_at",
	}).AddRow(id, tenant, name, value, enabled, time.Now().UTC(), time.Now().UTC())
}

// Every statement in this repository used the identifier "ticket-automation",
// which postgres parses as ticket minus automation and rejects with
// "syntax error at or near \"-\"". Five live routes therefore failed before
// reaching SQL semantics. The assertions below pin the underscored name; a
// revert to the hyphen fails all of them.
func TestStatementsUseValidSQLIdentifier(t *testing.T) {
	mock, repo := newMockRepo(t)
	mock.ExpectExec(`INSERT INTO ticket_automation `).
		WillReturnResult(sqlmock.NewResult(0, 1))
	if _, err := repo.Create(context.Background(), "tenant-a", &models.AutomationRule{
		Name: "n", Value: "v",
	}); err != nil {
		t.Fatalf("Create: %v", err)
	}
	mock.ExpectQuery(`SELECT \* FROM ticket_automation WHERE id = \$1 AND tenant_id = \$2`).
		WithArgs("a-1", "tenant-a").WillReturnRows(ruleRow("a-1", "tenant-a", "n", "v", true))
	if _, err := repo.GetByID(context.Background(), "tenant-a", "a-1"); err != nil {
		t.Fatalf("GetByID: %v", err)
	}
	mock.ExpectQuery(`SELECT \* FROM ticket_automation WHERE tenant_id = \$1`).
		WithArgs("tenant-a").WillReturnRows(ruleRow("a-1", "tenant-a", "n", "v", true))
	if _, err := repo.List(context.Background(), "tenant-a"); err != nil {
		t.Fatalf("List: %v", err)
	}
	mock.ExpectExec(`DELETE FROM ticket_automation WHERE id = \$1 AND tenant_id = \$2`).
		WithArgs("a-1", "tenant-a").WillReturnResult(sqlmock.NewResult(0, 1))
	if err := repo.Delete(context.Background(), "tenant-a", "a-1"); err != nil {
...
func TestUpdateBindsWhitelistedColumnsPositionally(t *testing.T) {
	mock, repo := newMockRepo(t)
	mock.ExpectExec(
		`UPDATE ticket_automation SET name = \$1, value = \$2, enabled = \$3, `+
			`updated_at = \$4 WHERE id = \$5 AND tenant_id = \$6`,
	).WithArgs("n2", "v2", false, sqlmock.AnyArg(), "a-1", "tenant-a").
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectQuery(
		`SELECT \* FROM ticket_automation WHERE id = \$1 AND tenant_id = \$2`,
	).WithArgs("a-1", "tenant-a").WillReturnRows(ruleRow("a-1", "tenant-a", "n2", "v2", false))
