...
func newHandler() *Handler {
	return NewHandler(&fakeTicket_automationService{})
}
...
func TestHandler_TICKET_AUTOM_Create(t *testing.T) {
...
func TestHandler_TICKET_AUTOM_Get(t *testing.T) {
	c, w := makeCtx(http.MethodGet, "/")
	newHandler().Get(c)
	if w.Code >= 500 {
...
func TestHandler_TICKET_AUTOM_List(t *testing.T) {
	c, w := makeCtx(http.MethodGet, "/")
	newHandler().List(c)
	if w.Code >= 500 {
...
func TestHandler_TICKET_AUTOM_Update(t *testing.T) {
	c, w := makeCtx(http.MethodGet, "/")
	newHandler().Update(c)
	if w.Code >= 500 {
...
func TestHandler_TICKET_AUTOM_Delete(t *testing.T) {
	c, w := makeCtx(http.MethodGet, "/")
	newHandler().Delete(c)
	if w.Code >= 500 {
