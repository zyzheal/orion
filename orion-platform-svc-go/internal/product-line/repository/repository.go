...
func (r *Repository) Create(ctx context.Context, m *models.ProductLine) error {
	...
	_, err := r.db.NamedExecContext(ctx,
		`INSERT INTO product_lines (id, tenant_id, name, created_at, updated_at) VALUES (:id, :tenant_id, :name, :created_at, :updated_at)`, m)
	return err
}
...
func (r *Repository) Update(ctx context.Context, tenantID, id string, updates map[string]interface{}) error {
	...
	_, err := r.db.ExecContext(ctx, `UPDATE product_lines SET updated_at = NOW() WHERE id=$1 AND tenant_id=$2`, id, tenantID)
	if err != nil {
