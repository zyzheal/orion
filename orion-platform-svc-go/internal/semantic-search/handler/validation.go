...
func (v *SemanticSearchValidator) ValidateName(name string) error {
	if strings.TrimSpace(name) == "" {
		return ErrSemanticSearchInvalidInput
	}
	if len(name) > v.MaxNameLength {
		return fmt.Errorf("semantic-search: name too long")
	}
...
func (v *SemanticSearchValidator) ValidateDescription(desc string) error {
	if len(desc) > v.MaxDescLength {
		return fmt.Errorf("semantic-search: description too long")
	}
	return nil
}
...
type SemanticSearchValidationResult struct {
	Valid  bool
	Errors []string
}
