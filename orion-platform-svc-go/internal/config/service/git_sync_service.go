...
func NewGitSyncService(repo *repository.RepositoryV2) *GitSyncService {
...
func (s *GitSyncService) Create(ctx context.Context, tenantID string, req models.CreateGitSyncRequest) (*models.GitSyncConfig, error) {
	...
	if err := s.repo.CreateGitSyncV2(ctx, g); err != nil {
		return nil, err
	}
	return g, nil
}
...
func (s *GitSyncService) SyncNow(ctx context.Context, tenantID, id string) (*models.SyncResult, error) {
	g, err := s.repo.GetGitSyncV2(ctx, tenantID, id)
	if err != nil {
	...
	if err := s.repo.UpdateGitSyncStatusV2(ctx, tenantID, g.ID, "success"); err != nil {
		return nil, err
	}
