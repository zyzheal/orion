...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	r := rg.Group("/script-library")
	r.GET("", auth.RequirePermission("script_library", "read"), h.List)
	r.GET("/:id", auth.RequirePermission("script_library", "read"), h.Get)
	r.POST("", auth.RequirePermission("script_library", "write"), h.Create)
	r.PUT("/:id", auth.RequirePermission("script_library", "write"), h.Update)
	r.DELETE("/:id", auth.RequirePermission("script_library", "delete"), h.Delete)
}
...
func (h *Handler) Create(c *gin.Context) {
	...
	defer span.End()
	tenantID := c.GetString("tenant_id")
	var req models.CreateScriptLibraryRequest
...
func (h *Handler) Update(c *gin.Context) {
	...
	tenantID := c.GetString("tenant_id")
	id := c.Param("id")
	var req models.UpdateScriptLibraryRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		errors.WriteError(c, errors.ErrBadRequest, err.Error(), 400)
		return
	}
