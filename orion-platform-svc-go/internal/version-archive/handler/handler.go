...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	r := rg.Group("/version-archive")
	r.GET("", auth.RequirePermission("version_archive", "read"), h.List)
	r.GET("/:id", auth.RequirePermission("version_archive", "read"), h.Get)
	r.POST("", auth.RequirePermission("version_archive", "write"), h.Create)
	r.PUT("/:id", auth.RequirePermission("version_archive", "write"), h.Update)
	r.DELETE("/:id", auth.RequirePermission("version_archive", "delete"), h.Delete)
}
...
func (h *Handler) Create(c *gin.Context) {
	...
	defer span.End()
	tenantID := c.GetString("tenant_id")
	var req models.CreateVersionArchiveRequest
...
func (h *Handler) Update(c *gin.Context) {
	...
	tenantID := c.GetString("tenant_id")
	id := c.Param("id")
	var req models.UpdateVersionArchiveRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		errors.WriteError(c, errors.ErrBadRequest, err.Error(), 400)
		return
	}
