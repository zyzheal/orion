...
func (m *mockSecurityRepo) GetByID(_ context.Context, tenantID, id string) (*models.Vulnerability, error) {
	...
	if !ok { return nil, errors.New("vulnerability not found") }
	return v, nil
}
...
func (m *mockSecurityRepo) GetByCVEID(_ context.Context, tenantID, cveID string) (*models.Vulnerability, error) {
	...
	for _, v := range m.vulns {
		if v.TenantID == tenantID && v.CVEID == cveID { return v, nil }
	}
...
func (m *mockSecurityRepo) GetByCVEIDAndPackage(_ context.Context, tenantID, cveID, packageName string) (*models.Vulnerability, error) {
	if m.err != nil { return nil, m.err }
	for _, v := range m.vulns {
		if v.TenantID == tenantID && v.CVEID == cveID && v.PackageName == packageName { return v, nil }
	}
	return nil, errors.New("vulnerability not found")
}
...
func (m *mockSecurityRepo) List(_ context.Context, tenantID string, opt models.ListVulnerabilitiesOptions) ([]models.Vulnerability, int, error) {
	...
	for _, v := range m.vulns {
		...
		if opt.Severity != "" && v.Severity != opt.Severity { continue }
...
func TestSecurityErrInvalidInput(t *testing.T) {
	if !errors.Is(ErrInvalidInput, ErrInvalidInput) { t.Error("ErrInvalidInput should be self") }
}
...
func TestSecurityRemediationActions(t *testing.T) {
	...
	if !validActions[models.VulnerabilityStatusRemediated] { t.Error("remediated should be valid") }
	if !validActions[models.VulnerabilityStatusIgnored] { t.Error("ignored should be valid") }
	if !validActions[models.VulnerabilityStatusFalsePositive] { t.Error("false_positive should be valid") }
	if validActions[models.VulnerabilityStatusOpen] { t.Error("open should NOT be valid for remediation") }
}
...
func TestMockSecurityRepoCreate_Success(t *testing.T) {
	...
	if v.ID != "v-1" { t.Errorf("expected v-1, got %s", v.ID) }
	if v.Status != models.VulnerabilityStatusOpen { t.Errorf("expected open, got %s", v.Status) }
}
...
func TestMockSecurityRepoGetByID_Success(t *testing.T) {
	...
	if err != nil { t.Fatalf("expected no error, got %v", err) }
	if got.PackageName != "lodash" { t.Errorf("expected lodash, got %s", got.PackageName) }
}
...
func TestMockSecurityRepoGetByCVEID_Success(t *testing.T) {
	...
	if err != nil { t.Fatalf("expected no error, got %v", err) }
	if got.CVEID != "CVE-2021-1" { t.Errorf("expected CVE-2021-1, got %s", got.CVEID) }
}
...
func TestMockSecurityRepoList_FilteredBySeverity(t *testing.T) {
	...
	if items[0].Severity != models.VulnerabilitySeverityHigh { t.Errorf("expected high, got %s", items[0].Severity) }
}
...
func TestMockSecurityRepoUpdateStatus_Success(t *testing.T) {
	...
	if err != nil { t.Fatalf("expected no error, got %v", err) }
	if v.Status != models.VulnerabilityStatusRemediated { t.Errorf("expected remediated, got %s", v.Status) }
}
