...
func (r *Repository) Create(ctx context.Context, m *models.Team) error {
	...
	_, err := r.db.NamedExecContext(ctx,
		`INSERT INTO teams (id, tenant_id, name, created_at, updated_at) VALUES (:id, :tenant_id, :name, :created_at, :updated_at)`, m)
	return err
}
...
func (r *Repository) List(ctx context.Context, tenantID string, limit, offset int) ([]models.Team, error) {
	if limit <= 0 {
		limit = 50
	}
	var items []models.Team
	err := r.db.SelectContext(ctx, &items, `SELECT * FROM teams WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`, tenantID, limit, offset)
	if err != nil {
		return nil, err
	}
	return items, nil
...
func (r *Repository) Update(ctx context.Context, tenantID, id string, updates map[string]interface{}) error {
	...
	_, err := r.db.ExecContext(ctx, `UPDATE teams SET updated_at = NOW() WHERE id=$1 AND tenant_id=$2`, id, tenantID)
	if err != nil {
