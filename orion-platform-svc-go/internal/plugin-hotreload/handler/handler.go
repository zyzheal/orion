...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	r := rg.Group("/plugin-hotreload")
	r.GET("", auth.RequirePermission("plugin_hotreload", "read"), h.List)
	r.GET("/:id", auth.RequirePermission("plugin_hotreload", "read"), h.Get)
	r.POST("", auth.RequirePermission("plugin_hotreload", "write"), h.Create)
	r.PUT("/:id", auth.RequirePermission("plugin_hotreload", "write"), h.Update)
	r.DELETE("/:id", auth.RequirePermission("plugin_hotreload", "delete"), h.Delete)
}
...
func (h *Handler) Create(c *gin.Context) {
	...
	defer span.End()
	tenantID := c.GetString("tenant_id")
	var req models.CreatePluginHotreloadRequest
...
func (h *Handler) Update(c *gin.Context) {
	...
	tenantID := c.GetString("tenant_id")
	id := c.Param("id")
	var req models.UpdatePluginHotreloadRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		errors.WriteError(c, errors.ErrBadRequest, err.Error(), 400)
		return
	}
