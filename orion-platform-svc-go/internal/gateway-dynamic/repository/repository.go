...
func (r *Repository) Create(ctx context.Context, m *models.GatewayRoute) error {
	...
	_, err := r.db.NamedExecContext(ctx,
		`INSERT INTO gateway_routes (id, tenant_id, name, created_at, updated_at) VALUES (:id, :tenant_id, :name, :created_at, :updated_at)`, m)
	return err
}
...
func (r *Repository) List(ctx context.Context, tenantID string, limit, offset int) ([]models.GatewayRoute, error) {
	...
	err := r.db.SelectContext(ctx, &items, `SELECT * FROM gateway_routes WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`, tenantID, limit, offset)
	if err != nil {
		return nil, err
	}
	return items, nil
...
func (r *Repository) Delete(ctx context.Context, tenantID, id string) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM gateway_routes WHERE id=$1 AND tenant_id=$2`, id, tenantID)
	return err
}
