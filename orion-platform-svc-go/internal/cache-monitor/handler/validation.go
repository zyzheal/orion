...
func (v *CacheMonitorValidator) ValidateName(name string) error {
	if strings.TrimSpace(name) == "" {
		return ErrCacheMonitorInvalidInput
	}
	if len(name) > v.MaxNameLength {
		return fmt.Errorf("cache-monitor: name too long")
	}
...
func (v *CacheMonitorValidator) ValidateDescription(desc string) error {
	if len(desc) > v.MaxDescLength {
		return fmt.Errorf("cache-monitor: description too long")
	}
	return nil
}
...
type CacheMonitorValidationResult struct {
	Valid  bool
	Errors []string
}
