...
type PromptSecurityError struct {
	Code    string
	Message string
	Cause   error
}
...
func (e *PromptSecurityError) Is(target error) bool {
	_, ok := target.(*PromptSecurityError)
	return ok
}
...
func NewPromptSecurityError(code, msg string) error {
	return &PromptSecurityError{Code: code, Message: msg}
}
