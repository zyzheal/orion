...
func (v *PromptSecurityValidator) ValidateName(name string) error {
	if strings.TrimSpace(name) == "" {
		return ErrPromptSecurityInvalidInput
	}
	if len(name) > v.MaxNameLength {
		return fmt.Errorf("prompt-security: name too long")
	}
...
func (v *PromptSecurityValidator) ValidateDescription(desc string) error {
	if len(desc) > v.MaxDescLength {
		return fmt.Errorf("prompt-security: description too long")
	}
	return nil
}
...
type PromptSecurityValidationResult struct {
	Valid  bool
	Errors []string
}
