...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
    r := rg.Group("/autonomous-pipeline")
    r.GET("", auth.RequirePermission("autonomous-pipeline", "read"), h.List)
	r.GET("/:id", auth.RequirePermission("autonomous-pipeline", "read"), h.Get)
	r.POST("", auth.RequirePermission("autonomous-pipeline", "write"), h.Create)
	r.PUT("/:id", auth.RequirePermission("autonomous-pipeline", "write"), h.Update)
	r.DELETE("/:id", auth.RequirePermission("autonomous-pipeline", "delete"), h.Delete)
	r.POST("/:id/trigger", auth.RequirePermission("autonomous-pipeline", "write"), h.Trigger)
	r.GET("/:id/status", auth.RequirePermission("autonomous-pipeline", "read"), h.GetStatus)
	r.GET("/templates", auth.RequirePermission("autonomous-pipeline", "read"), h.ListTemplates)
}
...
func (h *Handler) GetStatus(c *gin.Context) {
    ...
    tenantID := c.GetString("tenant_id")
    ctx := ctx
    record, err := h.svc.GetStatus(ctx, tenantID, c.Param("id"))
    if err != nil {
        errors.WriteError(c, errors.ErrNotFound, "not found", http.StatusNotFound)
        return
    }
    errors.WriteSuccess(c, gin.H{"status": record.Status})
}
...
func (h *Handler) UpdateConfig(c *gin.Context) {
    ...
    tenantID := c.GetString("tenant_id")
    ctx := ctx
    res, err := h.svc.UpdateConfig(ctx, tenantID)
    if err != nil {
        errors.WriteError(c, errors.ErrInternal, err.Error(), http.StatusInternalServerError)
        return
    }
...
func (h *Handler) ListModels(c *gin.Context) {
    ...
    tenantID := c.GetString("tenant_id")
    ctx := ctx
    models, err := h.svc.ListModels(ctx, tenantID)
    if err != nil {
        errors.WriteError(c, errors.ErrInternal, err.Error(), http.StatusInternalServerError)
        return
    }
    errors.WriteSuccess(c, gin.H{"data": models, "total": len(models)})
}
func (h *Handler) RegisterModel(c *gin.Context) {
    ...
    tenantID := c.GetString("tenant_id")
    ctx := ctx
    res, err := h.svc.RegisterModel(ctx, tenantID)
    if err != nil {
        errors.WriteError(c, errors.ErrInternal, err.Error(), http.StatusInternalServerError)
        return
    }
...
func (h *Handler) GetUtilization(c *gin.Context) {
    ...
    tenantID := c.GetString("tenant_id")
    ctx := ctx
    utilization, err := h.svc.GetUtilization(ctx, tenantID, c.Param("id"))
    if err != nil {
        errors.WriteError(c, errors.ErrInternal, err.Error(), http.StatusInternalServerError)
        return
    }
...
func (h *Handler) AddTag(c *gin.Context) {
    ...
    tenantID := c.GetString("tenant_id")
    ctx := ctx
    res, err := h.svc.AddTag(ctx, tenantID, c.Param("id"))
    if err != nil {
        errors.WriteError(c, errors.ErrNotFound, "not found", http.StatusNotFound)
        return
    }
...
func (h *Handler) DeleteTag(c *gin.Context) {
    ...
    tenantID := c.GetString("tenant_id")
    ctx := ctx
    res, err := h.svc.DeleteTag(ctx, tenantID, c.Param("id"))
    if err != nil {
        errors.WriteError(c, errors.ErrNotFound, "not found", http.StatusNotFound)
        return
    }
...
func (h *Handler) BatchCreate(c *gin.Context) {
    ...
    res, err := h.svc.BatchCreate(ctx, tenantID, reqs)
    if err != nil {
        errors.WriteError(c, errors.ErrInternal, err.Error(), http.StatusInternalServerError)
        return
    }
