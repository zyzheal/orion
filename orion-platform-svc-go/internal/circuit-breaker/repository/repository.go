...
func (r *Repository) GetByID(ctx context.Context, id, tenantID string) (*models.CircuitBreaker, error) {
	...
	err := r.db.GetContext(ctx, &entity,
		`SELECT * FROM circuit_breakers WHERE id=$1 AND tenant_id=$2`, id, tenantID)
	if err != nil {
		return nil, err
	}
	return &entity, nil
...
func (r *Repository) List(ctx context.Context, tenantID string) ([]models.CircuitBreaker, error) {
	...
	err := r.db.SelectContext(ctx, &entities,
		`SELECT * FROM circuit_breakers WHERE tenant_id=$1 ORDER BY created_at DESC`, tenantID)
	if err != nil {
		return nil, err
	}
	return entities, nil
...
func (r *Repository) Update(ctx context.Context, id, tenantID string, attrs map[string]interface{}) (*models.CircuitBreaker, error) {
	...
	query := fmt.Sprintf("UPDATE circuit_breakers SET %s WHERE id=$%d AND tenant_id=$%d",
...
func (r *Repository) Delete(ctx context.Context, id, tenantID string) (bool, error) {
	result, err := r.db.ExecContext(ctx,
		`DELETE FROM circuit_breakers WHERE id=$1 AND tenant_id=$2`, id, tenantID)
	if err != nil {
		return false, err
	}
	n, _ := result.RowsAffected()
