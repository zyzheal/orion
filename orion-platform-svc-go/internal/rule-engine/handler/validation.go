...
func (v *RuleEngineValidator) ValidateName(name string) error {
	if strings.TrimSpace(name) == "" {
		return ErrRuleEngineInvalidInput
	}
	if len(name) > v.MaxNameLength {
		return fmt.Errorf("rule-engine: name too long")
	}
...
func (v *RuleEngineValidator) ValidateDescription(desc string) error {
	if len(desc) > v.MaxDescLength {
		return fmt.Errorf("rule-engine: description too long")
	}
	return nil
}
...
type RuleEngineValidationResult struct {
	Valid  bool
	Errors []string
}
