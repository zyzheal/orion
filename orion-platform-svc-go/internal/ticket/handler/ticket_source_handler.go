...
func (h *TicketSourceHandler) FromAlert(c *gin.Context) {
	ctx, span := otel.Tracer("orion-platform-svc").Start(c.Request.Context(), "TicketFromAlert")
	defer span.End()
...
func (h *TicketSourceHandler) FromIncident(c *gin.Context) {
	ctx, span := otel.Tracer("orion-platform-svc").Start(c.Request.Context(), "TicketFromIncident")
	defer span.End()
	tenantID := c.GetString("tenant_id")
	createdBy := GetUserID(c)
