...
func (qm *QueueManager) GetSLAQueueEntries(ctx context.Context) ([]models.SLAQueueEntry, error) {
	...
	for _, entry := range entries {
		...
		if slaRecord != nil {
			...
			// Check SLA status
			elapsed := now.Sub(slaRecord.CreatedAt)
			total := slaRecord.ResolutionDeadlineAt.Sub(slaRecord.CreatedAt)
			if total > 0 {
				percent := float64(elapsed) / float64(total) * 100
				if percent >= 90 {
					slaEntry.EscalationLevel = 2
				} else if percent >= 75 {
					slaEntry.EscalationLevel = 1
				}
			}
		}
...
func (qm *QueueManager) GetSLAAlerts(ctx context.Context, alertType *models.SLAAlertType, limit int) ([]models.QueueAlert, error) {
	...
	for _, entry := range entries {
		if entry.SLADeadline == nil {
			continue
		}

		total := entry.SLADeadline.Sub(entry.EnqueuedAt)
		...
		var alert models.SLAAlertType
		if percent >= 100 {
			alert = models.SLAAlertBreach
		} else if percent >= 90 {
			alert = models.SLAAlertCritical
		} else if percent >= 75 {
			alert = models.SLAAlertWarning
		} else {
			continue
		}

		if alertType != nil && alert != *alertType {
			continue
		}

		alerts = append(alerts, models.QueueAlert{
			Type:     alert,
			TicketID: entry.TicketID,
			Priority: entry.Priority,
			Message:  fmt.Sprintf("Ticket %s: %s (%.0f%% elapsed)", entry.TicketID, alert, percent),
			Since:    entry.EnqueuedAt,
...
func (qm *QueueManager) calculateSLAPriority(entry models.DispatchQueueEntry, now time.Time) float64 {
	...
	attemptBoost := float64(entry.Attempts) * 0.5
