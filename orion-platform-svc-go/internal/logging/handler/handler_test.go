...
func (m *testRepo) GetByID(ctx context.Context, tid, id string) (*models.LogEntry, error) {
	return &models.LogEntry{ID: "x"}, nil
}
...
func (m *testRepo) Aggregation(ctx context.Context, q *models.LogQuery) (*models.LogAggregation, error) {
	...
	return &models.LogAggregation{Total: 10, ByLevel: map[string]int64{"INFO": 8, "ERROR": 2}, ByService: map[string]int64{"api": 5},
		...
		}{From: time.Now().Add(-24 * time.Hour), To: time.Now()}}, nil
}
func (m *testRepo) DeleteByTime(ctx context.Context, tid string, before time.Time) (int64, error) {
	m.delN = 7
	return 7, nil
}
...
func TestHandler_Ingest_OK(t *testing.T) {
	...
	if e.TenantID != "tenant-1" {
		t.Errorf("TenantID=%s want tenant-1", e.TenantID)
	}
	if e.TraceID != "t123" {
		t.Errorf("TraceID=%s want t123", e.TraceID)
	}
	if _, err := uuid.Parse(e.ID); err != nil {
		t.Errorf("ID not UUID: %s", e.ID)
	}
}
...
func TestHandler_Ingest_UUID_Generated(t *testing.T) {
	...
	if _, err := uuid.Parse(repo.entries[0].ID); err != nil {
		t.Fatalf("ID not UUID: %s (%v)", repo.entries[0].ID, err)
	}
}
...
func TestHandler_Ingest_CustomTimestamp(t *testing.T) {
	...
	if !repo.entries[0].Timestamp.Equal(ts) {
		t.Errorf("Timestamp=%v want %v", repo.entries[0].Timestamp, ts)
	}
}
...
func TestHandler_IngestBatch_OK(t *testing.T) {
	...
	if len(repo.entries) != 2 {
		t.Fatalf("got %d entries want 2", len(repo.entries))
	}
	var body map[string]interface{}
	json.NewDecoder(w.Body).Decode(&body)
	data := body["data"].(map[string]interface{})
	if data["ingested"].(float64) != 2 {
		t.Errorf("ingested=%v want 2", data["ingested"])
	}
}
...
func TestHandler_GetByTrace_OK(t *testing.T) {
	...
	if len(data) < 1 {
		t.Error("expected at least 1 trace entry")
	}
}
...
func TestHandler_Search_OK(t *testing.T) {
	...
	data := inner["data"].([]interface{})
	if len(data) != 1 {
		t.Errorf("got %d results want 1", len(data))
	}
}
...
func TestHandler_Aggregation_OK(t *testing.T) {
	...
	if agg["total"].(float64) != 10 {
		t.Errorf("total=%v want 10", agg["total"])
	}
}
...
func TestHandler_CleanupOld_OK(t *testing.T) {
	...
	if repo.delN != 7 {
		t.Errorf("deleted=%d want 7", repo.delN)
	}
}
...
func TestHandler_getTenantID_Present(t *testing.T) {
	...
	result := h.getTenantID(c)
	if result != "t-test" {
		t.Errorf("tenantID=%s want t-test", result)
	}
}
...
func TestHandler_RegisterRoutes(t *testing.T) {
	...
	if len(r.Routes()) < 5 {
		t.Fatalf("expected >=5 routes, got %d", len(r.Routes()))
	}
}
