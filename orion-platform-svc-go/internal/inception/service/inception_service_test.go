...
func TestSubmitToEngine_CheckSuccess(t *testing.T) {
	...
	if f.checkCalls != 1 {
		t.Fatalf("expected 1 CheckSQL call, got %d", f.checkCalls)
	}
	if affected == nil || *affected != 3 {
		t.Fatalf("expected affected=3, got %v", affected)
	}
	if execMs == nil || *execMs != 42 {
		t.Fatalf("expected exec_ms=42, got %v", execMs)
	}
	if len(warnings) != 1 || warnings[0] != "w1" {
		t.Fatalf("expected [w1], got %v", warnings)
	}
	if len(errs) != 0 {
		t.Fatalf("expected no errors, got %v", errs)
	}
	if f.lastCheck.TaskName != "audit-1" {
		t.Fatalf("expected task_name=audit-1, got %q", f.lastCheck.TaskName)
	}
	if !f.lastCheck.DryRun {
		t.Fatal("non-execute path should set DryRun=true")
	}
}
...
func TestSubmitToEngine_ExecuteUsesExecuteSQL(t *testing.T) {
	...
	if f.checkCalls != 0 || f.execCalls != 1 {
		t.Fatalf("expected only ExecuteSQL, got check=%d exec=%d", f.checkCalls, f.execCalls)
	}
	if f.lastExec.DryRun {
		t.Fatal("execute path should not set DryRun=true")
	}
}
...
func TestSubmitToEngine_TransportErrorPersistsFailed(t *testing.T) {
	...
	if len(errs) != 1 || errs[0] == "" {
		t.Fatalf("expected engine error, got %v", errs)
	}
}
...
func TestSubmitToEngine_SkipsTerminalStatus(t *testing.T) {
	...
	if status != "success" {
		t.Fatalf("expected success pass-through, got %q", status)
	}
	if f.checkCalls != 0 {
		t.Fatalf("expected no engine call, got %d", f.checkCalls)
	}
}
...
func TestSubmitToEngine_NonSuccessResultPersistsFailed(t *testing.T) {
	...
	if len(errs) != 1 || errs[0] != "syntax error" {
		t.Fatalf("expected syntax error, got %v", errs)
	}
}
...
func TestHealth_NoClientReportsNotConfigured(t *testing.T) {
	...
	if st != "not_configured" {
		t.Fatalf("expected not_configured, got %q", st)
	}
}
...
func TestHealth_ClientDownReportsUnreachable(t *testing.T) {
	...
	if st != "unreachable" {
		t.Fatalf("expected unreachable, got %q", st)
	}
}
...
func TestSetEngineClient(t *testing.T) {
	...
	if st, _ := s.Health(context.Background()); st != "ok" {
		t.Fatalf("expected ok, got %q", st)
	}
}
