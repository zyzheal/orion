...
func (v *CodeEmbeddingValidator) ValidateName(name string) error {
	if strings.TrimSpace(name) == "" {
		return ErrCodeEmbeddingInvalidInput
	}
	if len(name) > v.MaxNameLength {
		return fmt.Errorf("code-embedding: name too long")
	}
...
func (v *CodeEmbeddingValidator) ValidateDescription(desc string) error {
	if len(desc) > v.MaxDescLength {
		return fmt.Errorf("code-embedding: description too long")
	}
	return nil
}
...
type CodeEmbeddingValidationResult struct {
	Valid  bool
	Errors []string
}
