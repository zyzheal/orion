...
func TestHandler_NOTIFICATION_RegisterRoutes(t *testing.T) {
	_ = newHandler()
}
...
func TestHandler_NOTIFICATION_GetStats(t *testing.T) {
	t.Skip("handler uses concrete service, cannot inject mock")
	c, w := makeCtx(http.MethodGet, "/")
...
func TestHandler_NOTIFICATION_MarkRead(t *testing.T) {
	t.Skip("handler uses concrete service, cannot inject mock")
	c, w := makeCtx(http.MethodGet, "/")
	newHandler().MarkRead(c)
	if w.Code >= 500 {
		t.Fatalf("MarkRead: got %d", w.Code)
	}
}
func TestHandler_NOTIFICATION_MarkAllRead(t *testing.T) {
	t.Skip("handler uses concrete service, cannot inject mock")
	c, w := makeCtx(http.MethodGet, "/")
	newHandler().MarkAllRead(c)
	if w.Code >= 500 {
		t.Fatalf("MarkAllRead: got %d", w.Code)
	}
}
