...
// Repository is the sqlx-backed persistence layer for the middleware-ops module.
...
func (r *Repository) List(ctx context.Context, tenantID string) ([]models.Record, error) {
	...
	err := r.db.SelectContext(ctx, &rows,
		"SELECT * FROM middleware_ops_records WHERE tenant_id=$1 AND deleted_at IS NULL ORDER BY created_at DESC",
		tenantID)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) GetByID(ctx context.Context, tenantID, id string) (*models.Record, error) {
	...
	err := r.db.GetContext(ctx, &row,
		"SELECT * FROM middleware_ops_records WHERE id=$1 AND tenant_id=$2 AND deleted_at IS NULL", id, tenantID)
...
func (r *Repository) Create(ctx context.Context, tenantID string, req models.CreateRequest) (*models.Record, error) {
	...
	_, err = r.db.ExecContext(ctx,
		"INSERT INTO middleware_ops_records (id, tenant_id, name, status, metadata, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $6)",
...
func (r *Repository) Update(ctx context.Context, tenantID, id string, req models.CreateRequest) (*models.Record, error) {
	...
	result, err := r.db.ExecContext(ctx,
		"UPDATE middleware_ops_records SET name=$3, status=$4, metadata=$5, updated_at=$6 WHERE id=$1 AND tenant_id=$2 AND deleted_at IS NULL",
...
func (r *Repository) Delete(ctx context.Context, tenantID, id string) error {
	now := time.Now().UTC()
	result, err := r.db.ExecContext(ctx,
		"UPDATE middleware_ops_records SET deleted_at=$3 WHERE id=$1 AND tenant_id=$2 AND deleted_at IS NULL",
		id, tenantID, now)
	if err != nil {
		return err
	}
