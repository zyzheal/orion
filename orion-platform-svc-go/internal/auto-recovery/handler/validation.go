...
func (v *AutoRecoveryValidator) ValidateName(name string) error {
	if strings.TrimSpace(name) == "" {
		return ErrAutoRecoveryInvalidInput
	}
	if len(name) > v.MaxNameLength {
		return fmt.Errorf("auto-recovery: name too long")
	}
...
func (v *AutoRecoveryValidator) ValidateDescription(desc string) error {
	if len(desc) > v.MaxDescLength {
		return fmt.Errorf("auto-recovery: description too long")
	}
	return nil
}
...
type AutoRecoveryValidationResult struct {
	Valid  bool
	Errors []string
}
