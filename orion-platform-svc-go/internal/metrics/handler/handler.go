...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	r := rg.Group("/metrics")
	r.GET("", auth.RequirePermission("metrics", "read"), h.List)
	r.GET("/:id", auth.RequirePermission("metrics", "read"), h.Get)
	r.POST("", auth.RequirePermission("metrics", "write"), h.Create)
	r.PUT("/:id", auth.RequirePermission("metrics", "write"), h.Update)
	r.DELETE("/:id", auth.RequirePermission("metrics", "delete"), h.Delete)
}
...
func (h *Handler) Update(c *gin.Context) {
	tenantID := c.GetString("tenant_id")
	id := c.Param("id")
	var req models.UpdateMetricsRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		errors.WriteError(c, errors.ErrBadRequest, err.Error(), 400)
		return
	}
