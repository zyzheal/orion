...
func (r *Repository) List(ctx context.Context, tenantID string) ([]models.Record, error) {
    ...
    err := r.db.SelectContext(ctx, &records, "SELECT * FROM metadatas WHERE tenant_id=$1", tenantID)
    return records, err
}

func (r *Repository) GetByID(ctx context.Context, tenantID, id string) (*models.Record, error) {
    var record models.Record
    err := r.db.GetContext(ctx, &record, "SELECT * FROM metadatas WHERE id=$1 AND tenant_id=$2", id, tenantID)
