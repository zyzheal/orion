...
func NewAlertSilenceRepository(db *sql.DB, logger *zap.Logger) *AlertSilenceRepository {
...
func (r *AlertSilenceRepository) Query(ctx context.Context, tenantID uuid.UUID, status string, limit, offset int) (models.SilenceResponse, error) {
	...
	if err := r.db.QueryRowContext(ctx, countQuery, args[:len(args)-2]...).Scan(&resp.Total); err != nil {
...
func (r *AlertSilenceRepository) Delete(ctx context.Context, tenantID, id uuid.UUID) error {
	result, err := r.db.ExecContext(ctx, `DELETE FROM alert_silences WHERE id = $1 AND tenant_id = $2`, id, tenantID)
	if err != nil {
...
// Extend extends the duration of a silence record.
func (r *AlertSilenceRepository) Extend(ctx context.Context, tenantID uuid.UUID, id uuid.UUID, extendBy int) error {
	newExpiresAt, now := time.Now(), time.Now()
	query := `UPDATE alert_silences SET duration = duration + $1, expires_at = $2, updated_at = $3 WHERE id = $4 AND tenant_id = $5`
	_, err := r.db.ExecContext(ctx, query, extendBy, newExpiresAt, now, id, tenantID)
	if err != nil {
		return err
	}
	return nil
