...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	r := rg.Group("/script-version")
	r.GET("", auth.RequirePermission("script_version", "read"), h.List)
	r.GET("/:id", auth.RequirePermission("script_version", "read"), h.Get)
	r.POST("", auth.RequirePermission("script_version", "write"), h.Create)
	r.PUT("/:id", auth.RequirePermission("script_version", "write"), h.Update)
	r.DELETE("/:id", auth.RequirePermission("script_version", "delete"), h.Delete)
}
...
func (h *Handler) Create(c *gin.Context) {
	tenantID := c.GetString("tenant_id")
	var req models.CreateScriptVersionRequest
...
func (h *Handler) Update(c *gin.Context) {
	tenantID := c.GetString("tenant_id")
	id := c.Param("id")
	var req models.UpdateScriptVersionRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		errors.WriteError(c, errors.ErrBadRequest, err.Error(), 400)
		return
	}
