...
func (r *Repository) Create(ctx context.Context, m *models.ServiceTopology) error {
	...
	_, err := r.db.NamedExecContext(ctx, `
		INSERT INTO service-topology (id, tenant_id, name, value, enabled, created_at, updated_at)
...
func (r *Repository) GetByID(ctx context.Context, tenantID, id string) (*models.ServiceTopology, error) {
	...
	err := r.db.GetContext(ctx, &m, `SELECT * FROM service-topology WHERE id = $1 AND tenant_id = $2`, id, tenantID)
	if err != nil {
...
func (r *Repository) List(ctx context.Context, tenantID string) ([]models.ServiceTopology, error) {
	...
	err := r.db.SelectContext(ctx, &items, `SELECT * FROM service-topology WHERE tenant_id = $1 ORDER BY created_at DESC`, tenantID)
	return items, err
}
...
func (r *Repository) Update(ctx context.Context, tenantID, id string, updates map[string]interface{}) (*models.ServiceTopology, error) {
	...
	_, err := r.db.ExecContext(ctx,
		"UPDATE service-topology SET "+strings.Join(setParts, ", ")+
...
func (r *Repository) Delete(ctx context.Context, tenantID, id string) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM service-topology WHERE id = $1 AND tenant_id = $2`, id, tenantID)
	return err
}
