...
func (r *Repository) Create(ctx context.Context, d *models.IntelligenceTask) error {
	_, err := r.db.ExecContext(ctx, `INSERT INTO intelligence_tasks (id, tenant_id, name, insight_type, source, confidence, data, status, created_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
		d.ID, d.TenantID, d.Name, d.InsightType, d.Source, d.Confidence, d.Data, d.Status, d.CreatedAt)
	return err
}
...
func (r *Repository) Delete(ctx context.Context, tenantID, id string) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM intelligence_tasks WHERE id=$1 AND tenant_id=$2`, id, tenantID)
	return err
}

func (r *Repository) Count(ctx context.Context, tenantID string) (int, error) {
	var count int
	err := r.db.GetContext(ctx, &count, `SELECT COUNT(*) FROM intelligence_tasks WHERE tenant_id=$1`, tenantID)
	return count, err
}
