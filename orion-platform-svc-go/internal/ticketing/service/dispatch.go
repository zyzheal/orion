...
func (s *DispatchService) RegisterEngineer(ctx context.Context, req *models.RegisterEngineerRequest) (*models.DispatchEngineer, error) {
...
func (s *DispatchService) ListEngineers(ctx context.Context) ([]models.DispatchEngineer, error) {
	return s.engineerRepo.ListEngineersAll(ctx)
}

// GetEngineer returns a single engineer
func (s *DispatchService) GetEngineer(ctx context.Context, id string) (*models.DispatchEngineer, error) {
	return s.engineerRepo.GetEngineerByID(ctx, id)
}
...
func (s *DispatchService) ManualDispatch(ctx context.Context, ticketID, tenantID, engineerID, assignedBy, reason string) (*models.DispatchRecord, error) {
	...
	_, err := s.engineerRepo.GetEngineerByID(ctx, engineerID)
	if err != nil {
...
func (s *DispatchService) FindBestEngineer(ctx context.Context, ticket *models.Ticket) (*models.DispatchMatch, error) {
	engineers, err := s.engineerRepo.ListEngineersAll(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *DispatchService) CalculateDispatchScore(ctx context.Context, ticketID, tenantID, engineerID string) (*models.DispatchMatch, error) {
	...
	engineer, err := s.engineerRepo.GetEngineerByID(ctx, engineerID)
	if err != nil {
		return nil, fmt.Errorf("engineer not found: %w", err)
	}
...
func (s *DispatchService) calculateScore(ticket *models.Ticket, eng *models.DispatchEngineer) float64 {
	// Expertise match
	expertiseScore := 0.0
	for _, exp := range strings.Split(eng.Expertise, ",") {
...
func (s *DispatchService) getMatchReasons(ticket *models.Ticket, eng *models.DispatchEngineer, score float64) []string {
	var reasons []string
	for _, exp := range strings.Split(eng.Expertise, ",") {
...
func (s *DispatchService) GetEngineerPerformance(ctx context.Context, engineerID string) (*models.EngineerPerformance, error) {
	eng, err := s.engineerRepo.GetEngineerByID(ctx, engineerID)
	if err != nil {
		return nil, err
	}
