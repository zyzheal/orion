...
func (lb *LoadBalancer) GetBalancingReport(ctx context.Context) (*models.LoadBalanceReport, error) {
	engineers, err := lb.dispatchRepo.ListEngineersAll(ctx)
	if err != nil {
		return nil, err
	}
...
func (lb *LoadBalancer) SuggestReassignments(ctx context.Context) ([]models.ReassignmentSuggestion, error) {
	...
	var overloaded, underutilized []models.DispatchEngineer
	for _, eng := range engineers {
		if eng.MaxCapacity == 0 {
			continue
		}
...
func (lb *LoadBalancer) CheckEngineerCapacity(ctx context.Context, engineerID string) (*models.EngineerCapacityCheck, error) {
	eng, err := lb.dispatchRepo.GetEngineerByID(ctx, engineerID)
	if err != nil {
...
func (lb *LoadBalancer) GetAvailableEngineers(ctx context.Context) ([]models.DispatchEngineer, error) {
	engineers, err := lb.dispatchRepo.ListEngineersAll(ctx)
	if err != nil {
		return nil, err
	}

	var available []models.DispatchEngineer
	for _, eng := range engineers {
