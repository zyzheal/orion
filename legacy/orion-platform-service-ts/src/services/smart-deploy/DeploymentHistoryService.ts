 ...
 */
function toEntity(deployment: Deployment): Omit<DeploymentHistoryEntity, 'createdAt'> & { createdAt: Date } {
  const now = new Date();
  return {
    id: deployment.id,
    tenantId: 'default',
...
export class DeploymentHistoryService {
  ...
  async getMetrics(filters?: {
    ...
    if (filters?.appName) {
      filtered = filtered.filter(d => d.appName === filters.appName);
    }
    ...
    const successful = filtered.filter(d => d.status === 'completed').length;
    const failed = filtered.filter(d => d.status === 'failed').length;
    const rolledBack = filtered.filter(d => d.status === 'rolled_back').length;
