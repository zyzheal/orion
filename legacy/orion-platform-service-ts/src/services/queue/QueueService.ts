 ...
 * queue statistics.
 *
 * Backed exclusively by PostgreSQL via JobRepository.
 */
...
export class QueueService {
  ...
  async listJobs(options: ListJobsOptions = {}): Promise<PaginatedJobsResult> {
    ...
    // If tenantId specified, use tenant-scoped query
    if (options.tenantId && options.tenantId.trim() !== '') {
      ...
      // Apply additional filters on top of tenant results
      let filtered = jobs;
      if (options.status) {
    ...
    // No tenantId: list all jobs across statuses
    const allJobs: Job[] = [];
    if (options.status) {
    ...
    if (options.tenantId) {
      filtered = filtered.filter((j) => j.tenantId === options.tenantId);
    }
    ...
    return {
      ...
      page,
      limit,
      totalPages: Math.ceil(paginated.length / limit),
    };
  }
