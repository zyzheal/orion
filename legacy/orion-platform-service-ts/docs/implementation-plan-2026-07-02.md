...
| 1.3 | 212 个手动错误返回 → 统一使用 OrionError | ✅ 已完成 (2026-07-04) | Phase 1.3 Agent 执行 | 🟡 | 7文件多处替换，tsc通过 |
...
| 1.8 | 统一 API 路径前缀为 /api/v1/\<domain\>/ | 🔵 已确认方案 | 2026-07-04 | 🔴 → 🔵 | **已决策**：继续沿用 `/api/v1/<domain>/` 模式统一所有裸前缀（/config、/users、/pipelines 等约 100 个） |
| 1.9 | 更新前端 API 客户端 baseURL 一致性（239 个） | 🔵 待执行 | — | 🔵 | 依赖 1.8（已确认方案，可执行） |
...
| 1.13 | 路由级 ACL 中间件 | 🟡 待执行 | — | 🟡 | 依赖 1.8（已确认方案，可执行） |
...
| 2.20 | refresh_tokens 表缺 tenant_id | ✅ 已完成 (2026-07-04) | routes-auth.ts + sso-unified-routes.ts 均已支持 tenant_id：INSERT 含 $4 tenant_id 参数，SELECT 从 tenant_users 查询 | 🔵 | 代码验证通过 |
...
| # | 任务 | 状态 | 完成日期 | Agent | 备注 |
|---|------|------|---------|-------|------|
| 2.13 | VectorStore 向量搜索缺失 | ✅ 已完成 (2026-07-04) | 代码审计通过，3处不可达代码已确认修复，addDocument/search/deleteDocument 均正确使用 pgvector 余弦相似度 | 🟡 | vector-store-routes.ts + VectorRepository.ts 验证通过 |
| 2.14 | DBA 直接查询执行缺失 | ✅ 已完成 (2026-07-03) | DbaService.executeDirectQuery() 完整实现 + 路由注册 + dba-routes.test.ts | 🔵 | |
| 2.21 | 多渠道实际投递缺失 | ✅ 已完成 (2026-07-04) | NotificationService 通过 eventPublisher.publish() 实现多通道投递，orion-notify-svc 消费事件进行多通道分发 | 🟡 | dual-write: DB + event bus |
...
| 2.38 | 统一 FallbackStorageService | ✅ 已完成 (2026-07-04) | 14文件已集成: PermissionService, LoginAttemptService, TokenBlacklistService, DeploymentEventService, PromotionService, ArtifactOperationService, AdapterRegistryService, OCIRegistryService 等 | 🟡 | 3-tier health-check-based 降级 |
...
| # | 任务 | 状态 | 完成日期 | Agent | 备注 |
|---|------|------|---------|-------|------|
| 4.1 | OCI/Docker Registry 对接 | ✅ 已完成 (2026-07-04) | oci-registry-routes.ts (188行, 10个RESTful端点) + OCIRegistryService + DockerRegistryClient (853行, 支持Basic Auth + Bearer Token双认证) + routes.ts注册 /api/v1/oci | 🟡 | 代码验证通过 |
| 4.2 | PromotionService 内存级 → FallbackStorageService | ✅ 已完成 (2026-07-04) | PromotionService 已使用 FallbackStorageService 进行持久化，依赖 2.38 | 🟡 | 已验证源码 |
| 4.3 | ArtifactOperationService 内存降级 → FallbackStorageService | ✅ 已完成 (2026-07-04) | ArtifactOperationService 已使用 FallbackStorageService 进行持久化，依赖 2.38 | 🟡 | 已验证源码 |
...
| 4.8 | 内存 Map 降级数据丢失风险 → FallbackStorageService | ✅ 已完成 (2026-07-04) | Auth 模块所有服务已迁移: LoginAttemptService, TokenBlacklistService 均使用 FallbackStorageService，SsoService InMemorySsoStateStore 为 TTL 缓存无需持久化 | 🟡 | 依赖 2.38 |
...
| 4.19 | Code 内存 Map 适配器注册表 → FallbackStorageService | ✅ 已完成 (2026-07-04) | AdapterRegistryService 已使用 FallbackStorageService，WebhookService 双写 secretRepo，依赖 2.38 | 🟡 | 已验证源码 |
...
| 4.27 | DataPipeline 异步执行引擎 | ✅ 已完成 (2026-07-04) | DataPipelineAsyncEngine.ts + DataPipelineTaskScheduler.ts 已实现，含持久化 + 测试 | 🟡 | 已验证源码 |
...
| 4.64 | 前端页面需完善 | ✅ 已完成 (2026-07-04) | AIAgents/AuditLogViewer (CRUD+loading+onExecuteAgent) + AgentDashboard (edit button+modal+onEdit prop)，交互链完整 | 🔵 | Agent 执行完成 |
...
| 4.68 | 为 14 个仅 Go 服务编写 Spec 文档 | ✅ 已完成 (2026-07-04) | 14份 Go 服务 Spec 已存在于 docs/specs/ (chaos/feature-flag/governance/intelligence/llm/monitor/pipeline-template/runner/secret/skill-config/ticket/visor/workflow/eventbus)，新增 go-services/ 输出目录 | 🔵 | 代码验证通过 |
...
| 4.73 | 审计日志合规性检查（SOC2/ISO27001） | ✅ 已完成 (2026-07-04) | AuditService 实现 exportAuditLogs(CSV/JSON) + audit-routes.ts 注册 POST /export + POST /export/json + GET /logs/export（resourceId/dateFrom/dateTo 过滤），400/402 tests pass | 🔵 | 代码验证通过 |
...
| 6.2 | API Gateway 路由改为动态发现 | ✅ 已完成 (2026-07-04) | `orion-api-gateway/src/services/gateway-dynamic-routes.ts` (689行) + `orion-api-gateway/src/services/gateway-route-sync.ts` + `orion-api-gateway/src/routes/api.ts` 注册，syncWithServiceRegistry + listRoutes + CRUD | 🔴 | 代码验证通过 |
| 6.3 | 服务注册表 Repository（PostgreSQL） | ✅ 已完成 (2026-07-04) | ServiceRegistryRepository (254行) + service-registry-routes.ts + routes.ts 注册 /api/v1/service-registry | 🟡 | 代码验证通过 |
| 6.4 | 服务健康检查器 | ✅ 已完成 (2026-07-04) | HealthCheckerService.ts (854行) + health-check-routes.ts + routes.ts 注册 /health-checks | 🟡 | 代码验证通过 |
| 6.5 | ServiceRegistryPage 前端 | ✅ 已完成 (2026-07-04) | `orion-frontend/src/pages/service-registry/ServiceRegistry/index.tsx` (511行) + `orion-frontend/src/pages/ServiceRegistry/index.tsx` (180字节 re-export wrapper) + `service-registry.ts` API client | 🔵 | 代码验证通过 |
| 6.6 | 路由管理（基于 Go 迁移后的真实路由） | ✅ 已完成 (2026-07-04) | gateway-dynamic-routes.ts 含 CRUD (createRoute/updateRoute/deleteRoute/toggleRoute) + gateway-route-sync.ts | 🟡 | 依赖 6.2 代码验证通过 |
| 6.7 | GatewayRoutesPage 前端 | ✅ 已完成 (2026-07-04) | GatewayRoutes/index.tsx (860行) + gateway-routes.ts API client | 🔵 | 代码验证通过 |
| 6.8 | 健康仪表盘 | ✅ 已完成 (2026-07-04) | HealthDashboard/index.tsx (437行) + health.ts API client + 聚合多服务健康状态 | 🟡 | 代码验证通过 |
| 6.9 | 服务拓扑可视化 | ✅ 已完成 (2026-07-04) | service-topology/ServiceTopology/index.tsx (252行) + apm/ServiceTopology/index.tsx (258行) + service-topology.ts API client | 🔵 | 代码验证通过 |
| 6.10 | Go 迁移第二阶段：10 个业务服务 | 🟡 已确认方案 | — | 🔴 → 🟡 | **评审完成**：10/10 构建通过，分 3 批迁移（CMDB/Artifact/Code → Security/Monitor/Config → Skill/Approval/ChatOps/Ticket）。**待决策**：D-1 事件系统（建议复用 orion/go-common/pkg/eventbus）、D-2 服务间调用协议（建议 HTTP/REST + Event-driven） |
| 6.11 | 版本管理 + 流量治理 API | ✅ 已完成 (2026-07-03) | canary-traffic-routes + pipeline-version-routes + artifact-version-routes + TrafficManager | 🟡 | |
| 6.12 | 版本管理页面 + 流量治理页面 | ✅ 已完成 (2026-07-04) | VersionManagement/index.tsx (Pipeline/Artifact/Deploy Tab + 版本对比/回滚/基线) + TrafficGovernance/index.tsx (流量规则CRUD + 发布/回滚/权重调整) + routes.tsx 注册 /version-management /traffic-governance | 🔵 | 代码验证通过 |
| 6.13 | 集成到 Console 页面 + 端到端测试 | 🟡 部分完成 (2026-07-04) | Console 已集成 4 入口 (ServiceRegistry/GatewayRoutes/HealthDashboard/ServiceTopology)，Console.integration.test.tsx 已创建；VersionManagement/TrafficGovernance 路由已注册待 Console 菜单补充 | 🟡 | 测试文件已创建，待补充 6.12 菜单项 |
